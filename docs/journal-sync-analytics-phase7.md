...
- 일반 모의 원장은 마지막 서버 확인 버전을 `baseVersion`으로 별도 보존한다. 신규 기록은 null, 이전 메타데이터의 알 수 없는 기준은 필드 누락으로 구분하며 추측하지 않는다.
- 서버에 ID가 없고 이전 서버 버전도 확인한 적이 없을 때 신규 업로드한다. 이미 확인한 서버 기록이 사라졌으면 자동 재생성하지 않는다.
- 기기 version이 높고 `baseVersion`이 현재 서버 version과 일치할 때만 조건부 업로드한다.
- 서버 version이 높고 기기의 version이 `baseVersion`과 같아 로컬 변경이 없을 때만 다운로드한다.
- version과 내용이 같으면 unchanged로 처리한다.
- version이 같고 내용이 다르거나, 오프라인 수정의 서버 기준이 다르거나 알 수 없으면 충돌을 생성하고 두 버전을 보존한다. 수정 횟수가 큰 기기를 자동 우선하지 않는다.
...
- 서버 확인은 업로드·다운로드·변경 없음 응답을 실제로 적용한 후에만 갱신한다. 기기 시각이나 수정 횟수로 서버 확인을 추정하지 않는다.

위 기준은 HTTP로 동기화하는 일반 모의거래 원장에 적용된다. 서버 전용 다중 통화 연구 원장의 별도 저장 경로를 클라이언트가 선택할 수 없다. 이 보호는 다중 계좌·riskState·processedEvents 전체 복원이나 충돌 해결 트랜잭션의 운영 내구성 증거를 대신하지 않는다.
...
기기 버전 유지 시 서버와 기기 version 중 큰 값보다 1 높은 version으로 저장한다. 둘 다 보존은 거래일지에만 허용하며 원본 서버 기록과 연구 성과 집계에서 제외되는 사본을 남긴다. 주문·포지션·체결·계좌는 복제하지 않는다. 명시적 선택은 해당 충돌 기록에만 적용하며, 충돌 표시 후 추가 로컬 변경이 있으면 다시 동기화해야 한다. 해결된 충돌은 다시 처리할 수 없다.
...
- Secret 유사 키 거부
- 신규 업로드와 서버 다운로드
- 마지막 서버 확인 버전 검사, 오프라인 분기 충돌 및 알려진 서버 기록의 재생성 차단
- 같은 version 충돌
- tombstone
- idempotency
- 부분 실패
