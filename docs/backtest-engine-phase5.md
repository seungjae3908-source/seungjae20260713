...
각 인덱스 결과는 해당 인덱스까지의 데이터만 사용한다. 첫 유효값 이전와 데이터 부족 구간은 `null`이다. NaN과 Infinity는 결과에 사용하지 않는다.
...
Phase 5 단위·성능 테스트 73개와 전체 API 스모크 11개가 CI에서 성공했다. 성능 테스트는 1,000개, 10,000개, 최대 20,000개 완료 캔들을 실행하며 10,000개는 5초, 20,000개는 10초의 안정적인 상한을 적용한다.

CI run `30727457223`의 실제 측정값:

| 캔들 수 | 순수 계산 시간 | heap 증감 | 거래 수 | timeout |
|---:|---:|---:|---:|---|
| 1,000 | 14.10 ms | +3,533,136 bytes | 0 | 없음 |
| 10,000 | 74.45 ms | +2,379,120 bytes | 0 | 없음 |
| 20,000 | 54.47 ms | -2,028,944 bytes | 0 | 없음 |

fixture는 신호가 발생하지 않게 구성해 지표·순회 비용을 측정했다. heap 증감은 가비지 컬렉션 시점에 따라 음수가 될 수 있으므로 절대 메모리 사용량으로 해석하지 않는다. 마이크로초 수준의 고정 성능값을 CI 통과 조건으로 사용하지 않고 합리적인 초 단위 상한만 적용한다.
