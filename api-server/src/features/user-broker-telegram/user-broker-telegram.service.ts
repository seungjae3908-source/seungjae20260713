...
function executionTypeForState(state: TradingOrder['state']): UserExecutionEventType | null {
...
export function executionEventFromTradingOrder(
  ...
  options: {
    ...
    executionMethod?: UserExecutionMethod;
  } = {},
): UserExecutionEvent | null {
  if (transition.userId !== order.userId || order.userId !== plan.userId || order.planId !== plan.id) {
    throw new Error('EXECUTION_OWNER_MISMATCH');
  }
  const type = executionTypeForState(order.state);
  if (!type) return null;
  return {
    id: randomUUID(),
    sourceEventId: transition.id,
    userId: order.userId,
    brokerConnectionRef: order.exchange,
    orderPlanId: order.planId,
    executionId: order.id,
    type,
    source: plan.accountMode === 'live' ? 'BROKER_EXECUTION' : 'PAPER_EXECUTION',
    executionMethod: options.executionMethod ?? 'USER_APPROVED',
    symbol: plan.symbol,
    market: plan.market,
    side: plan.side,
    ...
    maskedAccount: maskBrokerAccount(options.accountNumber),
    strategy: plan.strategyId,
    remainingQuantity: safeNumber(order.remainingQuantity),
    realizedPnl: null,
...
export function manualPortfolioEvent(input: {
  ...
  return {
    ...
    realizedPnl: null,
    averageEntryPrice: safeNumber(input.price),
    averageExitPrice: null,
    occurredAt: input.occurredAt ?? new Date().toISOString(),
    metadata: { registrationMethod: 'MANUAL' },
  };
}
...
export function renderUserExecutionTelegramMessage(event: UserExecutionEvent): string {
  ...
  if (event.quantity != null || event.price != null) {
    lines.push(`${formatNumber(event.quantity)} × ${formatNumber(event.price)}`);
  }
  ...
  if (event.executionMethod) {
    lines.push('', `실행방식: ${event.executionMethod === 'AUTO_POLICY' ? '자동매매 정책' : '사용자 승인'}`);
  }
  return lines.join('\n');
}
...
export class UserBrokerTelegramService {
  ...
  async createTelegramLink(userId: string, now = new Date()) {
    ...
    return {
      deepLink: username ? `https://t.me/${username}?start=${encodeURIComponent(token)}` : null,
      tokenForBotStart: username ? null : token,
      expiresAt,
    };
  }

  async bindTelegramStart(input: {
    token: string;
    ...
    await this.repository.bindTelegramConnection({
      userId,
  ...
  async revokeTelegram(userId: string, now = new Date()) {
    await this.repository.revokeTelegramConnection(userId, now.toISOString());
  }
  ...
  async savePreferences(userId: string, patch: Partial<NotificationPreferences>, now = new Date()) {
    ...
    await this.repository.savePreferences(userId, next, now.toISOString());
    return next;
  }
  ...
  async recordEvent(event: UserExecutionEvent, now = new Date()) {
    ...
    await this.portfolioSink.accept(event);
  ...
  async processDelivery(userId: string, deliveryId: string, now = new Date()) {
    const timestamp = now.toISOString();
    const claimed = await this.repository.claimDelivery(userId, deliveryId, timestamp);
    ...
    if (!event || !connection || connection.status !== 'ACTIVE') {
      const result = await this.repository.finishDelivery(
        userId, deliveryId, 'DEAD_LETTER', claimed.attempts, null,
    ...
    try {
      transportResult = await this.transport.send(connection.telegramChatId, renderUserExecutionTelegramMessage(event));
    } catch {
      transportResult = { ok: false, errorCode: 'TELEGRAM_TRANSPORT_ERROR' };
    }
    ...
    await this.repository.finishDelivery(
      ...
      transportResult.errorCode?.slice(0, 120) || 'TELEGRAM_DELIVERY_FAILED',
      timestamp,
    );
