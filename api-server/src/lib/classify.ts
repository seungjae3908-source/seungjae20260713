...
function isMarketCapUnderOneBillionKrw(
  marketCap: number | null,
  currency?: string | null,
) {
  if (marketCap == null || !Number.isFinite(marketCap) || marketCap <= 0) {
  ...
  if (currency === 'USD') {
    return marketCap < 770_000;
  }

  return marketCap < 1_000_000_000;
}

function detectDelistingWarning(
  text: string,
  marketCap: number | null,
  currency?: string | null,
) {
  ...
  return includesAny(text, strongWords) || isMarketCapUnderOneBillionKrw(marketCap, currency);
}
...
function buildRiskCaption(
  ...
  if (delistingWarning) {
    if (isMarketCapUnderOneBillionKrw(marketCap, currency)) {
      return '초소형·상장폐지 경고';
    }
...
export function classifyStock(input: StockClassifierInput): StockClassification {
  ...
  const protectedLargeCap =
    LARGE_US_TICKERS.has(ticker) ||
    LARGE_KR_TICKERS.has(ticker) ||
    marketCapGrade === '초대형' ||
    marketCapGrade === '대형';

  const delistingWarning = detectDelistingWarning(text, marketCap, input.currency);
  ...
  if (delistingWarning) {
    score -= protectedLargeCap ? 18 : 45;
    reasons.push('명확한 상장폐지 경고 조건이 확인되어 리스크를 반영했습니다.');
  }

  if (equity != null && equity <= 0) {
    score -= protectedLargeCap ? 10 : 28;
    reasons.push('자본잠식 또는 음의 자본 가능성이 있습니다.');
  }

  if (debtRatio != null && debtRatio > 250) {
    score -= protectedLargeCap ? 8 : 18;
  ...
  if (operatingIncome != null && operatingIncome > 0) {
    ...
    score -= protectedLargeCap ? 4 : 10;
    reasons.push('영업이익이 적자입니다.');
  }
  ...
  if (dilutionScore > 0) {
    score -= protectedLargeCap
      ? Math.min(8, dilutionScore * 3)
      : Math.min(25, 8 + dilutionScore * 5);
  ...
  if (otherRiskScore > 0) {
    score -= protectedLargeCap
      ...
      : Math.min(18, 8 + otherRiskScore * 5);
