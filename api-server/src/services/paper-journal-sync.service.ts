...
function scanForbidden(value: unknown, path = 'payload'): string | null {
  ...
  for (const [key, item] of Object.entries(value)) {
    ...
    if (userIdentityKey.test(key)) return `${path}.${key}`;
...
function parseTimestamp(value: unknown, code: string) {
  if (typeof value !== 'string' || !Number.isFinite(Date.parse(value))) {
    throw new PaperJournalError(code, '동기화 시각 형식을 확인하세요.');
  }
...
function validateRecord(value: unknown): PaperJournalSyncRecord {
  ...
  if (!PAPER_JOURNAL_RECORD_KINDS.includes(value.kind as PaperJournalRecordKind)) {
    throw new PaperJournalError('INVALID_RECORD_KIND', '지원하지 않는 동기화 레코드 종류입니다.');
  }
  if (typeof value.id !== 'string' || !idPattern.test(value.id)) {
    throw new PaperJournalError('INVALID_RECORD_ID', '동기화 레코드 ID를 확인하세요.');
  }
  if (!Number.isSafeInteger(value.version) || Number(value.version) < 1) {
    throw new PaperJournalError('INVALID_RECORD_VERSION', '동기화 version은 1 이상의 정수여야 합니다.');
  }
  ...
  if (forbidden) {
    const code = userIdentityKey.test(forbidden.split('.').at(-1) ?? '') ? 'CLIENT_USER_ID_FORBIDDEN' : 'SECRET_FIELD_FORBIDDEN';
    throw new PaperJournalError(code, '사용자 식별자나 Secret 유사 필드는 동기화 payload에 포함할 수 없습니다.');
  }
  return {
    kind: value.kind as PaperJournalRecordKind,
    id: value.id,
...
export function validatePaperJournalSyncRequest(value: unknown): PaperJournalSyncRequest {
  ...
  if ('user_id' in value || 'userId' in value) {
    throw new PaperJournalError('CLIENT_USER_ID_FORBIDDEN', '사용자 ID는 로그인 세션에서만 결정됩니다.');
  }
  if (typeof value.idempotencyKey !== 'string' || !idempotencyPattern.test(value.idempotencyKey)) {
    throw new PaperJournalError('INVALID_IDEMPOTENCY_KEY', '동기화 idempotency key를 확인하세요.');
  }
  ...
  if (value.records.length > MAX_SYNC_RECORDS) {
    throw new PaperJournalError('TOO_MANY_SYNC_RECORDS', `한 번에 최대 ${MAX_SYNC_RECORDS}개 레코드만 동기화할 수 있습니다.`, 413);
  }
...
function sameRecord(server: StoredPaperJournalRecord, device: PaperJournalSyncRecord) {
  return server.version === device.version
    && server.deletedAt === device.deletedAt
    && canonical(server.payload) === canonical(device.payload);
}
...
function conflictId(record: PaperJournalSyncRecord) {
  const digest = createHash('sha256')
    .update(`${record.kind}:${record.id}:${record.version}:${canonical(record.payload)}:${record.deletedAt ?? ''}`)
    .digest('hex')
    .slice(0, 24);
...
function safeFailure(record: PaperJournalSyncRecord, cause: unknown) {
  if (cause instanceof PaperJournalError) {
    return { kind: record.kind, id: record.id, code: cause.code, message: cause.message };
  }
  return { kind: record.kind, id: record.id, code: 'SYNC_ITEM_FAILED', message: '이 레코드를 동기화하지 못했습니다.' };
}

export async function syncPaperJournal(
  ...
  for (const record of request.records) {
    try {
      ...
      if (record.version < server.version) {
        downloaded.push(server);
        continue;
      }
      if (sameRecord(server, record)) {
        unchanged.push({ kind: record.kind, id: record.id, version: record.version });
        continue;
      }
      const conflict: PaperJournalConflict = {
        id: conflictId(record),
        kind: record.kind,
        recordId: record.id,
        version: record.version,
        serverRecord: server,
        deviceRecord: record,
        differenceSummary: differenceSummary(server, record),
      ...
      failed.push(safeFailure(record, cause));
    }
  }

  const result: PaperJournalSyncResult = {
    ok: true,
    mode: JOURNAL_SYNC_MODE,
...
function decodeCursor(value: unknown) {
  ...
  try {
    ...
    throw new PaperJournalError('INVALID_CURSOR', 'snapshot cursor를 확인하세요.');
  }
}

export async function getPaperJournalSnapshot(
  repository: PaperJournalRepository,
  userId: string,
  cursorValue: unknown,
  limitValue: unknown,
  now = new Date(),
): Promise<PaperJournalSnapshotResult> {
  ...
  const records = (await repository.listSnapshot(userId))
    .sort((left, right) => left.serverUpdatedAt.localeCompare(right.serverUpdatedAt) || left.kind.localeCompare(right.kind) || left.id.localeCompare(right.id));
  ...
  return {
    ...
    exchangeRequestSent: false,
    records: page,
    nextCursor: cursor + page.length < records.length ? encodeCursor(cursor + page.length) : null,
...
export async function resolvePaperJournalConflict(
  repository: PaperJournalRepository,
  userId: string,
  conflictIdValue: unknown,
  choiceValue: unknown,
  now = new Date(),
): Promise<ConflictResolutionResult> {
  if (typeof conflictIdValue !== 'string' || !idPattern.test(conflictIdValue)) {
    throw new PaperJournalError('INVALID_CONFLICT_ID', '충돌 ID를 확인하세요.');
  }
  if (!['server', 'device', 'preserve_both'].includes(String(choiceValue))) {
    throw new PaperJournalError('INVALID_CONFLICT_CHOICE', '충돌 해결 방법을 확인하세요.');
  }
  ...
  if (choice === 'server') {
    ...
    records.push(await repository.upsertRecord(
      userId,
      cloneWithVersion(conflict.deviceRecord, Math.max(conflict.serverRecord.version, conflict.deviceRecord.version) + 1, serverTime),
      serverTime,
    ));
  } else {
    records.push(conflict.serverRecord);
...
export async function deleteAllPaperJournalData(
  repository: PaperJournalRepository,
  userId: string,
  confirmation: unknown,
) {
  if (confirmation !== DELETE_ALL_CONFIRMATION) {
    throw new PaperJournalError('DELETE_CONFIRMATION_REQUIRED', `확인 문자열 ${DELETE_ALL_CONFIRMATION}을 입력해야 합니다.`);
  }
  return {
    ok: true as const,
    mode: JOURNAL_SYNC_MODE,
    ...
    deleted: await repository.deleteAll(userId),
  };
}
