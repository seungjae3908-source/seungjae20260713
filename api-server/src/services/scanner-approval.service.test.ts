...
test('existing trade automation keeps idempotency and explicit approval for scanner paper plans', async () => {
  ...
  const scannerPolicy = { maximumOrderKrw: 100_000, accountValueKrw: 1_000_000 };
  const signal = createScannerSignal(candidate(), scannerPolicy);
  const input = scannerSignalToPaperPlan(signal, scannerPolicy, candidate().marketSnapshot);
  const policy = normalizeTradingPolicy(DEFAULT_TRADING_POLICY);
