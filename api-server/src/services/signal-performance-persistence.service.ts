...
type PaperLoopIdentity = Readonly<{
  strategyId: string;
  strategyVersion: string;
  parameterHash: string;
  researchCodeSha: string;
...
type PaperLoopSignalInput = Readonly<{
  cycle: Readonly<{ evaluatedAtMs: number }>;
  identity: PaperLoopIdentity;
  candidate: Readonly<{
    signal: Readonly<{ signalId: string; learningSnapshot: SignalSnapshot }>;
    profitEvidence: Readonly<Record<string, unknown>>;
    riskEvidence: Readonly<Record<string, unknown>>;
    execution: Readonly<{ dataEvidence?: Readonly<{ provider?: unknown; provenance?: unknown }> }>;
  }>;
  sample: Readonly<{ status: string; identity: Readonly<{ signalId: string }> }>;
}>;

type PaperLoopOutcomeInput = Readonly<{
  cycle: Readonly<{ evaluatedAtMs: number }>;
  identity: PaperLoopIdentity;
  position: Readonly<{ signalId: string }>;
  settlement: Readonly<{
    settlementId: string;
    outcome: SignalOutcomeStatus;
    entryEvaluatedAtMs: number;
    settledAtMs: number;
    holdingMs: number;
    strategyId: string;
    strategyVersion: string;
...
function assertPaperIdentity(event: PersistentSignalEvent, identity: PaperLoopIdentity, settlement?: PaperLoopOutcomeInput['settlement']) {
  const expected = event.identity;
  if (expected.strategyFamily !== identity.strategyId || expected.strategyVersion !== identity.strategyVersion
    || expected.parameterHash !== identity.parameterHash || expected.researchCodeSha !== identity.researchCodeSha.toLowerCase()
    || expected.costPolicyVersion !== identity.costPolicyVersion || expected.executionPolicyVersion !== identity.executionPolicyVersion) {
    throw new Error('PAPER_LEARNING_IDENTITY_MISMATCH');
  }
  if (settlement && (settlement.strategyId !== identity.strategyId || settlement.strategyVersion !== identity.strategyVersion
    || settlement.parameterHash !== identity.parameterHash || settlement.researchCodeSha.toLowerCase() !== identity.researchCodeSha.toLowerCase()
    || settlement.costPolicyVersion !== identity.costPolicyVersion)) {
    throw new Error('PAPER_LEARNING_SETTLEMENT_IDENTITY_MISMATCH');
  }
}

/**
 * Canonical adapter mounted by recurring-paper-loop-v1. Persistence runs before
 * ledger mutation, so a temporary failure aborts the cycle and is safely retryable.
 */
export function createPaperLoopSignalLearningAdapter(repository: SignalPerformanceRepository, ownerId: string) {
  if (!nonEmpty(ownerId)) throw new Error('SIGNAL_PERFORMANCE_OWNER_REQUIRED');
  return Object.freeze({
    async persistSignal(input: PaperLoopSignalInput) {
      if (input.sample.status !== 'OPEN' || input.sample.identity.signalId !== input.candidate.signal.signalId) {
        throw new Error('PAPER_LEARNING_OPEN_SIGNAL_REQUIRED');
      }
      const signal = input.candidate.signal.learningSnapshot;
      if (!signal || signal.signalId !== input.candidate.signal.signalId) throw new Error('PAPER_LEARNING_SNAPSHOT_REQUIRED');
      const identity: PersistentStrategyIdentity = {
        strategyFamily: input.identity.strategyId,
        strategyVersion: input.identity.strategyVersion,
        parameterHash: input.identity.parameterHash,
        researchCodeSha: input.identity.researchCodeSha,
        costPolicyVersion: input.identity.costPolicyVersion,
        executionPolicyVersion: input.identity.executionPolicyVersion,
      };
      const provider = input.candidate.execution.dataEvidence?.provider;
      const provenance = input.candidate.execution.dataEvidence?.provenance;
      const event = buildPersistentSignalEvent({
        ownerId,
        signalId: signal.signalId,
        snapshot: signal,
        identity,
        featureCutoffTimestamp: signal.dataTimestamp,
        profitFirstEvidence: input.candidate.profitEvidence,
        dataHealth: input.candidate.riskEvidence,
        providerProvenance: [provider, provenance, ...signal.dataProvenance].filter(nonEmpty),
        createdAt: new Date(input.cycle.evaluatedAtMs).toISOString(),
      });
      return persistSignalEvent(repository, event);
    },
    async persistOutcome(input: PaperLoopOutcomeInput) {
      const event = await repository.getSignal(ownerId, input.position.signalId);
      if (!event) throw new Error('SIGNAL_OUTCOME_WITHOUT_EVENT');
      assertPaperIdentity(event, input.identity, input.settlement);
      if (input.settlement.settledAtMs !== input.cycle.evaluatedAtMs) throw new Error('PAPER_LEARNING_RESOLUTION_TIME_MISMATCH');
      const outcome = buildPersistentSignalOutcome({
        ownerId,
        outcomeId: input.settlement.settlementId,
        signalId: input.position.signalId,
        source: 'PAPER',
        outcome: input.settlement.outcome,
        entryTime: new Date(input.settlement.entryEvaluatedAtMs).toISOString(),
        exitTime: new Date(input.settlement.settledAtMs).toISOString(),
        resolvedAt: new Date(input.cycle.evaluatedAtMs).toISOString(),
        resolutionSource: 'RECURRING_PAPER_LOOP_V1',
        mfePercent: input.settlement.mfePercent ?? null,
        maePercent: input.settlement.maePercent ?? null,
        targetBeforeStop: input.settlement.targetBeforeStop ?? null,
        grossPnl: input.settlement.grossPnl ?? null,
        totalCost: input.settlement.totalExplicitCost ?? null,
        netPnl: input.settlement.netPnl ?? null,
        grossReturnPercent: input.settlement.grossReturnPercent ?? null,
        netReturnPercent: input.settlement.netReturnPercent ?? null,
        holdingDurationMs: input.settlement.holdingMs,
      }, event);
      return persistSignalOutcome(repository, outcome);
    },
  });
}
