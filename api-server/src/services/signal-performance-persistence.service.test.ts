...
import {
  ...
  createPaperLoopSignalLearningAdapter, PaperJournalSignalPerformanceRepository, persistSignalEvent, persistSignalOutcome, profitFirstPerformanceEvidence,
...
describe('persistent signal performance learning', () => {
  ...
  it('connects canonical Paper producer events exactly once and fail-closes identity drift', async () => {
    const repo = new MemoryRepository();
    const adapter = createPaperLoopSignalLearningAdapter(repo, 'user-a');
    const loopIdentity = { strategyId: 'family', strategyVersion: 'v1', parameterHash: 'params-a', researchCodeSha: 'a'.repeat(40), costPolicyVersion: 'cost-v1', executionPolicyVersion: 'execution-v1' };
    const signalInput: any = {
      cycle: { evaluatedAtMs: Date.parse('2026-08-15T00:00:01.000Z') },
      identity: loopIdentity,
      candidate: { signal: { signalId: 'sig-1', learningSnapshot: snapshot }, profitEvidence: { status: 'READY' }, riskEvidence: { status: 'APPROVED' }, execution: { dataEvidence: { provider: 'BITGET_PUBLIC', provenance: 'public' } } },
      sample: { status: 'OPEN', identity: { signalId: 'sig-1' } },
    };
    assert.equal((await adapter.persistSignal(signalInput)).inserted, true);
    assert.equal((await adapter.persistSignal(signalInput)).inserted, false);
    const settlementInput: any = {
      cycle: { evaluatedAtMs: Date.parse('2026-08-15T01:01:00.000Z') },
      identity: loopIdentity,
      position: { signalId: 'sig-1' },
      settlement: {
        settlementId: 'paper-settlement-1', outcome: 'LOSS',
        entryEvaluatedAtMs: Date.parse('2026-08-15T00:01:00.000Z'), settledAtMs: Date.parse('2026-08-15T01:01:00.000Z'), holdingMs: 3_600_000,
        strategyId: 'family', strategyVersion: 'v1', parameterHash: 'params-a', researchCodeSha: 'a'.repeat(40), costPolicyVersion: 'cost-v1',
        mfePercent: 2, maePercent: -3, grossPnl: 2, totalExplicitCost: 3, netPnl: -1, grossReturnPercent: 2, netReturnPercent: -1,
      },
    };
    assert.equal((await adapter.persistOutcome(settlementInput)).inserted, true);
    assert.equal((await adapter.persistOutcome(settlementInput)).inserted, false);
    const paper = await buildSignalPerformanceReadModel(repo, 'user-a', { source: 'PAPER', minimumSampleSize: 1 });
    assert.equal(paper.sampleSize, 1); assert.equal(paper.netProfitableRate, 0); assert.equal(paper.totalCosts, 3);
    await assert.rejects(() => adapter.persistOutcome({ ...settlementInput, identity: { ...loopIdentity, parameterHash: 'drifted' } }), /IDENTITY_MISMATCH/);
  });
});
