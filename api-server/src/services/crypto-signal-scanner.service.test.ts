...
import './crypto-futures-directional-scanner.service.test';
import {
...
function fallingCandles(count = 40, current = Date.now()): CryptoCandle[] {
  return Array.from({ length: count }, (_, index) => {
...
test('futures directional pair exposes LONG/SHORT separately and returns explicit NO_TRADE or direction decision', () => {
  ...
  const staleTime = Date.now() - 24 * 60 * 60_000;
  const stale = evaluateFuturesDirectionalPair({
    ...
    candles: candles().map((row) => ({ ...row, time: row.time - 24 * 60 * 60_000 })),
    now: Date.now(),
  });
