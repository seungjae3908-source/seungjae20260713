...
export function validateTradingCostPolicy(policy: TradingCostPolicy): TradingCostPolicy {
  ...
  for (const value of [policy.commissionPercent, policy.taxPercent, policy.spreadPercent, policy.slippagePercent]) {
    if (!validNonNegative(value)) throw new Error('Trading cost components must be finite and non-negative');
  }
...
export function totalTradingCostPercent(policy: TradingCostPolicy): number {
  ...
  return round(policy.commissionPercent + policy.taxPercent + policy.spreadPercent + policy.slippagePercent);
}
...
function emptyEvidence(input: {
  ...
  return Object.freeze({
    ...
    tradingCostPercent: input.tradingCostPercent ?? null,
...
export function calculateProfitEvidence(input: {
  ...
  const expectedValue = winProbability * averageGrossWin - lossOutcomeProbability * averageGrossLoss - tradingCostPercent;
...
export function evaluateNoTradeGate(input: {
  ...
  if (input.evidence.status === 'READY') {
    ...
    if ((input.evidence.expectedValue ?? Number.NEGATIVE_INFINITY) <= 0) reasons.push('EV_NON_POSITIVE');
...
export function rankProfitCandidates(candidates: readonly ProfitRankCandidate[]): readonly ProfitRankCandidate[] {
  return Object.freeze([...candidates]
    ...
    .sort((left, right) => {
      ...
      return (b.expectedNetReturn ?? Number.NEGATIVE_INFINITY) - (a.expectedNetReturn ?? Number.NEGATIVE_INFINITY)
