...
export type ProviderPredeployEvidence = {
  ...
  authenticatedUserScopeVerified: boolean;
  ...
  privateCacheScopeVerified: boolean;
  serverCredentialFallbackDetected: boolean;
  credentialPlaintextReturned: boolean;
...
export function evaluateProviderPredeployReadiness(input: ProviderPredeployEvidence): ProviderPredeployResult {
  ...
  if (!input.authenticatedUserScopeVerified) blockers.push('AUTHENTICATED_USER_SCOPE_NOT_VERIFIED');
  if (!input.accountIdentityAvailable) blockers.push('ACCOUNT_IDENTITY_NOT_AVAILABLE');
  if (!input.accountBindingVerified) blockers.push('USER_ACCOUNT_BINDING_NOT_VERIFIED');
  ...
  if (!input.privateCacheScopeVerified) blockers.push('PRIVATE_CACHE_SCOPE_NOT_VERIFIED');
  if (input.serverCredentialFallbackDetected) blockers.push('SERVER_PROVIDER_CREDENTIAL_FALLBACK_FORBIDDEN');
  if (input.credentialPlaintextReturned) blockers.push('CREDENTIAL_PLAINTEXT_RETURN_FORBIDDEN');
