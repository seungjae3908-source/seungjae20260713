...
const forbiddenMetricPattern = /(?:\bPF\b|profit\s*factor|\bEV\b|expectancy|\bMDD\b|\bMAE\b|\bMFE\b|Sharpe|\bDSR\b|\bPBO\b|net\s*alpha|full\s*cost|position\s*size|leverage|champion|promotion)/i;
...
function assertExactKeys(row: Record<string, unknown>, allowed: Set<string>, label: string): void {
  for (const key of Object.keys(row)) {
    if (!allowed.has(key)) throw new ResearchDualFreeAiError('INVALID_AI_OUTPUT', `${label} contains unsupported key ${key}`);
  }
}
