...
function bitgetHeaders(requestPath: string, query = ''): Record<string, string> {
  ...
  const signature = createHmac('sha256', secret)
    .update(`${timestamp}GET${requestPath}${queryPart}`)
    .digest('base64');
  return {
    'ACCESS-KEY': apiKey,
...
async function readUpbit() {
  ...
  if (!configured) {
    return { provider: 'upbit', configured: false, connected: false, readOnly: true, error: 'UPBIT_NOT_CONFIGURED' };
  }
  try {
    const rows = await fetchJson<JsonRecord[]>(`${UPBIT_BASE}/v1/accounts`, {
      Authorization: upbitAuthorization(),
    });
...
async function readBitget() {
  ...
  if (!configured) {
    return { provider: 'bitget', configured: false, connected: false, readOnly: true, error: 'BITGET_NOT_CONFIGURED' };
  }
  ...
  return {
    ...
    accounts: accountRows.map((row) => ({
      marginCoin: stringValue(row.marginCoin),
    ...
    positions: positionRows.map((row) => ({
      ...
      marginCoin: stringValue(row.marginCoin),
      side: stringValue(row.holdSide),
      total: finite(row.total),
      available: finite(row.available),
      leverage: finite(row.leverage),
      averageOpenPrice: finite(row.openPriceAvg),
...
router.get('/status', (_req, res) => {
  ...
  return res.json({
    ...
    providers: {
      kiwoom: { configured: isKiwoomConfigured(), ...getKiwoomStatus() },
      upbit: { configured: Boolean(process.env.UPBIT_ACCESS_KEY?.trim() && process.env.UPBIT_SECRET_KEY?.trim()) },
      bitget: {
        configured: Boolean(
          process.env.BITGET_API_KEY?.trim()
            && process.env.BITGET_SECRET_KEY?.trim()
