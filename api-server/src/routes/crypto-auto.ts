...
router.get('/crypto/futures/auto/status', requireMember, async (req, res) => {
  await ensureJournalLoaded();
...
router.get('/crypto/futures/auto/journal', requireMember, async (req, res) => {
  await ensureJournalLoaded();
...
router.post('/crypto/futures/auto/plan', requireMember, async (req, res) => {
...
router.post('/crypto/futures/auto/execute', requireMember, async (req, res) => {
  if (!requireSafety(req, res)) return;
  if (!tradingEnabled()) {
    return res.status(409).json({
      ok: false,
...
router.post('/crypto/futures/auto/close-plan', requireMember, async (req, res) => {
  if (!requireSafety(req, res)) return;
  try {
    const symbol = safeSymbol(req.body?.symbol);
    const positionMode = String(req.body?.positionMode ?? 'one_way_mode') as PositionMode;
...
router.post('/crypto/futures/auto/close', requireMember, async (req, res) => {
  if (!requireSafety(req, res)) return;
  if (!tradingEnabled()) {
...
router.post('/crypto/futures/auto/configure-plan', requireMember, async (req, res) => {
  if (!requireSafety(req, res)) return;
  try {
    const symbol = safeSymbol(req.body?.symbol);
    const positionMode = String(req.body?.positionMode ?? 'one_way_mode') as PositionMode;
...
router.post('/crypto/futures/auto/configure', requireMember, async (req, res) => {
  if (!requireSafety(req, res)) return;
