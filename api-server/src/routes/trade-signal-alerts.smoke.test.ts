...
async function startServer(repository: InMemoryTradingRepository, authenticated = true) {
  ...
  if (authenticated) app.use((req, _res, next) => {
    req.member = {
      id: USER, login_name: 'test', display_name: 'test', role: 'regular',
...
function input(signalId: string) {
  const now = new Date().toISOString();
  return {
    exchange: 'kiwoom', accountMode: 'paper', strategyId: 'scanner-1d', signalId,
    ...
    targetPrices: [74_500], splitRatios: [100], signalReasons: ['trend'], signalWarnings: [],
    signalScore: 84, signalConfidence: 79, minimumSignalScore: 70,
    ...
    marketSnapshot: {
      observedAt: now, dataDelayMs: 0, oneMinuteMovePercent: 0, spreadPercent: 0.01,
      orderbookGapPercent: 0.01, halted: false, availableBalance: 1_000_000,
...
test('approval alerts are member scoped, deterministic, and expose no credentials', async () => {
  ...
  await service.revalidatePlan(USER, created.plan.id, {
    score: 84, confidence: 79, coreConditionsMaintained: true, riskReward: 1.8,
    reasons: ['trend'], dataTimestamp: new Date().toISOString(),
