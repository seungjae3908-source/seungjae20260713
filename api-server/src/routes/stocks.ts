...
function legacyRealOrderDisabled(req: AuthenticatedRequest) {
	void req;
	return {
		ok: false as const,
		status: 409,
...
// Legacy stock auto-trade endpoints are compatibility-only. They may expose
// historical journal/monitor reads, but they can never create a provider mutation.
router.use("/auto-trade", requireCapability("canAccessAutoTrading"));

function rejectLegacyRealOrder(req: AuthenticatedRequest, res: any) {
	const access = legacyRealOrderDisabled(req);
...
router.post("/auto-trade/plan", rejectLegacyRealOrder);
router.post("/auto-trade/close-plan", rejectLegacyRealOrder);
router.post("/auto-trade/close-execute", rejectLegacyRealOrder);
router.post("/auto-trade/execute", rejectLegacyRealOrder);
