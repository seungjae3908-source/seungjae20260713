...
test('canonical exit plan fails closed when the position changes after preview', async () => {
  let reads = 0;
  setTradeExitPreviewReadersFactoryForTests(() => ({
    toss: async () => {
      reads += 1;
  ...
  try {
    ...
    assert.equal(reads, 2);
    assert.equal(planBody.error, 'EXIT_PLAN_PREVIEW_STALE_OR_POSITION_CHANGED');
