  ...
  return {
    ...
    selectedConditions: Array.isArray(input.selectedConditions)
      ...
      : Array.isArray(input.conditions)
        ...
        : [],
    requestedInvestmentKrw: input.requestedInvestmentKrw == null ? undefined : Number(input.requestedInvestmentKrw),
...
async function approveScannerPaperPlan(req: AuthenticatedRequest, res: Response) {
  ...
  if (created.duplicate) {
    return res.json({
      ...
      order: created.order,
      duplicate: true,
      executionClaimed: created.executionClaimed,
      paperOrderCreated: true,
      liveOrderSubmitted: false,
      exchangeRequestSent: false,
    });
  ...
  const quantity = Number(created.plan.quantity);
  const averageFillPrice = quantity > 0 ? created.plan.estimatedKrw / quantity : null;
  ...
  return res.json({
    ...
    executionClaimed: created.executionClaimed,
    paperOrderCreated: true,
    serverRevalidatedAt: created.plan.lastSignalValidatedAt,
    liveOrderSubmitted: false,
    exchangeRequestSent: false,
  });
}
