...
const logoutHelperStart = spec.indexOf('async function logout(page: Page)');
const logoutHelperEnd = spec.indexOf('\nasync function expectMembership(', logoutHelperStart);
assert(logoutHelperStart >= 0 && logoutHelperEnd > logoutHelperStart, 'logout helper boundaries are missing');
const logoutHelperBlock = spec.slice(logoutHelperStart, logoutHelperEnd);
assert(logoutHelperBlock.includes('const origin = new URL(page.url()).origin;'), 'logout must freeze the same-origin identity before observing scoped reads');
assert(logoutHelperBlock.includes('[...(pendingApiGetRequests.get(page) ?? [])]'), 'logout observation must inherit exact GET requests already pending before the click');
assert(logoutHelperBlock.includes('.filter((request) => isLogoutScopedRead(request, origin))'), 'pre-existing logout candidates must still pass the exact scoped-read identity matcher');
