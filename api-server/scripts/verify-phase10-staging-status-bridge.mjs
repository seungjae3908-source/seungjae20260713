...
assert(workflow.includes('actions.listArtifactsForRepo'), 'status must locate the exact verdict artifact');
assert(workflow.includes('actions.getWorkflowRun'), 'status must directly verify the artifact source run');
assert(workflow.includes("run.path === '.github/workflows/staging-readiness.yml'"), 'only official Staging Readiness runs may match');
assert(workflow.includes('actions/download-artifact@v4'), 'status must download the exact verdict');
assert(workflow.includes('staging-verdict.json'), 'status must report verdict contents');
assert(workflow.includes('RELEASE_READY'), 'status must report release readiness');
assert(workflow.includes('Skipped:'), 'status must report skipped checks');
assert(workflow.includes('Console errors:'), 'status must report console errors');
assert(workflow.includes('Unexpected HTTP errors:'), 'status must report unexpected HTTP errors');
assert(workflow.includes('actions: read'), 'status job must use read-only Actions permission');
assert(workflow.includes('issues: write'), 'status job needs issue-comment write permission');
...
console.log('[phase10-staging-status-bridge] owner-only exact-SHA verdict lookup is artifact-backed and read-only; staging and production cannot be modified');
