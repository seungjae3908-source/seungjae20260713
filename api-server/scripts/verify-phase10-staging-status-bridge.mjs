...
assert(workflow.includes('git rev-parse origin/main^{commit}'), 'status query must resolve current main');
assert(workflow.includes('[[ "$TARGET_SHA" == "$MAIN_SHA" ]]'), 'status query must require the exact current main SHA');
...
assert(workflow.includes('run.head_sha === targetSha'), 'matching run must use the requested current main SHA');
...
assert(documentation.includes('/status-staging <exact-40-character-current-main-sha>'), 'documentation must show the exact command');
...
console.log('[phase10-staging-status-bridge] owner-only current-main status lookup is read-only; staging and production cannot be modified');
