...
values
  ('$USER_ID', '$PLAN_1', 'recovery-lease-plan-1', 'SUBMITTED',
    jsonb_build_object('id', '$PLAN_1', 'userId', '$USER_ID', 'state', 'SUBMITTED', 'version', 0), 0,
    '2026-08-05T05:00:00.000Z'),
  ('$USER_ID', '$PLAN_2', 'recovery-lease-plan-2', 'SUBMITTED',
    jsonb_build_object('id', '$PLAN_2', 'userId', '$USER_ID', 'state', 'SUBMITTED', 'version', 0), 0,
    '2026-08-05T05:00:00.000Z');
...
values
  ('$USER_ID', '$ORDER_1', '$PLAN_1', 'upbit', 'recovery-client-1', 'ACCEPTED',
    jsonb_build_object(
      ...
      'createdAt', '2026-08-05T05:00:00.000Z', 'updatedAt', '2026-08-05T05:00:00.000Z'
    ), 0, 1, '[]'::jsonb, false, '2026-08-05T05:00:00.000Z'),
...
wrong_prepare="$("${PSQL[@]}" <<SQL
set role service_role;
select case when public.transition_trade_recovery_order_atomic(
  '$WRONG_WORKER', '$USER_ID', '$ORDER_1', 'ACCEPTED', 0, 'RECOVERY_REQUIRED',
  (select payload || jsonb_build_object('state', 'RECOVERY_REQUIRED', 'updatedAt', clock_timestamp()::text)
   from public.trade_orders where id = '$ORDER_1'),
  jsonb_build_object(
    'id', '83000000-0000-0000-0000-000000000001', 'userId', '$USER_ID', 'orderId', '$ORDER_1',
    'fromState', 'ACCEPTED', 'toState', 'RECOVERY_REQUIRED', 'reason', 'WRONG_WORKER_PREPARE',
    'metadata', jsonb_build_object('orderSubmissionAttempted', false), 'createdAt', clock_timestamp()::text
  ), false
...
correct_prepare="$("${PSQL[@]}" <<SQL
set role service_role;
select case when public.transition_trade_recovery_order_atomic(
  '$claimed_owner', '$USER_ID', '$ORDER_1', 'ACCEPTED', 0, 'RECOVERY_REQUIRED',
  (select payload || jsonb_build_object('state', 'RECOVERY_REQUIRED', 'updatedAt', clock_timestamp()::text)
   from public.trade_orders where id = '$ORDER_1'),
  jsonb_build_object(
    'id', '83000000-0000-0000-0000-000000000002', 'userId', '$USER_ID', 'orderId', '$ORDER_1',
    'fromState', 'ACCEPTED', 'toState', 'RECOVERY_REQUIRED', 'reason', 'LEASE_OWNER_PREPARE',
    'metadata', jsonb_build_object('orderSubmissionAttempted', false), 'createdAt', clock_timestamp()::text
  ), false
) is null then 0 else 1 end;
SQL
...
wrong_final="$("${PSQL[@]}" <<SQL
set role service_role;
select case when public.transition_trade_recovery_order_atomic(
  '$WRONG_WORKER', '$USER_ID', '$ORDER_1', 'RECOVERY_REQUIRED', 1, 'CANCELED',
  (select payload || jsonb_build_object('state', 'CANCELED', 'updatedAt', clock_timestamp()::text)
   from public.trade_orders where id = '$ORDER_1'),
  jsonb_build_object(
    'id', '83000000-0000-0000-0000-000000000003', 'userId', '$USER_ID', 'orderId', '$ORDER_1',
    'fromState', 'RECOVERY_REQUIRED', 'toState', 'CANCELED', 'reason', 'WRONG_WORKER_FINAL',
    'metadata', jsonb_build_object('orderSubmissionAttempted', false), 'createdAt', clock_timestamp()::text
  ), true
) is null then 0 else 1 end;
SQL
...
correct_final="$("${PSQL[@]}" <<SQL
set role service_role;
select case when public.transition_trade_recovery_order_atomic(
  '$claimed_owner', '$USER_ID', '$ORDER_1', 'RECOVERY_REQUIRED', 1, 'CANCELED',
  (select payload || jsonb_build_object('state', 'CANCELED', 'updatedAt', clock_timestamp()::text)
   from public.trade_orders where id = '$ORDER_1'),
  jsonb_build_object(
    'id', '83000000-0000-0000-0000-000000000004', 'userId', '$USER_ID', 'orderId', '$ORDER_1',
    'fromState', 'RECOVERY_REQUIRED', 'toState', 'CANCELED', 'reason', 'LEASE_OWNER_FINAL',
    'metadata', jsonb_build_object('orderSubmissionAttempted', false), 'createdAt', clock_timestamp()::text
  ), true
) is null then 0 else 1 end;
SQL
