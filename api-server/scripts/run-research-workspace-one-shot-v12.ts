...
if(process.argv[1]&&import.meta.url===pathToFileURL(resolve(process.argv[1])).href){
  if(process.argv.includes('--help')){
    ...
    runResearchWorkspaceOneShotCliV12(process.argv.slice(2)).then((result:any)=>{
      process.stdout.write(JSON.stringify({status:result.status,manifestDigest:result.manifest?.manifestDigest??result.manifestDigest??null,
        packageDigest:result.reviewPackage?.packageDigest??null,providerCalls:result.providerCalls??{gemini:0,groq:0},executionAuthority:'NONE'})+'\n');
