-- Remove any legacy self-read policies and keep personal Telegram storage server-only.
begin;

do $personal_telegram_policy_cleanup$
declare
  target_table text;
  policy_name text;
begin
  ...
  loop
    if to_regclass(format('public.%I', target_table)) is null then
      raise exception 'personal Telegram storage table public.% is missing', target_table;
    end if;
...
$personal_telegram_policy_cleanup$;
