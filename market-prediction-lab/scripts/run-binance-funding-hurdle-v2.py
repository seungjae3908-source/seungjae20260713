...
OUT=Path("market-prediction-lab/artifacts/binance-funding-hurdle-v2");OUT.mkdir(parents=True,exist_ok=True)
...
def simulate(data,cost,start_fraction=0.0,mode="TOP2_POSITIVE"):
    ...
    for i in range(start,len(dates)-1):
        ...
        if (i-start)%7==0:
            ...
            elif mode=="COST_HURDLE":
                chosen=[]
                for score,s in scores:
                    projected_30d=(score*(30/7))/2
                    if score>0 and projected_30d>hurdle:
                        chosen.append(s)
                    if len(chosen)>=2:
                        break
            else:
...
def main():
    ...
    for k,v in COSTS.items():
        results[k]={}
        for mode in ("TOP2_POSITIVE","COST_HURDLE"):
            results[k][mode]={"full":simulate(data,v,0.0,mode),"recent30pct":simulate(data,v,0.70,mode)}
    out={"schemaVersion":1,"kind":"binance-funding-hurdle-v2","researchOnly":True,"publicDataOnly":True,
         "liveTrading":False,"privateApi":False,"ordersSubmitted":0,
         "formula":{"rank":"trailing realized 7d funding sum","modes":["TOP2_POSITIVE","COST_HURDLE"],"rebalanceDays":7,
                    "costHurdle":"projected 30d funding contribution on fully-funded pair > 2x modeled round-trip cost",
