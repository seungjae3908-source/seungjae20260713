...
OUT=Path("market-prediction-lab/artifacts/cross-venue-static-carry-v1")
...
def simulate_static(times,hl_px,bin_px,hl_f,bin_f,costs):
    # Literature-grounded static orientation:
    # LONG Binance perpetual / SHORT Hyperliquid perpetual.
    # Fully funded two-leg capital, daily equal-notional hedge approximation.
    eq=1.0
    curve=[1.0]
    daily=[]
    funding_sum=0.0
    ...
    open_cost=pair_event_cost(costs)
    eq*=1-open_cost
    cost_sum-=open_cost
    curve[-1]=eq
    ...
    while i+6<len(times):
        ...
        # SHORT HL earns HL funding; LONG Binance pays Binance funding.
        day_hl=window_sum(hl_f,t,t2)
        day_bin=window_sum(bin_f,t,t2)
        funding=(day_hl-day_bin)/2

        # LONG Binance / SHORT HL, equal notional on total two-leg capital.
    ...
    return summarize(
        daily,curve,days,days,2,funding_sum,basis_sum,cost_sum
    )
...
def main():
    ...
    for cname,cost in COSTS.items():
        by={coin:simulate_static(*data[coin],cost) for coin in SYMBOLS}
        results[cname]={"portfolio":portfolio(by),"symbols":by}

    payload={
        "schemaVersion":1,
        "kind":"cross-venue-static-carry-v1",
        "research_only":True,
        ...
        "formula":{
            ...
            "orientation":"STATIC LONG Binance perpetual / SHORT Hyperliquid perpetual",
            "holding":"entire available six-month sample; no funding-driven orientation switching",
    ...
    lines=[
        "# Hyperliquid ↔ Binance Static Funding Carry V1",
        "",
        "RESEARCH ONLY / PUBLIC DATA ONLY / NO ORDERS",
        "",
        "| Cost | EW total | EW ann. | Positive | Avg MDD | Avg exposure |",
        "|---|---:|---:|---:|---:|---:|",
    ]
    for cname in COSTS:
        p=results[cname]["portfolio"]
        lines.append(
            f'| {cname} | {p["equal_weight_return"]*100:.2f}% | '
            f'{p["equal_weight_annualized"]*100:.2f}% | {p["positive_symbols"]}/3 | '
            f'{p["avg_mdd"]*100:.2f}% | {p["avg_exposure"]*100:.1f}% |'
        )
    lines+=["","## Base static carry by symbol","","| Coin | Return | Ann. | MDD | PF | Exposure | Funding* | Basis* | Costs* |","|---|---:|---:|---:|---:|---:|---:|---:|---:|"]
