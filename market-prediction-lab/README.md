...
- 전체 `npm run validate` 통과 후에만 결과 기록
- 원본 데이터는 임시 아티팩트로만 보관하고 저장소에는 검증 요약과 SHA-256만 기록

검증 요약은 `docs/live-smoke-result.json`에 있습니다. 실검증용 임시 GitHub Actions 파일은 실행 직후 삭제되어 최종 변경 범위에 포함되지 않습니다.
