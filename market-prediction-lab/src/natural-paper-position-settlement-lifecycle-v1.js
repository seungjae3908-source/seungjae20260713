...
function freezeExitTrigger(position, lifecycle, observation, exit) {
  ...
  const payload = {
    ...
    triggerObservationId: observation.observationId,
    ...
    strategyIdentity: lifecycle.strategyIdentity,
    researchCodeSha: position.researchCodeSha,
    costPolicyId: position.costPolicyVersion,
    costPolicyVersion: position.costPolicyVersion,
    riskPolicyId: lifecycle.riskPolicyIdentity?.policyId ?? null,
    cycleId: handoff?.cycleIdentity?.cycleId ?? null,
    cycleIdentityDigest: observation.cycleIdentityDigest ?? handoff?.cycleIdentity?.identityDigest ?? null,
    accountIdSha256: handoff?.accountIdentity?.accountIdSha256 ?? null,
    accountIdentityDigest: observation.accountIdentityDigest ?? handoff?.accountIdentity?.identityDigest ?? null,
    entryEvidenceDigest: observation.entryEvidenceDigest ?? lifecycle.entry?.evidenceDigest ?? null,
    riskPolicyIdentityDigest: observation.riskPolicyIdentityDigest ?? handoff?.riskPolicyIdentity?.identityDigest ?? null,
    costPolicyIdentity: structuredClone(observation.costPolicyIdentity ?? handoff?.costPolicyIdentity ?? { version: position.costPolicyVersion }),
    schedulerHandoffDigest: handoff ? hash(handoff) : null,
  ...
  return Object.freeze({
    ...
    evidence: deepFreeze({
      ...
      triggerBinding: {
        ...
        accountIdSha256: trigger.accountIdSha256,
        strategyId: trigger.strategyId,
        costPolicyId: trigger.costPolicyId,
        riskPolicyId: trigger.riskPolicyId,
        cycleIdentityDigest: trigger.cycleIdentityDigest,
        accountIdentityDigest: trigger.accountIdentityDigest,
        entryEvidenceDigest: trigger.entryEvidenceDigest,
        riskPolicyIdentityDigest: trigger.riskPolicyIdentityDigest,
        schedulerHandoffDigest: trigger.schedulerHandoffDigest,
      },
