...
export function prepareMeaningfulSearchPaperCandidate({
  searchOutcome,
  candidate,
  profitGate,
  profitEvidence,
  ...
  if (profitGate?.decision !== "ELIGIBLE" || profitGate?.eligible !== true) {
    return result("NO_TRADE", gateReasons.length ? gateReasons : ["PROFIT_GATE_NOT_ELIGIBLE"]);
  }
  ...
  validateCandidate(candidate, blockers);
  if (blockers.length) return result("BLOCKED", blockers);

  const paperCandidate = freeze({
    ...candidate,
    profitGate: freeze({
      decision: "ELIGIBLE",
      eligible: true,
    ...
    profitEvidence: freeze({ ...profitEvidence, executionAuthority: "NONE" }),
  });
