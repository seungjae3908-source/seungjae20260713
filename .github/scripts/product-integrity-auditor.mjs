...
export const IDENTITY_CONTRACTS = [
  ...
  },
  {
    ...
    destinationPaths: ['api-server/src/routes/scanner-paper-plans.ts', 'api-server/src/services/product-paper-source-registry.service.ts'],
    dimensions: ['candidateId', 'strategyId', 'parameterHash', 'market', 'symbol', 'timeframe', 'side', 'leverage', 'accountMode'],
    required: [
      { id: 'scanner-paper-route-mounted', paths: ['api-server/src/routes/trade-automation.ts'], allOf: ['router.use(createScannerPaperPlansRouter())'] },
      { id: 'scanner-server-source-canonical-identity', paths: ['api-server/src/services/product-paper-source-registry.service.ts'], allOf: ['assertPaperApprovalEnvelope', "this.read('SCANNER'", 'resolveScannerCanonicalPaperIdentity(', 'SERVER_LEVERAGE_PROVENANCE_REQUIRED'] },
      { id: 'scanner-canonical-server-consumer', paths: ['api-server/src/routes/scanner-paper-plans.ts'], allOf: ["router.post('/scanner/plans'", 'registry.resolveScanner(', 'resolveCanonicalPaperAdmissionBridgeCandidate(', 'runRecurringPaperCycle('] },
    ],
    transport: {
...
const LEDGER_CONTEXT = Object.freeze({
  CG003: ['REAL_PRODUCT_GAP', 'Scanner source validation is not a canonical admission/execution consumer. The required server chain remains incomplete.', 'Connect verified canonical admission and real Paper samples/positions; do not forward to generic live-capable /plans.'],
  ...
  CG005: ['REAL_PRODUCT_GAP', 'The KR-only composer and incomplete canonical server admission/execution chain do not connect Spot to real Paper samples/positions.', 'Preserve existing admission workflows; connect an independently verified Spot Paper-only server consumer.'],
  CG006: ['REAL_PRODUCT_GAP', 'The KR-only composer and incomplete canonical server admission/execution chain do not connect Futures LONG/SHORT with server-owned leverage to real Paper samples/positions.', 'Preserve original signalDirection, explicit LONG/SHORT and canonical leverage through admission/execution; never default missing identity.'],
  CG007: ['REAL_PRODUCT_GAP', 'Server-bound Backtest source validation is not an accepted same-candidate Paper admission/execution route. An owner-delegated engine consumer alone does not complete the product chain.', 'Consume genuine owner evidence through the accepted route and preserve all eight identity fields; never credit preview or source-only validation.'],
  CG011: ['REAL_PRODUCT_GAP', 'Genuine eight-component cost issuer, position/entry/exit/settlement binding and persisted authoritative readback are not proven. An owner-delegated cost consumer is not the missing issuer.', 'Connect actual measured cost evidence through existing persistence/readback; preserve missing components as OPEN, never zero.'],
  CG012: ['REAL_PRODUCT_GAP', 'Accepted genuine Full Cost evidence-to-settlement-to-Net PnL-to-persisted-readback lineage is incomplete. Forwarding canonical netPnl in a conditional consumer does not originate that evidence.', 'Consume the same genuine cost evidence and settlement without independent calculation or default COMPLETE.'],
  CG013: ['REAL_PRODUCT_GAP', 'Genuine immutable same-candidate validation receipt issuer, persisted readback and actual admission authority are not proven. A typed receipt consumer or generic Risk Policy is not validation authority.', 'Connect the genuine identity-bound issuer/readback to the actual consumer; never promote fixture verification flags, replay or backfilled data into authority.'],
  ...
  ID003: ['REAL_PRODUCT_GAP', 'Canonical Scanner source comparison does not carry all eight identity fields through admission, execution and position/lifecycle consumers.', 'Connect explicit validated direction and server-issued leverage through real Paper consumers, without default BUY/LONG/1x.'],
});
...
// Acceptance below comes from the user's direct 2026-09-17 Paper-owner handoff,
// not historical authorship. Acceptance never closes an implementation gap.
const PAPER_OWNER_ACK = 'ACCEPTED_DRAFT_IN_PROGRESS; task 01a0a969-2ab0-7971-b52b-39934162761f, direct user handoff turn 01a0ad6c-30fc-7922-8a70-93476cb9abe2; responsibility only, not runtime/receipt/closure proof';
export const P1_SCOPE_RECONCILIATION = Object.freeze({
  ...
  CG011: { classification: 'EXISTING_OWNER', owner: '#1089 Paper-owner task Repair and activate paper risk flow; historical #1001/#891', ownerAcknowledgement: PAPER_OWNER_ACK, dependencyOwner: '#1085 consumes, not redefines, genuine measured cost issuer/persist/readback contract' },
  CG012: { classification: 'EXISTING_OWNER', owner: '#1089 Paper-owner task Repair and activate paper risk flow; historical #1001/#891', ownerAcknowledgement: PAPER_OWNER_ACK, dependencyOwner: '#1085 preserves the same genuine cost lineage; conditional Net PnL forwarding is not closure' },
  CG013: { classification: 'EXISTING_OWNER', owner: '#1089 Paper-owner task Repair and activate paper risk flow; validation namespace #547 retained', ownerAcknowledgement: PAPER_OWNER_ACK, dependencyOwner: '#1085 wiring follows genuine receipt issuer/readback; missing authority is an implementation gap, not merely runtime evidence' },
...
function ledgerEntry(id, severity, area, wiringStatus, reason, evidenceLevel, details = {}) {
  ...
  return {
    ...
    ownerAcknowledgement: scope?.ownerAcknowledgement ?? (scope?.classification === 'EXISTING_OWNER' ? 'NOT_CONFIRMED; existing domain provenance, not accepted handoff' : 'Current-chat #1085 scope authorized'),
...
export function buildErrorLedger(graph, capabilities, identities) {
  ...
  for (const edge of graph.edges) {
    ...
    entries.push(ledgerEntry(`EL-${edge.id}`, edge.severity, `${edge.from} → ${edge.to}`, edge.status, [...gaps, ...blockers].join(', ') || 'NON_PROVEN_EDGE', EVIDENCE_LEVEL.NOT_PROVEN, {
      ...
      currentProof: [
        ...[...edge.required, ...edge.blockers].map(probe => ({ id: probe.id, state: probe.state, matchedFiles: probe.matchedFiles ?? [] })),
        ...(edge.observations ?? []).map(probe => ({ id: probe.id, state: probe.state,
          matchedFiles: probe.matchedFiles ?? [], role: 'NON_CLOSURE_CONTEXT_ONLY' })),
      ],
    }));
  }
