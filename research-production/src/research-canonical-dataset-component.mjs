...
export function buildCanonicalDatasetComponentV1({
  ...
  const splitKeys=['TRAIN','VALIDATION','OOS'];
  let splitCursor=0;
  for(const key of splitKeys){
    const values=splitAssignments[key];
    if(!Array.isArray(values)||values.length===0||values.some(value=>!Number.isSafeInteger(value))){
      throw new Error('DATASET_SPLIT_CONTRACT_NOT_EXACT');
    }
    const expected=timestamps.slice(splitCursor,splitCursor+values.length);
    if(expected.length!==values.length||values.some((value,index)=>value!==expected[index])){
      throw new Error('DATASET_SPLIT_CONTRACT_NOT_EXACT');
    }
    splitCursor+=values.length;
  }
  if(splitCursor!==timestamps.length){
    throw new Error('DATASET_SPLIT_CONTRACT_NOT_EXACT');
  }
