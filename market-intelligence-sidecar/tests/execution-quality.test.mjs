...
test('book walk reports visible-depth coverage and non-negative buy slippage without inventing missing depth', () => {
  const result = walkOrderBook({ direction: 'BUY', targetQty: 3, arrivalPrice: 100, asks: [[100, 1], [101, 1], [102, 1]] }, { maxBookWalkSlippageBps: 500 });
...
test('insufficient L2 depth vetoes executable coverage rather than assuming a full fill', () => {
  const result = walkOrderBook({ direction: 'SELL', targetQty: 10, arrivalPrice: 100, bids: [[100, 2], [99, 2]] });
...
test('queue analytics require verified order-level evidence and never infer L3 position from L2', () => {
  ...
  const observed = evaluateQueueEvidence({ queueEvidenceVerified: true, queueAheadQty: 10, ownOrderQty: 2, marketableQtyAtLevel: 11, cancellationsAheadQty: 0 });
...
test('realized TCA separates delay, execution, fees and total implementation shortfall', () => {
  const result = calculateRealizedTca({ direction: 'BUY', decisionPrice: 100, arrivalPrice: 100.1, fillVwap: 100.2, feesBps: 2 }, { maxObservedImplementationShortfallBps: 100 });
...
test('observe-only execution quality never deletes scanner candidate or grants order authority', () => {
  const result = evaluateExecutionQuality({ now: NOW, bookWalk: { direction: 'BUY', targetQty: 1, arrivalPrice: 100, asks: [[100, 2]] }, fillModel: {} });
...
test('required execution-quality mode keeps missing calibrated fill evidence out of parent eligibility', () => {
  const result = evaluateExecutionQuality({ now: NOW, bookWalk: { direction: 'BUY', targetQty: 1, arrivalPrice: 100, asks: [[100, 2]] }, fillModel: {} }, { enforcement: 'REQUIRED_FOR_PARENT_GATE' });
...
test('calibrated low fill probability can veto auto-trading eligibility without changing scanner visibility', () => {
  const result = evaluateExecutionQuality({ now: NOW, bookWalk: { direction: 'BUY', targetQty: 1, arrivalPrice: 100, asks: [[100, 2]] }, fillModel: { ...goodFillModel(), fillProbability: 0.3 } }, { enforcement: 'REQUIRED_FOR_PARENT_GATE' });
