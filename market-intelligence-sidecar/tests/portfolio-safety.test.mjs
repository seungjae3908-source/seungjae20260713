...
function lossSamples() {
  return Array.from({ length: 300 }, (_, index) => 0.2 + (index % 30) * 0.1);
}
...
test('portfolio risk checks gross and single-name limits without inventing correlation', () => {
  const result = evaluatePortfolioRisk({
    ...
    proposedSymbol: 'SOLUSDT',
    proposedCluster: 'CRYPTO_BETA',
    proposedNotionalKrw: 100_000,
    correlationEvidenceVerified: false,
  });
...
test('verified correlation cluster exposure can veto concentrated correlated entries', () => {
  const result = evaluatePortfolioRisk({
    ...
    proposedCluster: 'CRYPTO_BETA',
    proposedNotionalKrw: 100_000,
    correlationEvidenceVerified: true,
  });
...
test('signal TTL and revalidation independently protect against stale entries', () => {
  const expired = evaluateSignalFreshness({
    generatedAt: NOW - 20 * 60_000,
    revalidatedAt: NOW - 10_000,
  }, {}, NOW);
  assert.equal(expired.status, 'VETO');
  assert.equal(expired.reason, 'SIGNAL_TTL_EXPIRED');

  const staleRevalidation = evaluateSignalFreshness({
    generatedAt: NOW - 60_000,
    revalidatedAt: NOW - 5 * 60_000,
  }, {}, NOW);
...
test('global kill switch blocks new entries but has no liquidation/cancel/order authority', () => {
  const result = evaluateGlobalKillSwitch({
    dailyDrawdownPct: 5,
    dataIntegrityCritical: true,
  });
