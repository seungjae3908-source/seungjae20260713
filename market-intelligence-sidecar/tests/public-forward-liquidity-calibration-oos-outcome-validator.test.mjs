...
const IMPLEMENTATION_BLOB_SHA = 'd'.repeat(40);
...
function upstreamSource({ key, collectorCodeSha, digit }) {
  const receiptDigest = hex(String(Number(digit) + 1));
  const artifactId = String(1000 + Number(digit));
  const artifactDigest = hex(String(Number(digit) + 2));
  const rawBatchDigest = hex(String(Number(digit) + 3));
  return {
    ...
    ingestReceiptChainDigest: sha256(`receipt-chain:${key}`),
    ingestReceiptCount: 1,
    ingestReceiptDigests: [receiptDigest],
    artifactIds: [artifactId],
    artifactDigests: [artifactDigest],
    rawBatchDigests: [rawBatchDigest],
...
function multiAssignment({ id, split, eventTimestampMs, digit, source }) {
  return {
    ...
    sourceReceiptChainDigest: source.ingestReceiptChainDigest,
    sourceReceiptCount: source.ingestReceiptCount,
...
function multiSourceSplitAudit(overrides = {}) {
  ...
  const body = {
    ...
    receiptChainDigests: upstreamSources.map((source) => source.ingestReceiptChainDigest),
    receiptCounts: upstreamSources.map((source) => source.ingestReceiptCount),
  ...
  if (overrides.upstreamSources) {
    ...
    body.receiptChainDigests = ordered.map((source) => source.ingestReceiptChainDigest);
    body.receiptCounts = ordered.map((source) => source.ingestReceiptCount);
...
function splitReceipt(audit, overrides = {}) {
  const body = {
    ...
    scopeBindingsDigest: hex('1'),
    regimeBindingsDigest: hex('2'),
    splitAudit: audit,
...
function resignReceipt(receipt, overrides = {}) {
  const body = { ...withoutKey(receipt, 'receiptDigest'), ...overrides };
  return { ...body, receiptDigest: sha256(body) };
}
...
function outcome(audit, method, overrides = {}, suppliedReceipt = null) {
  const oos = audit.assignments.find((item) => item.split === 'OOS');
  const receipt = audit.schemaVersion === PUBLIC_FORWARD_LIQUIDITY_MULTI_SOURCE_SPLIT_AUDIT_VERSION
    ? (suppliedReceipt ?? splitReceipt(audit))
    : null;
  const lineage = audit.schemaVersion === PUBLIC_FORWARD_LIQUIDITY_MULTI_SOURCE_SPLIT_AUDIT_VERSION
    ? {
      splitReceiptDigest: receipt.receiptDigest,
      ...
      sourceReceiptChainDigest: oos.sourceReceiptChainDigest,
      sourceReceiptCount: oos.sourceReceiptCount,
...
function validate({ audit = legacySplitAudit(), method = methodology(), outcomes, receipt } = {}) {
  const currentReceipt = audit.schemaVersion === PUBLIC_FORWARD_LIQUIDITY_MULTI_SOURCE_SPLIT_AUDIT_VERSION
    ? (receipt === undefined ? splitReceipt(audit) : receipt)
    : null;
  return validatePublicForwardLiquidityOosOutcomes({
    splitAudit: audit,
    splitReceipt: currentReceipt,
    methodology: method,
    expectedOutcomeProducerCodeSha: PRODUCER_SHA,
    outcomes: outcomes ?? [outcome(audit, method, {}, currentReceipt)],
  });
}
...
test('multi-source frozen split requires authenticated producer receipt and preserves complete receipt-chain lineage', () => {
  const audit = multiSourceSplitAudit();
  const method = methodology();
  const receipt = splitReceipt(audit);
  const result = validate({ audit, method, receipt, outcomes: [outcome(audit, method, {}, receipt)] });
  assert.equal(result.status, 'PRESENT');
  assert.equal(result.validation.schemaVersion, PUBLIC_FORWARD_LIQUIDITY_MULTI_SOURCE_OOS_VALIDATION_VERSION);
  assert.equal(result.validation.splitReceiptVersion, PUBLIC_FORWARD_LIQUIDITY_MULTI_SOURCE_SPLIT_RECEIPT_VERSION);
  assert.equal(result.validation.splitReceiptDigest, receipt.receiptDigest);
  assert.equal(result.validation.evidenceLineageDigest, receipt.receiptDigest);
  ...
  assert.deepEqual(result.validation.receiptChainDigests, audit.receiptChainDigests);
  assert.deepEqual(result.validation.receiptCounts, audit.receiptCounts);
...
test('multi-source loose split audit without producer receipt fails closed', () => {
  const audit = multiSourceSplitAudit();
  const result = validate({ audit, receipt: null });
  assert.equal(result.status, 'BLOCKED_DATA');
  assert.ok(result.blockers.includes('MULTI_SOURCE_SPLIT_RECEIPT_REQUIRED'));
});

test('forged producer receipt digest fails closed', () => {
  const audit = multiSourceSplitAudit();
  const receipt = { ...splitReceipt(audit), receiptDigest: hex('f') };
  const result = validate({ audit, receipt });
  assert.equal(result.status, 'BLOCKED_DATA');
  assert.ok(result.blockers.includes('MULTI_SOURCE_SPLIT_RECEIPT_DIGEST_MISMATCH'));
});

test('re-signed producer receipt cannot point at another split audit', () => {
  const audit = multiSourceSplitAudit();
  const receipt = resignReceipt(splitReceipt(audit), { splitAuditDigest: hex('f') });
  const result = validate({ audit, receipt });
  assert.equal(result.status, 'BLOCKED_DATA');
  assert.ok(result.blockers.includes('MULTI_SOURCE_SPLIT_RECEIPT_AUDIT_DIGEST_MISMATCH'));
});

test('malformed upstream receipt-chain count fails closed before OOS credit', () => {
  const audit = multiSourceSplitAudit();
  const badFirst = { ...audit.upstreamSources[0], ingestReceiptCount: 2 };
  const badAudit = multiSourceSplitAudit({ upstreamSources: [badFirst, audit.upstreamSources[1]] });
  const result = validate({ audit: badAudit });
  assert.equal(result.status, 'BLOCKED_DATA');
  assert.ok(result.blockers.includes('UPSTREAM_SOURCE_RECEIPT_CHAIN_RECEIPTS_INVALID'));
});

test('split audit receipt-count lineage list is revalidated independently', () => {
  const audit = multiSourceSplitAudit();
  audit.receiptCounts = [2, 1];
  audit.auditDigest = sha256(withoutKey(audit, 'auditDigest'));
  const result = validate({ audit });
  assert.equal(result.status, 'BLOCKED_DATA');
  assert.ok(result.blockers.includes('UPSTREAM_RECEIPT_COUNTS_MISMATCH'));
});

test('multi-source assignment receipt-chain provenance must bind its authenticated upstream source', () => {
  const audit = multiSourceSplitAudit();
  audit.assignments = audit.assignments.map((item) => item.split === 'OOS'
    ? { ...item, sourceReceiptChainDigest: hex('f') }
    : item);
...
test('multi-source outcome receipt-chain provenance must match frozen assignment', () => {
  const audit = multiSourceSplitAudit();
  const method = methodology();
  const receipt = splitReceipt(audit);
  const invalid = outcome(audit, method, { sourceReceiptCount: 99 }, receipt);
  const result = validate({ audit, method, receipt, outcomes: [invalid] });
  assert.equal(result.status, 'BLOCKED_DATA');
  assert.ok(result.blockers.includes('OOS_SOURCE_RECEIPT_COUNT_MISMATCH'));
});
...
test('multi-source lineage tampering fails closed before OOS credit', () => {
  ...
  audit.auditDigest = sha256(withoutKey(audit, 'auditDigest'));
  const method = methodology();
  const result = validate({ audit, method });
  assert.equal(result.status, 'BLOCKED_DATA');
...
test('multi-source outcome cannot smuggle a synthetic aggregate dataset digest', () => {
  ...
  const method = methodology();
  const receipt = splitReceipt(audit);
  const invalid = outcome(audit, method, { datasetDigest: hex('f') }, receipt);
  const result = validate({ audit, method, receipt, outcomes: [invalid] });
...
test('safety contract keeps OOS validation evidence-only and requires authenticated multi-source lineage', () => {
  assert.equal(PUBLIC_FORWARD_LIQUIDITY_OOS_VALIDATION_SAFETY.verifiedFrozenSplitAuditRequired, true);
  assert.equal(PUBLIC_FORWARD_LIQUIDITY_OOS_VALIDATION_SAFETY.authenticatedMultiSourceSplitReceiptRequired, true);
  assert.equal(PUBLIC_FORWARD_LIQUIDITY_OOS_VALIDATION_SAFETY.multiSourceLineageRequired, true);
  assert.equal(PUBLIC_FORWARD_LIQUIDITY_OOS_VALIDATION_SAFETY.completeIngestReceiptChainRequired, true);
