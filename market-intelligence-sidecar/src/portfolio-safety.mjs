...
export function evaluateExpectedShortfall(raw = {}, policyInput = {}) {
  ...
  if (samples.length < policy.minExpectedShortfallSamples) {
    return { status: 'NOT_AVAILABLE', reason: 'EXPECTED_SHORTFALL_SAMPLE_INSUFFICIENT', sampleSize: samples.length, minimumSamples: policy.minExpectedShortfallSamples };
  }
  ...
  return {
    ...
    alpha, sampleSize: samples.length, varPct, expectedShortfallPct, tailSampleSize: tail.length, empiricalOnly: true,
  };
}
...
export function evaluatePortfolioRisk(raw = {}, policyInput = {}) {
  ...
  return {
    status: reasons.length ? 'VETO' : 'PASS', reasons, equityKrw, positionCount: positions.length, currentGrossKrw, proposedNotionalKrw,
    grossAfterKrw, grossExposurePct, proposedSymbol: proposedSymbol || null, singlePositionPct, proposedCluster, clusterExposurePct,
    clusterEvidenceStatus, correlationInvented: false,
  };
}
...
export function evaluateSignalFreshness(raw = {}, policyInput = {}, nowInput = Date.now()) {
  ...
  if (ageMs > ttlMs) return { status: 'VETO', reason: 'SIGNAL_TTL_EXPIRED', generatedAt, ageMs, ttlMs };
  const revalidatedAt = finite(raw.revalidatedAt);
  if (revalidatedAt == null) return { status: 'NOT_AVAILABLE', reason: 'SIGNAL_REVALIDATION_REQUIRED', generatedAt, ageMs, ttlMs };
  const revalidationAgeMs = Math.max(0, now - revalidatedAt);
  if (revalidationAgeMs > policy.maxRevalidationAgeMs) return { status: 'VETO', reason: 'SIGNAL_REVALIDATION_STALE', generatedAt, ageMs, ttlMs, revalidatedAt, revalidationAgeMs };
...
export function evaluateAntiChurn(raw = {}, policyInput = {}, nowInput = Date.now()) {
  ...
  return {
    status: reasons.length ? 'VETO' : 'PASS', reasons, requestedDirection: requestedDirection || null, sameDirectionEntries,
    windowMs: policy.churnWindowMs, lastExitAt, cooldownRemainingMs: lastExitAt == null ? 0 : Math.max(0, policy.reentryCooldownMs - (now - lastExitAt)),
  };
}
...
export function evaluateGlobalKillSwitch(raw = {}, policyInput = {}) {
  ...
  return {
    state: reasons.length ? 'BLOCK_NEW_ENTRIES' : 'NORMAL', reasons: [...new Set(reasons)], dailyDrawdownPct, rollingDrawdownPct,
    consecutiveExecutionErrors, forcedLiquidationAuthority: false, cancelAuthority: false, orderAuthority: false, executionAuthority: 'NONE',
  };
}
...
export function evaluatePortfolioSafety(raw = {}, policyInput = {}) {
  ...
  const insufficientEvidence = policy.enforcement === 'REQUIRED_FOR_PARENT_GATE' && required.some((status) => status === 'NOT_AVAILABLE');
  const state = vetoReasons.length ? 'VETO' : insufficientEvidence ? 'INSUFFICIENT_EVIDENCE' : 'PASS';
  return {
    contract: 'market-intelligence-portfolio-safety/v1', policy, portfolio, expectedShortfall, signalFreshness, antiChurn, killSwitch,
    scanner: { mode: 'OBSERVE_ONLY', candidateDeletionAllowed: false },
    autoTrading: {
      state, reasons: [...new Set(vetoReasons)], insufficientEvidence, newEntryAllowedByThisLayer: state === 'PASS' && killSwitch.state === 'NORMAL',
      parentGateStillRequired: true, orderAllowed: false, executionAuthority: 'NONE',
    },
    safety: { executionAuthority: 'NONE', privateTradingApiAllowed: false, realOrderAllowed: false, orderSubmissionAllowed: false, forcedLiquidationAllowed: false },
  };
}
