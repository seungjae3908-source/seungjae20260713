...
export async function executeAutoTradeCandidates(
	...
	const planResponse = await authorizedFetch("/api/stocks/auto-trade/plan", {
		...
		body: JSON.stringify({
			candidates: targets,
			totalInvestment: effectiveInvestmentPerTrade,
			stage: 1,
	...
	const approved = window.confirm(
		[
			...
			`수량: ${order.quantity}주`,
			`분할 단계: ${order.stage}차 진입 · 총액의 ${order.stageRatio}%`,
			`진입 비율: ${order.allocation.entryRatios.join(' / ')}%`,
			`청산 비율: ${order.allocation.exitRatios.join(' / ')}%`,
			`분석 조정 위험도: ${order.allocation.adjustedRiskScore}점 (${order.allocation.riskBand})`,
