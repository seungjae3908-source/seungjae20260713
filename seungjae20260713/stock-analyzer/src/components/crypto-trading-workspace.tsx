      ...
      {selectedScannerRow && (
        ...
        >
          <div className="max-h-[82vh] w-full max-w-sm overflow-y-auto rounded-3xl border border-card-border bg-card p-4 text-center shadow-2xl">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              <button type="button" onClick={() => setSelectedScannerRow(null)} className="rounded-2xl border border-card-border bg-background px-3 py-3 text-xs font-black">닫기</button>
              <button type="button" onClick={() => { selectSymbol(selectedScannerRow.ticker.symbol); setSelectedScannerRow(null); changeView("chart"); }} className="rounded-2xl bg-primary px-3 py-3 text-xs font-black text-primary-foreground">종목으로 이동</button>
            </div>
          </div>
        </div>
      )}
