...
import { CHART_TIMEFRAMES, loadVisibleChartTimeframes, saveVisibleChartTimeframes, type VisibleChartTimeframe } from "@/lib/chart-preferences";
...
const OVERLAYS: Array<{
	...
	{ key: "ma5", label: "5이평", group: "차트선", description: "최근 5개 봉의 종가 평균입니다. 현재가가 선 위에서 유지되는지로 매우 짧은 추세를 확인합니다." },
	{ key: "ma20", label: "20이평", group: "차트선", description: "최근 20개 봉 평균으로 단기 추세와 눌림 구간을 봅니다. 가격의 상향·하향 교차는 거래량과 함께 확인합니다." },
	{ key: "ma60", label: "60이평", group: "차트선", description: "최근 60개 봉 평균으로 중기 추세를 확인합니다. 짧은 이평선보다 반응은 느리지만 잡음이 적습니다." },
	{ key: "ma120", label: "120이평", group: "차트선", description: "최근 120개 봉 평균으로 장기 방향을 확인합니다. 봉 수가 부족하면 표시되지 않습니다." },
				...
				<>
					<section className="overflow-hidden rounded-3xl border border-card-border bg-card shadow-sm">
						<div className="flex items-center justify-between border-b border-card-border px-4 py-3">
							...
							<span className="text-[10px] font-extrabold text-muted-foreground">{TIMEFRAMES.find((item) => item.key === timeframe)?.label}</span>
						</div>
