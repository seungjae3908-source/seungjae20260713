...
export default function PortfolioPage() {
	...
	const [
		...
		useState('');
	const [settingsHolding, setSettingsHolding] = useState<Holding | null>(null);
	const [settingsQuantity, setSettingsQuantity] = useState('');
	const [settingsAveragePrice, setSettingsAveragePrice] = useState('');
	...
	async function addHolding(
		...
		try {
			...
			const missingPurchaseDateColumn =
				insertError &&
				/purchase_date|schema cache|column/i.test(
					String((insertError as { message?: string }).message ?? insertError),
				);

			if (missingPurchaseDateColumn) {
				const { purchase_date: _purchaseDate, ...legacyPayload } = holdingPayload;
			...
			rememberPurchaseDate(resolvedTicker, purchaseDate);
	...
	const openHoldingSettings = (holding: Holding) => {
		setSettingsHolding(holding);
		setSettingsQuantity(String(holding.quantity));
		setSettingsAveragePrice(String(holding.average_price));
	...
	const averagingResult = (() => {
		const currentQuantity = Number(settingsQuantity);
		const currentAverage = Number(settingsAveragePrice);
		const buyQuantity = Number(additionalQuantity);
		const buyPrice = Number(additionalPrice);
		if (![currentQuantity, currentAverage, buyQuantity, buyPrice].every(Number.isFinite) || currentQuantity <= 0 || currentAverage <= 0 || buyQuantity <= 0 || buyPrice <= 0) return null;
		const totalQuantity = currentQuantity + buyQuantity;
		return { totalQuantity, averagePrice: (currentQuantity * currentAverage + buyQuantity * buyPrice) / totalQuantity };
	})();

	const saveHoldingSettings = async () => {
		if (!settingsHolding || !auth.user) return;
		const nextQuantity = Number(settingsQuantity);
		const nextAveragePrice = Number(settingsAveragePrice);
		if (!Number.isFinite(nextQuantity) || nextQuantity <= 0 || !Number.isFinite(nextAveragePrice) || nextAveragePrice <= 0) {
			setSettingsError('보유수량과 구매가는 0보다 큰 숫자로 입력해 주세요.');
			return;
		}
		...
		try {
			const { error: updateError } = await getSupabase()
				.from('portfolio_holdings')
				.update({ quantity: nextQuantity, average_price: nextAveragePrice })
				.eq('id', settingsHolding.id)
				.eq('user_id', auth.user.id);
			if (updateError) throw updateError;
			await load();
			setSettingsHolding(null);
		} catch (cause) {
			setSettingsError(supabaseErrorMessage(cause));
		} finally {
	...
	useEffect(() => {
		...
		const close = (event: KeyboardEvent) => { if (event.key === 'Escape') setSettingsHolding(null); };
	...
	return (
		<div className="flex h-full min-h-0 flex-col overflow-y-auto overscroll-contain bg-background text-foreground">
										...
										return (
											...
											>
												<div className="flex items-start justify-between gap-3">
													...
													<button type="button" onClick={() => openHoldingSettings(row)} className="flex h-9 w-9 items-center justify-center rounded-xl border border-card-border text-muted-foreground active:bg-muted" aria-label={`${row.name} 설정`}><Settings2 className="h-4 w-4" /></button>
													<button
														type="button"
														onClick={() => {
															if (
															...
															navigate(
																`/stock/${encodeURIComponent(
																	row.ticker,
																)}?back=${encodeURIComponent('/portfolio')}`,
															);
														}}
														aria-disabled={
															manual
			...
			{settingsHolding && (
				<div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/65 p-3" onMouseDown={(event) => { if (event.target === event.currentTarget) setSettingsHolding(null); }}>
					<section role="dialog" aria-modal="true" aria-labelledby="holding-settings-title" className="max-h-[calc(100dvh-1.5rem)] w-full max-w-md overflow-y-auto rounded-3xl border border-card-border bg-background p-4 shadow-2xl">
						<div className="flex items-start justify-between gap-3"><div className="min-w-0"><h2 id="holding-settings-title" className="truncate text-left text-base font-black">{settingsHolding.name} 설정</h2><p className="mt-1 text-left text-[11px] font-bold text-muted-foreground">{settingsHolding.ticker}</p></div><button type="button" onClick={() => setSettingsHolding(null)} aria-label="설정 닫기" className="rounded-full bg-secondary p-2"><X className="h-4 w-4" /></button></div>
						<div className="mt-4 grid grid-cols-2 gap-2">
							<label className="text-left text-[11px] font-bold text-muted-foreground">보유수량 변경<input type="number" min="0" step="any" value={settingsQuantity} onChange={(event) => setSettingsQuantity(event.target.value)} className="mt-1 h-11 w-full rounded-xl border border-card-border bg-card px-3 text-sm font-bold text-foreground outline-none focus:border-primary" /></label>
							<label className="text-left text-[11px] font-bold text-muted-foreground">구매가 변경<input type="number" min="0" step="any" value={settingsAveragePrice} onChange={(event) => setSettingsAveragePrice(event.target.value)} className="mt-1 h-11 w-full rounded-xl border border-card-border bg-card px-3 text-sm font-bold text-foreground outline-none focus:border-primary" /></label>
						</div>
						<div className="mt-3 grid grid-cols-2 gap-2 rounded-2xl border border-card-border bg-card p-3">
							<div className="flex items-center justify-between rounded-xl bg-secondary/60 px-3 py-2"><span className="text-xs font-black">즐겨찾기</span><FavoriteButton item={{ ticker: settingsHolding.ticker, name: settingsHolding.name, market: settingsHolding.market, currency: settingsHolding.currency }} /></div>
							<div className="flex items-center justify-between rounded-xl bg-secondary/60 px-3 py-2"><span className="text-xs font-black">지정가 알림</span><InstrumentAlertButton instrument={{ ticker: settingsHolding.ticker, name: settingsHolding.name, market: settingsHolding.market }} /></div>
						</div>
						<div className="mt-3 rounded-2xl border border-card-border bg-card p-3 text-left">
							<h3 className="text-sm font-black">물타기 계산</h3><p className="mt-1 text-[10px] font-bold text-muted-foreground">현재 보유수량·현재 평단과 추가 구매수량·가격을 합산합니다.</p>
							<div className="mt-3 grid grid-cols-2 gap-2"><label className="text-[11px] font-bold text-muted-foreground">구매수량<input type="number" min="0" step="any" value={additionalQuantity} onChange={(event) => setAdditionalQuantity(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-card-border bg-background px-3 text-sm font-bold text-foreground" /></label><label className="text-[11px] font-bold text-muted-foreground">구매가격<input type="number" min="0" step="any" value={additionalPrice} onChange={(event) => setAdditionalPrice(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-card-border bg-background px-3 text-sm font-bold text-foreground" /></label></div>
							<div className="mt-3 grid grid-cols-2 gap-2"><HoldingValue label="계산 후 수량" value={averagingResult ? averagingResult.totalQuantity.toLocaleString() : '입력 필요'} /><HoldingValue label="계산 후 평단" value={averagingResult ? money(averagingResult.averagePrice, settingsHolding.currency) : '입력 필요'} /></div>
							<button type="button" disabled={!averagingResult} onClick={() => { if (!averagingResult) return; setSettingsQuantity(String(averagingResult.totalQuantity)); setSettingsAveragePrice(String(averagingResult.averagePrice)); }} className="mt-3 w-full rounded-xl border border-primary px-3 py-2 text-xs font-black text-primary disabled:opacity-40">계산 결과를 변경값에 적용</button>
						</div>
						{settingsError && <p className="mt-3 rounded-xl bg-destructive/10 p-3 text-left text-xs font-bold text-destructive">{settingsError}</p>}
						<div className="mt-4 grid grid-cols-2 gap-2"><button type="button" onClick={() => setSettingsHolding(null)} className="rounded-xl border border-card-border px-3 py-3 text-sm font-bold">취소</button><button type="button" onClick={() => void saveHoldingSettings()} disabled={settingsSaving} className="rounded-xl bg-primary px-3 py-3 text-sm font-black text-primary-foreground disabled:opacity-50">{settingsSaving ? '저장 중…' : '저장'}</button></div>
					</section>
				</div>
			)}
			</>
