...
function formatDateTime(now: Date) {
  return {
    date: new Intl.DateTimeFormat('ko-KR', { timeZone: 'Asia/Seoul', year: 'numeric', month: 'long', day: 'numeric', weekday: 'short' }).format(now),
    time: new Intl.DateTimeFormat('ko-KR', { timeZone: 'Asia/Seoul', hour: '2-digit', minute: '2-digit', second: '2-digit' }).format(now),
  };
}
    ...
    <div className="h-full overflow-y-auto overscroll-contain bg-background">
	  <header className="sticky top-0 z-30 border-b border-card-border bg-background/95 px-3 pb-3 pt-3 backdrop-blur">
        <div className="flex items-start justify-between gap-3">
          ...
          <p className="shrink-0 text-right text-[10px] font-bold leading-4 text-muted-foreground"><span className="block">{clock.date}</span><span className="block">{clock.time}</span></p>
        </div>
        <AssetSwitch className="mt-2" />
        {/* 검색은 현물·선물 또는 국내·해외 선택 바로 아래에 둡니다. */}
        <button type="button" onClick={() => navigate('/stocks')} className="mt-2 flex w-full items-center gap-2 rounded-xl border border-card-border bg-card px-3 py-2.5 text-left">
          <Search className="h-4 w-4 text-muted-foreground" />
          <span className="text-sm font-black text-muted-foreground">검색</span>
        </button>
      </header>
...
function StockHome({ mode, summary, sectorData, summaryLoading, summaryError, sectorLoading, sectorError, onNavigate }: { mode: 'KR' | 'US'; summary: AnyObj[]; sectorData?: SectorPopularData; summaryLoading: boolean; summaryError: boolean; sectorLoading: boolean; sectorError: boolean; onNavigate: (to: string) => void }) {
  const wanted = mode === 'KR' ? ['kospi', 'kosdaq'] : ['nasdaq', 'sp500'];
    ...
    <>
      ...
      {selected && (
        <SectorPopup title={`${selected.label} 인기종목`} sortBasis={sectorData?.sortBasis ?? '거래대금 기준'} onViewAll={() => onNavigate('/stocks')} onClose={() => setOpenSector(null)}>
          {selected.rows.slice(0, 10).map((row, index) => <StockRow key={`${row.market}:${row.ticker}`} row={row} rank={row.rank ?? index + 1} onClick={() => onNavigate(`/stock/${encodeURIComponent(row.ticker)}`)} />)}
    ...
    <>
      ...
      {selected && (
        <SectorPopup title={`${selected.label} 인기코인`} sortBasis="거래대금 기준" onViewAll={() => onNavigate('/stocks')} onClose={() => setOpenSector(null)}>
          {selectedRows.slice(0, 10).map((row, index) => <CryptoRow key={String(row.symbol)} row={row} rank={index + 1} currency={mode === 'spot' ? 'KRW' : 'USDT'} onClick={() => onNavigate(`/stock-info?asset=coin&coinMarket=${mode}&symbol=${encodeURIComponent(String(row.symbol))}`)} />)}
...
// 섹터를 눌렀을 때 뜨는 중앙 팝업 — 모바일에서도 화면 안에 맞춥니다.
function SectorPopup({ title, sortBasis, children, onViewAll, onClose }: { title: string; sortBasis: string; children: React.ReactNode; onViewAll: () => void; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
      <div role="dialog" aria-modal="true" aria-label={title} className="max-h-[85dvh] w-full max-w-md overflow-hidden rounded-3xl border border-card-border bg-card shadow-lg" onClick={(event) => event.stopPropagation()}>
...
function StockRow({ row, rank, onClick }: { row: QuoteRow | SectorPopularRow; rank: number; onClick: () => void }) {
  ...
  return <div role="button" tabIndex={0} onClick={onClick} onKeyDown={(event) => { if (event.key === 'Enter') onClick(); }} className="flex w-full items-center gap-2 rounded-2xl bg-secondary/60 p-3 text-left"><span className="w-5 text-center text-sm font-black text-primary">{rank}</span><div className="flex min-w-0 flex-1 items-start gap-1"><FavoriteButton item={{ ticker: row.ticker, name, market: row.market, currency: row.currency }} /><div className="min-w-0"><p className="truncate text-sm font-black">{name}</p><p className="mt-0.5 text-[10px] font-bold text-muted-foreground">{row.ticker}</p></div></div><div className="flex shrink-0 items-end gap-1"><div className="text-right"><p className="text-xs font-black">{formatAppPrice(row.price, row.currency)}</p><p className={cn('text-[10px] font-black', row.changePercent >= 0 ? 'text-positive' : 'text-destructive')}>{formatAppPercent(row.changePercent)}</p></div><InstrumentAlertButton instrument={{ ticker: row.ticker, name, market: row.market }} /></div></div>;
}
...
function CryptoRow({ row, rank, currency, onClick }: { row: AnyObj; rank: number; currency: string; onClick: () => void }) {
  ...
  const name = displayCoinName(symbol, row.koreanName, row.englishName);
  const coinMarket = currency === 'KRW' ? 'UPBIT' : 'BITGET';
  const assetType = currency === 'KRW' ? 'coin_spot' as const : 'coin_futures' as const;
  return <div role="button" tabIndex={0} onClick={onClick} onKeyDown={(event) => { if (event.key === 'Enter') onClick(); }} className="flex w-full items-center gap-2 rounded-2xl bg-secondary/60 p-3 text-left"><span className="w-5 text-center text-sm font-black text-primary">{rank}</span><div className="flex min-w-0 flex-1 items-start gap-1"><FavoriteButton item={{ ticker: symbol, name, market: coinMarket, currency }} /><div className="min-w-0"><p className="truncate text-sm font-black">{name}</p><p className="mt-0.5 text-[10px] font-bold text-muted-foreground">{symbol}</p></div></div><div className="flex shrink-0 items-end gap-1"><div className="text-right"><p className="text-xs font-black">{formatAppPrice(Number(row?.price), currency)}</p><p className={cn('text-[10px] font-black', change == null ? 'text-muted-foreground' : change >= 0 ? 'text-positive' : 'text-destructive')}>{change == null ? '데이터 없음' : formatAppPercent(change)}</p></div><InstrumentAlertButton instrument={{ ticker: symbol, name, market: coinMarket, assetType }} /></div></div>;
}
