  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-y-auto overscroll-contain bg-background">
      <header className="relative z-30 shrink-0 border-b border-card-border bg-background/95 px-4 pb-3 pt-5 backdrop-blur">
		<h1 className="text-left text-2xl font-extrabold">종목</h1>
          ...
          {rows.map((row) => {
            ...
            return (
              <div
                ...
                onKeyDown={(event) => { if (event.key === 'Enter') openDetail(row); }}
              ...
              >
                <div className="grid grid-cols-[36px_minmax(0,1fr)_auto] items-start gap-3">
                  ...
                  <div className="flex min-w-0 items-start gap-1">
                    <FavoriteButton item={{ ticker: row.ticker, name: stockName, market: row.market, currency: row.currency }} />
                  ...
                  <div className="flex shrink-0 items-end gap-1 text-right">
                    ...
                    <InstrumentAlertButton instrument={{ ticker: row.ticker, name: stockName, market: row.market }} />
                  </div>
                </div>
