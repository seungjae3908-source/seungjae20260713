...
const CHART_SECTIONS = [
  { title: '추세선', upIds: ['trend', 'moving-average', 'support-resistance'], downIds: ['trend', 'moving-average', 'support-resistance'] },
  {
    title: '패턴',
    upIds: ['double-bottom', 'inverse-head-shoulders', 'ascending-triangle', 'bull-flag', 'cup-handle', 'falling-wedge', 'triangle'],
    downIds: ['double-top', 'head-shoulders', 'descending-triangle', 'bear-flag', 'rising-wedge', 'triangle'],
  },
  { title: '캔들', upIds: ['bullish-candles', 'candlestick'], downIds: ['bearish-candles', 'candlestick'] },
  { title: '지표', upIds: ['bollinger', 'rsi', 'macd', 'volume'], downIds: ['bollinger', 'rsi', 'macd', 'volume'] },
  { title: '신호', upIds: ['golden-cross', 'breakout', 'pullback'], downIds: ['death-cross', 'breakdown', 'failed-rally'] },
] as const;

type ChartSectionTitle = (typeof CHART_SECTIONS)[number]['title'];
type ChartDirection = '상승' | '하락';

const FINANCE_TOPIC_IDS = ['income-statement', 'balance-sheet', 'cash-flow', 'per', 'pbr', 'roe', 'debt'];
const KNOWLEDGE_TOPIC_IDS = ['dilution', 'delisting'];
...
export default function LearnPage() {
  ...
  const [section, setSection] = useState<StudySection>('차트');
  const [chartSection, setChartSection] = useState<ChartSectionTitle>('추세선');
  const [chartDirection, setChartDirection] = useState<ChartDirection>('상승');
  ...
  const visibleTopics = useMemo(() => {
    if (section === '재무') return TOPICS.filter((topic) => FINANCE_TOPIC_IDS.includes(topic.id));
    if (section === '지식') return TOPICS.filter((topic) => KNOWLEDGE_TOPIC_IDS.includes(topic.id));
    const category = CHART_SECTIONS.find((item) => item.title === chartSection) ?? CHART_SECTIONS[0];
    const ids: readonly string[] = chartDirection === '상승' ? category.upIds : category.downIds;
    return ids.flatMap((id) => TOPICS.filter((topic) => topic.id === id));
  }, [chartDirection, chartSection, section]);
  ...
  if (selected) {
    return (
	  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 p-4" onMouseDown={closeTopic}>
		<section className="flex max-h-[88vh] w-full max-w-lg flex-col overflow-hidden rounded-3xl border border-card-border bg-background shadow-2xl" onMouseDown={(event) => event.stopPropagation()} role="dialog" aria-modal="true" aria-label={`${selected.title} 설명`}>
		<header className="flex-none border-b border-card-border bg-background px-4 py-3">
		  <div className="flex items-center justify-between gap-3">
			<div className="min-w-0">
			  <h1 className="truncate text-left text-lg font-extrabold">{selected.title}</h1>
			  <p className="mt-0.5 text-left text-xs font-bold text-muted-foreground">{selected.group}</p>
			</div>
			<button
			  type="button"
			  onClick={closeTopic}
			  className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-card-border bg-card text-muted-foreground"
			  aria-label="닫기"
			>
			  <X className="h-4 w-4" />
			</button>
		...
		<main className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-3">
		  <div className="space-y-2">
            <StudyCard title="쉬운 설명">
			  <p className="break-keep text-left text-sm font-semibold leading-relaxed text-foreground">
                {selected.easy}
              </p>
            </StudyCard>
            ...
            <StudyCard title="잘못 쓰면 위험한 점">
              <div className="flex gap-3">
				...
				<p className="break-keep text-left text-sm font-semibold leading-relaxed text-destructive">
                  {selected.danger}
                </p>
              </div>
            </StudyCard>
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-background">
      <header className="sticky top-0 z-20 border-b border-card-border bg-background/95 px-4 pb-4 pt-5 glass">
		<h1 className="text-left text-2xl font-extrabold">공부</h1>
      </header>
      ...
      >
		<section className="rounded-3xl border border-card-border bg-card p-3 shadow-sm">
		  <div className="grid grid-cols-3 gap-1">
			{(['차트', '재무', '지식'] as const).map((item) => (
			  <button key={item} type="button" onClick={() => { setSection(item); if (listRef.current) listRef.current.scrollTop = 0; }} className={cn('rounded-xl px-2 py-2 text-xs font-extrabold', section === item ? 'bg-primary text-primary-foreground' : 'bg-background text-muted-foreground')}>{item}</button>
			))}
		  </div>
			...
			<>
			  <div className="mt-2 flex gap-1 overflow-x-auto pb-1">
				{CHART_SECTIONS.map((item) => (
				  <button key={item.title} type="button" onClick={() => setChartSection(item.title)} className={cn('shrink-0 rounded-xl border px-3 py-2 text-[11px] font-extrabold', chartSection === item.title ? 'border-primary bg-primary/10 text-primary' : 'border-card-border bg-background text-muted-foreground')}>{item.title}</button>
				))}
			  </div>
			  <div className="mt-2 grid grid-cols-2 gap-1">
				{(['상승', '하락'] as const).map((direction) => (
				  <button key={direction} type="button" onClick={() => setChartDirection(direction)} className={cn('rounded-xl px-2 py-2 text-xs font-black', chartDirection === direction ? direction === '상승' ? 'bg-positive text-white' : 'bg-destructive text-white' : 'bg-background text-muted-foreground')}>{direction}</button>
				))}
			  </div>
			  <h2 className="mt-3 text-left text-sm font-extrabold">{chartSection} · {chartDirection}</h2>
			</>
		  )}
		  {section !== '차트' && <h2 className="mt-3 text-left text-sm font-extrabold">{section} 학습 항목</h2>}

		  <div className="mt-2 space-y-1.5">
			{visibleTopics.map((topic) => (
			  <StudyTopicButton key={`${section}:${chartDirection}:${topic.id}`} topic={topic} onClick={() => openTopic(topic)} />
			))}
		  </div>
		</section>
      </main>
...
function StudyTopicButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
	  className="grid w-full grid-cols-[40px_minmax(0,1fr)_20px] items-center gap-2 rounded-2xl border border-card-border bg-background p-2.5 text-left transition active:scale-[0.99]"
    >
      ...
      <div className="min-w-0">
		<p className="text-sm font-extrabold">{topic.title}</p>

		<p className="mt-0.5 line-clamp-2 break-keep text-[11px] font-semibold leading-relaxed text-muted-foreground">
          {topic.short}
        </p>
      </div>
...
function StudyCard({
  ...
  return (
	<section className="rounded-2xl border border-card-border bg-card p-3 shadow-sm">
	  <h2 className="mb-2 text-left text-sm font-extrabold">{title}</h2>
