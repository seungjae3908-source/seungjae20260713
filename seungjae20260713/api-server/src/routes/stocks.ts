...
const SUPPORTED_TIMEFRAMES: readonly Timeframe[] = [
	"1m", "3m", "5m", "15m", "30m", "60m", "1H", "4H", "8H", "12H",
	"1D", "3D", "5D", "10D", "15D", "20D", "1W", "1M", "3M", "6M", "1Y", "ALL",
];

function isTimeframe(value: string): value is Timeframe {
	return SUPPORTED_TIMEFRAMES.some((timeframe) => timeframe === value);
}

function normalizeTimeframe(value: unknown): Timeframe {
	const raw = String(value ?? "1D").trim();
	return isTimeframe(raw) ? raw : "1D";
}
...
function dailyChartRiskMetrics(candles: Array<{ high?: unknown; low?: unknown; close?: unknown }>) {
	const rows = candles
		.map((candle) => ({
	...
	const latestClose = rows.at(-1)?.close ?? 0;
	return {
		volatilityPercent: latestClose > 0 ? atr / latestClose * 100 : 0,
...
interface AutoTradeApprovalPlan {
	...
	order: {
		...
		stage: TradeStage; stageRatio: number; totalInvestment: number; allocation: StagedTradeAllocation;
	};
}
...
router.post("/auto-trade/plan", requireAdmin, async (req: AuthenticatedRequest, res) => {
	const enabled = process.env.KIWOOM_AUTO_TRADE_ENABLED === "true";
	const realMode = String(process.env.KIWOOM_MODE ?? "").trim().toLowerCase() === "real";
	const configuredKey = String(process.env.KIWOOM_AUTO_TRADE_KEY ?? "").trim();
	const suppliedKey = String(req.header("X-Auto-Trade-Key") ?? "").trim();
	...
	const currency: "KRW" | "USD" = market === "US" ? "USD" : "KRW";
	const totalInvestment = Math.max(1, Math.min(100_000_000, Math.round(Number(req.body?.totalInvestment ?? req.body?.investmentPerTrade ?? 0))));
	...
	const [quote, dailyMeta] = await Promise.all([
		MarketDataService.getQuoteRow(ticker),
		MarketDataService.getCandlesMeta(ticker, "1D"),
	]);
	...
	const chartMetrics = dailyChartRiskMetrics(dailyMeta.candles);
	if (!chartMetrics) return res.status(409).json({ ok: false, message: "분할 비율 산정에 필요한 실제 일봉이 20개 미만입니다." });
	const candidateCompleteness = Number(candidate.dataCompleteness);
	const allocation = deriveStagedTradeAllocation({
		confidenceScore: Number(candidate.probability ?? candidate.score),
		riskScore: Number(candidate.riskScore),
		dataCompleteness: Number.isFinite(candidateCompleteness)
			? Math.min(candidateCompleteness, chartMetrics.dataCompleteness)
			: chartMetrics.dataCompleteness,
	...
	const investmentPerTrade = Math.floor(stageAmount(totalInvestment, allocation.entryRatios, stage));
	if (investmentPerTrade <= 0) return res.status(409).json({ ok: false, message: "해당 단계에 배정된 주문금액을 계산하지 못했습니다." });
	...
	const plan: AutoTradeApprovalPlan = {
		...
		body: { candidates, investmentPerTrade, totalInvestment, stage, allocation, stopLossPercent, takeProfitPercent },
		order: { ticker, name: String(candidate.name ?? ticker), market, currency, quantity, currentPrice, estimatedAmount: quantity * currentPrice, stopPrice, targetPrice, stage, stageRatio: allocation.entryRatios[stage - 1], totalInvestment, allocation },
	};
...
router.post("/auto-trade/close-plan", requireAdmin, async (req: AuthenticatedRequest, res) => {
...
router.post("/auto-trade/close-execute", requireAdmin, async (req: AuthenticatedRequest, res) => {
	cleanupAutoTradeApprovalPlans();
...
router.post("/auto-trade/execute", requireAdmin, async (req: AuthenticatedRequest, res) => {
	cleanupAutoTradeApprovalPlans();
	const approvalToken = String(req.body?.approvalToken ?? "").trim();
...
router.post("/auto-trade/monitor", requireAdmin, async (req: AuthenticatedRequest, res) => {
	const configuredKey = String(process.env.KIWOOM_AUTO_TRADE_KEY ?? "").trim();
	const suppliedKey = String(req.header("X-Auto-Trade-Key") ?? "").trim();
	if (!configuredKey || suppliedKey !== configuredKey) return res.status(401).json({ ok: false, message: "자동매매 실행키가 올바르지 않습니다." });
...
router.get("/auto-trade/status", requireAdmin, (_req, res) => {
...
router.get("/auto-trade/journal", requireAdmin, async (req: AuthenticatedRequest, res) => {
	await ensureAutoTradePositionsLoaded();
