...
import { requireFullMember, requireMember } from '../middleware/auth';
...
router.get('/market/scan', requireMember, requireFullMember, async (req, res) => {
  const scope = normalizeMarket(req.query.market);
...
router.get('/market/recommendations', requireMember, requireFullMember, async (req, res) => {
  const market = String(req.query.market ?? 'KR').toUpperCase() === 'US' ? 'US' : 'KR';
