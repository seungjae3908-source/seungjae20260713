...
// Shared stock classification logic used by api-server and stock-analyzer.
...
function removeNegatedRiskText(text: string) {
  return text
    ...
    .replace(/상장\s*폐지\s*(위험|리스크|가능성|우려)?\s*(없음|낮음|해당\s*없음|미해당|아님|없다|낮다)/g, '')
...
export function classifyStock(input: StockClassifierInput): StockGrade {
  ...
  const delistingKeywords = ['상장폐지', 'delisting', '관리종목', '거래정지'];
  const delistingWarning = delistingKeywords.some((k) => cleanedText.includes(k));

  const dilutionKeywords = ['유상증자', '전환사채', 'cb', 'bw', 'atm', 'offering', '희석'];
  const dilutionScore = dilutionKeywords.filter((k) => cleanedText.includes(k)).length;

  const otherRiskKeywords = ['횡령', '배임', '검찰', '수사', '분식'];
  const otherRiskScore = otherRiskKeywords.filter((k) => cleanedText.includes(k)).length;

  let score = ai;
  const reasons: string[] = [];

  if (delistingWarning && !protectedLargeCap) {
    score -= 30;
    reasons.push('상장폐지 위험 키워드가 확인됩니다.');
  }

  if (dilutionScore >= 2 && !protectedLargeCap) {
    score -= Math.min(20, dilutionScore * 6);
    reasons.push('주식 희석 위험 키워드가 다수 확인됩니다.');
  } else if (dilutionScore === 1 && !protectedLargeCap) {
    score -= 8;
  }

  if (debtRatio != null && debtRatio > 300 && !protectedLargeCap) {
    score -= 10;
    reasons.push(`부채비율이 ${debtRatio.toFixed(0)}%로 높습니다.`);
  }

  if (equity != null && equity <= 0) {
    score -= 15;
    reasons.push('자본잠식 상태입니다.');
  }

  if (roe != null && roe >= 15 && !delistingWarning) {
    score += 5;
  }

  if (otherRiskScore > 0) {
    score -= protectedLargeCap ? Math.min(8, otherRiskScore * 3) : Math.min(18, 8 + otherRiskScore * 5);
  ...
  if (trueJunk) {
    label = '잡주';
    reason = reasons[0] ?? '시총, 재무, 공시 리스크 기준으로 고위험 종목에 가깝습니다.';
  } else if (bluechip) {
    label = '우량주';
    reason = '시총, 재무 안정성, 시장 대표성을 기준으로 우량주에 가깝습니다.';
    ...
    label = '보통주';
    reason = '시총, 재무, 차트, 공시 기준으로 우량/저평가/고위험에 강하게 치우치지 않습니다.';
  }

  const riskCaption = buildRiskCaption(label, delistingWarning, dilutionScore, marketCap, input.currency);
...
/** Identity conversion — StockGrade is the canonical output of classifyStock. */
export function toStockGrade(classification: StockGrade): StockGrade {
