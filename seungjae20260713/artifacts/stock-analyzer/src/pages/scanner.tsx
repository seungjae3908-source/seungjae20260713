...
export default function ScannerPage() {
	...
	const autoTradeStatus = useQuery({
		...
		queryFn: async () => {
			...
			if (!response.ok)
				throw new Error(payload?.message || "자동매매 상태 확인 실패");
			return payload as {
				mode: "real" | "mock";
				enabled: boolean;
		...
		return source
			.map((card) => {
				...
				const assessment = assessAutoTradeCandidate({
					...
					financialScore: toNumber(
						card.financialScore ?? card.fundamentalScore,
					),
	...
	const tradeJournal = useQuery({
		...
		queryFn: async () => {
			...
			if (!response.ok)
				throw new Error(payload.message || "매매일지를 불러오지 못했습니다.");
	...
	const journalAnalysis = useMemo(() => {
		...
		const averageProfit = closed.length
			? closed.reduce(
					(sum, entry) => sum + Number(entry.profitPercent ?? 0),
					0,
				) / closed.length
			: 0;
	...
	const autoRiskPreview = useMemo(
		() =>
			calculateAutoTradeRiskPreview(
				autoSettings,
				autoCandidates[0]?.price ?? null,
			),
		[
			autoSettings.accountValue,
			autoSettings.riskPerTradePercent,
			autoSettings.investmentPerTrade,
			autoSettings.stopLossPercent,
			autoCandidatesKey,
		],
	);

	const autoSafetySnapshot = useMemo(
		() =>
			calculateAutoTradeSafetySnapshot(
				tradeJournal.data ?? [],
				autoSettings,
		...
		[
			...
			market,
			autoSettings.accountValue,
			autoSettings.dailyLossLimitPercent,
			autoSettings.maxOpenPositions,
			autoSettings.maxConsecutiveLosses,
			autoSettings.maxDailyOrders,
		],
	);
			...
			try {
				...
				if (approvalSignals.length) {
					const summary = approvalSignals
						.slice(0, 3)
						.map(
							(item) => `${item.ticker}: ${item.message ?? "청산 승인 필요"}`,
						)
						.join(" / ");
					setAutoMessage(
						`청산 조건이 감지됐습니다. 아래 매도계획 확인 버튼을 눌러야 주문이 전송됩니다. ${summary}`,
					);
				}
			} catch (error) {
	...
	const approvePositionClose = async (signal: AutoTradeExitSignal) => {
		...
		try {
			...
			setAutoMessage(
				result.message ||
					(result.ok
						? `${signal.ticker} 매도 주문을 전송했습니다.`
						: "매도 승인을 취소했습니다."),
			);
			if (result.ok) {
				setExitSignals((current) =>
					current.filter(
						(item) =>
							!(item.ticker === signal.ticker && item.market === signal.market),
					),
				);
				...
				{viewMode === "auto" && (
					<section className="rounded-3xl border border-card-border bg-card p-4 shadow-sm">
						<div className="flex items-start justify-between gap-3">
							<div className="min-w-0">
								...
								<p className="mt-1 break-keep text-[11px] font-semibold leading-5 text-muted-foreground">
									최대 100개 후보를 모델점수순으로 비교하며, 주문 버튼을 누른 뒤
									주문 내용을 한 번 더 승인한 1개 종목만 전송합니다.
								</p>
							</div>
							<button
								type="button"
						...
						<div className="mt-3 grid grid-cols-2 gap-2">
							<label className="rounded-2xl border border-card-border bg-background p-3">
								<span className="block text-[10px] font-extrabold text-muted-foreground">
									계좌 평가금액 ({market === "US" ? "USD" : "원"})
								</span>
								<input
									type="number"
									min={1}
									step={market === "US" ? 100 : 100000}
									inputMode="numeric"
									value={autoSettings.accountValue || ""}
									onChange={(event) =>
										updateAutoSettings({
											accountValue:
												event.target.value === ""
							...
							<label className="rounded-2xl border border-card-border bg-background p-3">
								...
								<div className="mt-1 flex items-center gap-1">
									<input
										...
										max={2}
										step={0.05}
										value={autoSettings.riskPerTradePercent || ""}
										onChange={(event) =>
											updateAutoSettings({
												riskPerTradePercent:
													event.target.value === ""
							...
							<label className="rounded-2xl border border-card-border bg-background p-3">
								...
								<div className="mt-1 flex items-center gap-1">
									<input
										...
										max={10}
										step={0.05}
										value={autoSettings.dailyLossLimitPercent || ""}
										onChange={(event) =>
											updateAutoSettings({
							...
							<label className="rounded-2xl border border-card-border bg-background p-3">
								...
								<div className="mt-1 flex items-center gap-1">
									<input
										...
										max={10}
										step={1}
										value={autoSettings.maxOpenPositions || ""}
										onChange={(event) =>
											updateAutoSettings({
												maxOpenPositions:
													event.target.value === ""
										...
										className="min-w-0 flex-1 bg-transparent text-sm font-extrabold outline-none"
									/>
									<span className="text-xs font-extrabold">종목</span>
								</div>
							</label>
							<label className="rounded-2xl border border-card-border bg-background p-3">
								<span className="block text-[10px] font-extrabold text-muted-foreground">
								...
								<div className="mt-1 flex items-center gap-1">
									<input
										...
										max={10}
										step={1}
										value={autoSettings.maxConsecutiveLosses || ""}
										onChange={(event) =>
											updateAutoSettings({
										...
										className="min-w-0 flex-1 bg-transparent text-sm font-extrabold outline-none"
									/>
									<span className="text-xs font-extrabold">회</span>
								</div>
							</label>
							<label className="rounded-2xl border border-card-border bg-background p-3">
								<span className="block text-[10px] font-extrabold text-muted-foreground">
								...
								<div className="mt-1 flex items-center gap-1">
									<input
										...
										max={20}
										step={1}
										value={autoSettings.maxDailyOrders || ""}
										onChange={(event) =>
											updateAutoSettings({
												maxDailyOrders:
													event.target.value === ""
						...
						<div className="mt-2 rounded-2xl border border-primary/20 bg-primary/5 p-3">
							...
							<div className="mt-2 grid grid-cols-2 gap-x-3 gap-y-1 text-[10px] font-bold text-muted-foreground">
								<p>
									허용손실금액 ·{" "}
									{formatAppPrice(
										autoRiskPreview.riskBudget,
										market === "US" ? "USD" : "KRW",
									)}
								</p>
								<p>예상 수량 · {autoRiskPreview.finalQuantity}주</p>
								<p>
									1주당 위험 ·{" "}
									{formatAppPrice(
										autoRiskPreview.perShareRisk,
										market === "US" ? "USD" : "KRW",
									)}
								</p>
								<p>
									예상 주문금액 ·{" "}
									{formatAppPrice(
										autoRiskPreview.estimatedAmount,
										market === "US" ? "USD" : "KRW",
									)}
								</p>
							</div>
						...
						<p className="mt-2 break-keep text-[10px] font-semibold leading-4 text-muted-foreground">
							실행키는 키움 비밀번호가 아닌 주문 보호키이며 이 브라우저 탭이
							닫히면 폐기됩니다. 모델점수는 후보 비교용이지 수익확률이 아닙니다.
							모든 매수·매도 주문은 주문별 확인 전까지 전송되지 않습니다.
						</p>
						...
						<div className="mt-2 grid grid-cols-2 gap-2 text-center text-[10px] font-extrabold">
							<div
								className={cn(
									...
									autoTradeStatus.data?.mode === "real"
										? "bg-emerald-500/10 text-emerald-600"
										: "bg-amber-500/10 text-amber-600",
								)}
							>
								서버 ·{" "}
								{autoTradeStatus.data?.mode === "real" ? "실전" : "모의/미설정"}
							</div>
							<div
								className={cn(
									"rounded-xl px-2 py-2",
									autoTradeStatus.data?.enabled
										? "bg-emerald-500/10 text-emerald-600"
						...
						<div
							className={cn(
								"mt-2 rounded-2xl border px-3 py-2 text-[10px] font-bold leading-5",
								autoSafetySnapshot.allowed
									? "border-emerald-500/20 bg-emerald-500/5 text-emerald-700"
									: "border-destructive/30 bg-destructive/5 text-destructive",
							)}
						>
							<p>
								보유 {autoSafetySnapshot.openPositions}/
								{autoSettings.maxOpenPositions} · 오늘 주문{" "}
								{autoSafetySnapshot.dailyOrders}/{autoSettings.maxDailyOrders} ·
								연속손실 {autoSafetySnapshot.consecutiveLosses}/
								{autoSettings.maxConsecutiveLosses}
							</p>
							<p>
								오늘 확정 손실{" "}
								{formatAppPrice(
									autoSafetySnapshot.dailyLossAmount,
									market === "US" ? "USD" : "KRW",
								)}{" "}
								({autoSafetySnapshot.dailyLossPercent.toFixed(2)}%)
							</p>
							{!autoSafetySnapshot.allowed &&
								autoSafetySnapshot.blockedReason && (
									<p>신규 주문 정지 · {autoSafetySnapshot.blockedReason}</p>
								)}
						</div>
							...
							<div className="mt-2 space-y-2 rounded-2xl border border-destructive/30 bg-destructive/5 p-3">
								...
								{exitSignals.map((signal) => (
									<div
										...
										className="flex items-center justify-between gap-3 rounded-xl bg-background p-2"
									>
										<div className="min-w-0">
									...
									<div className="grid grid-cols-3 gap-2 text-center">
										<div className="rounded-xl bg-secondary/70 p-2">
											...
											<p className="mt-1 text-xs font-extrabold">
												{journalAnalysis.total}건 / {journalAnalysis.open}건
											</p>
										</div>
										<div className="rounded-xl bg-secondary/70 p-2">
											<p className="text-[9px] font-bold text-muted-foreground">
											...
											<p className="mt-1 text-xs font-extrabold">
												{journalAnalysis.closed
													? `${journalAnalysis.winRate}%`
													: "-"}
											</p>
										</div>
										<div className="rounded-xl bg-secondary/70 p-2">
											...
											<p
												className={cn(
													...
													journalAnalysis.averageProfit > 0
														? "text-emerald-500"
														: journalAnalysis.averageProfit < 0
															? "text-destructive"
															: "",
											...
											>
												{journalAnalysis.closed
													? `${journalAnalysis.averageProfit > 0 ? "+" : ""}${journalAnalysis.averageProfit}%`
													: "-"}
											</p>
										</div>
									</div>
												...
												>
													...
													<div className="mt-3 space-y-3 border-t border-card-border pt-3 text-[11px] leading-5">
														<div>
															...
															<p className="mt-1 break-keep font-semibold text-muted-foreground">
																{entry.entryAnalysis}
															</p>
															<p className="mt-1 font-bold">
																근거:{" "}
																{entry.entryReasons.join(" · ") || "AI 조건"}
															</p>
														</div>
														<div className="grid grid-cols-3 gap-2 text-center">
															<div className="rounded-xl bg-secondary/70 p-2">
		...
		>
			...
			{expanded && (
				<>
					...
					>
						{classification.label} · {classification.reason}
					</div>
...
function CoinTradingWorkspace({
	...
	const rows = useMemo<
		Array<AnyObj & { rank: number; modelScore: number; change: number }>
	>(() => {
		const source =
		...
		const ranked = [...source]
			...
			.sort(
				(a, b) =>
					Number(b.tradingValue24h ?? 0) - Number(a.tradingValue24h ?? 0),
			);
		const maximumTradingValue = Math.max(
			1,
			...ranked.map((row) => Number(row.tradingValue24h ?? 0)),
		);
		return ranked.slice(0, 100).map((row, index) => {
			...
			const liquidityScore = Math.min(
				45,
				Math.round(
					(Number(row.tradingValue24h ?? 0) / maximumTradingValue) * 45,
				),
			);
			const momentumScore = Math.max(
				0,
				Math.min(35, Math.round(17.5 + change * 2.5)),
			);
			const riskPenalty = Math.min(30, Math.round(Math.abs(change) * 1.5));
			const modelScore = Math.max(
				0,
				Math.min(100, 20 + liquidityScore + momentumScore - riskPenalty),
			);
	...
	const connected =
		mode.coinMarket === "spot"
			? Boolean(statusData.upbit?.ok)
			: Boolean(statusData.bitget?.ok);
	const privateConfigured =
		mode.coinMarket === "spot"
			? Boolean(statusData.upbit?.privateKeyConfigured)
			: Boolean(statusData.bitget?.privateKeyConfigured);
	...
	return (
		<div className="h-full overflow-y-auto overscroll-contain bg-background">
			...
			<header className="border-b border-card-border px-4 pb-3 pt-4">
				<div className="flex items-start justify-between gap-3">
					<h1 className="text-xl font-black">도구</h1>
					<button
						type="button"
			...
			<main className="space-y-4 px-4 pb-28 pt-4">
				<section className="rounded-3xl border border-card-border bg-card p-4 shadow-sm">
					<div className="grid grid-cols-2 gap-2 text-center text-[11px] font-black">
						...
						>
							{mode.coinMarket === "spot" ? "UPBIT" : "BITGET"} 시세 ·{" "}
							{connected ? "정상" : "오류"}
						</div>
						<div
							className={cn(
						...
						>
							개인 API · {privateConfigured ? "환경변수 설정" : "미설정"}
						</div>
					</div>
					<p className="mt-3 rounded-2xl bg-warning/10 p-3 text-[11px] font-bold leading-relaxed text-warning">
						{viewMode === "auto"
							? "현재 이 화면은 실제 후보와 연결상태를 검증합니다. 코인 주문은 거래소별 주문 전 검사와 주문별 승인 토큰이 완성되기 전까지 잠겨 있으며 자동 전송되지 않습니다."
							: "검색 결과는 실제 거래소 공개 시세이며 모델점수는 상대 비교값이지 수익확률이 아닙니다."}
					</p>
				</section>
				<section className="rounded-3xl border border-card-border bg-card p-4 shadow-sm">
					<div className="flex items-center justify-between">
						<div>
							<h2 className="text-sm font-black">후보 종목</h2>
							<p className="mt-1 text-[10px] font-bold text-muted-foreground">
								거래대금·변동성·단기 방향 기반 모델점수
							</p>
						</div>
					...
					<div className="mt-3 space-y-2">
						{rows.map((row) => (
							<button
								...
								onClick={() =>
									navigate(
										`/stock-info?asset=coin&coinMarket=${mode.coinMarket}&symbol=${encodeURIComponent(String(row.symbol))}`,
									)
								}
								className="flex w-full items-center gap-3 rounded-2xl bg-secondary/60 p-3 text-left"
							>
