...
type CoreDetailData = Pick<
  DetailData,
  "ticker" | "quote" | "company" | "candles"
>;
type AnalysisDetailData = Pick<DetailData, "financials" | "risk">;
type ContentDetailData = Pick<DetailData, "filings" | "news">;

const LazyPanel = lazy(() => import("@/components/detail/lazy-panel"));
...
async function fetchCoreDetail(ticker: string): Promise<CoreDetailData> {
  const upper = ticker.toUpperCase();
...
async function fetchAnalysisDetail(ticker: string): Promise<AnalysisDetailData> {
  const upper = ticker.toUpperCase();
  const [financialRaw, riskRaw] = await Promise.all([
    tryJson<AnyObj>([`/api/stocks/${upper}/financials`], {}),
    tryJson<AnyObj>(
      [`/api/stocks/${upper}/risk`, `/api/stocks/${upper}/analysis`],
      {},
...
async function fetchContentDetail(
  ...
  const upper = ticker.toUpperCase();
  const [filingsRaw, newsRaw] = await Promise.all([
    tryJson<AnyObj>(
      [
        `/api/stocks/${upper}/filings`,
        `/api/stocks/${upper}/disclosures`,
  ...
  return {
    filings: filings.length ? filings : collectFilings(risk ?? {}),
    news: collectNews(newsRaw),
  };
}
...
export default function DetailPage() {
  ...
  const [indicatorStageReady, setIndicatorStageReady] = useState(false);

  const coreDetail = useQuery<CoreDetailData>({
    queryKey: ["stock-detail-core-v14", ticker],
  ...
  useEffect(() => {
    setIndicatorStageReady(false);
    if (!coreDetail.data) return;
    let cancelled = false;
    ...
    const handle = schedule(() => {
      if (!cancelled) setIndicatorStageReady(true);
    });
    return () => {
      cancelled = true;
      cancel(handle);
    };
  }, [ticker, coreDetail.data]);

  const analysisDetail = useQuery<AnalysisDetailData>({
    queryKey: ["stock-detail-analysis-v14", ticker],
    queryFn: () => fetchAnalysisDetail(ticker),
    enabled: Boolean(ticker && coreDetail.data && indicatorStageReady),
    staleTime: 5 * 60_000,
    gcTime: 30 * 60_000,
    refetchOnWindowFocus: false,
  });

  const contentDetail = useQuery<ContentDetailData>({
    queryKey: ["stock-detail-content-v14", ticker],
    queryFn: () => fetchContentDetail(ticker, analysisDetail.data?.risk ?? null),
    enabled: Boolean(ticker && analysisDetail.isSuccess),
    staleTime: 10 * 60_000,
    gcTime: 60 * 60_000,
    refetchOnWindowFocus: false,
  });
  ...
  const data = useMemo<DetailData | undefined>(() => {
    ...
    return {
      ...coreDetail.data,
      financials: analysisDetail.data?.financials ?? null,
      risk: analysisDetail.data?.risk ?? null,
      filings: contentDetail.data?.filings ?? [],
      news: contentDetail.data?.news ?? [],
    };
  }, [coreDetail.data, analysisDetail.data, contentDetail.data]);
  ...
  return (
    ...
    >
      ...
      <main className="flex-none px-3 pb-28 pt-3">
        ...
        {data && tab === "ai" && (
          <Suspense fallback={<PanelSkeleton text="AI 분석을 준비하는 중..." />}>
        ...
        {data && tab === "financials" && (
          <Suspense fallback={<PanelSkeleton text="재무정보를 불러오는 중..." />}>
            <LazyPanel minHeight={420}>
              {() => <MemoFinancialTab financials={data.financials} currency={currency} />}
            </LazyPanel>
          </Suspense>
        )}

        {data && tab === "filings" && (
          <Suspense fallback={<PanelSkeleton text="공시를 불러오는 중..." />}>
            <LazyPanel minHeight={320}>
              {() => <MemoFilingTab
            ticker={ticker}
            market={market}
        ...
        {data && tab === "news" && (
          <Suspense fallback={<PanelSkeleton text="뉴스를 불러오는 중..." />}>
            <LazyPanel minHeight={320}>
              {() => <MemoNewsTab ticker={ticker} news={data.news} summary={insights.newsAiSummary} />}
            </LazyPanel>
          </Suspense>
        )}
      </main>
...
function PanelSkeleton({ text }: { text: string }) {
  return (
    <div className="flex min-h-60 items-center justify-center rounded-2xl border border-card-border bg-card">
...
const MemoAiTab = memo(AiTab);
const MemoChartTab = memo(ChartTab);
const MemoFinancialTab = memo(FinancialTab);
const MemoFilingTab = memo(FilingTab);
const MemoNewsTab = memo(NewsTab);