...
export default function HomePage() {
  ...
  return (
    <div className="text-center h-full overflow-y-auto overscroll-contain bg-background">
      <header className="text-center border-b border-card-border px-4 py-4">
        <div className="text-center flex items-center justify-between gap-4">
          <h1 className="text-center text-2xl font-black">지식정보</h1>
          <time className="text-center shrink-0 text-sm font-black tabular-nums text-muted-foreground">
            {formatClock(now)}
          </time>
        </div>
      </header>

      <main className="text-center space-y-4 px-4 pb-28 pt-4">
        <section>
          <div className="text-center">
            <h2 className="text-center text-xl font-black">오늘의 증시현황</h2>
            <p className="text-center mt-1 text-xs font-bold text-muted-foreground">
              주요 뉴스와 시장 데이터를 함께 확인합니다.
            </p>
          </div>

          <div className="text-center mt-4 grid grid-cols-3 gap-2 rounded-2xl bg-secondary/70 p-1.5">
        ...
        <section className="rounded-3xl border border-card-border bg-card p-4 text-center shadow-sm">
          <div>
            <h3 className="text-center text-base font-black">{VIEW_META[view].indexTitle}</h3>
            <span className="text-center mt-1 block text-[10px] font-bold text-muted-foreground">실시간 제공기관 기준</span>
          </div>

          {view === 'COIN' ? (
            <div className="text-center mt-3 grid grid-cols-1 gap-2 sm:grid-cols-3">
            ...
            <div className="text-center mt-3 grid grid-cols-2 gap-2">
        ...
        <section className="rounded-3xl border border-card-border bg-card p-4 text-center shadow-sm">
          <div className="text-center flex flex-col items-center gap-2">
            <div>
              <h3 className="text-center text-base font-black">오늘의 이슈</h3>
              <p className="text-center mt-1 text-[11px] font-bold text-muted-foreground">주식시장 주요 뉴스 AI 분석</p>
            </div>
          ...
          {briefing.data && (
            <div className="text-center mt-4 space-y-4">
              <div className="text-center rounded-2xl bg-secondary/60 p-4">
                <p className="text-center text-sm font-black leading-6">{briefing.data.headline}</p>
                <p className="text-center mt-2 whitespace-pre-line text-sm font-semibold leading-6 text-muted-foreground">
              ...
              {briefing.data.issues.length > 0 && (
                <div className="text-center space-y-2">
                  {briefing.data.issues.slice(0, 5).map((issue, index) => (
                    ...
                    >
                      <p className="text-center line-clamp-2 text-sm font-semibold leading-5">
                        {issue.summary}
                      </p>
                      <div className="text-center mt-1 flex items-center justify-center gap-1 text-[10px] font-bold text-muted-foreground">
                        <span>{issue.source}</span>
                        <ExternalLink className="text-center h-3.5 w-3.5 shrink-0" />
                      </div>
                    </a>
                  ))}
                </div>
...
function IndexCard({ label, value, change }: { label: string; value: string; change: number | null }) {
  return (
    <div className="rounded-2xl bg-secondary/60 p-3 text-center">
      <p className="text-center text-[11px] font-bold text-muted-foreground">{label}</p>
      <p className="text-center mt-1 truncate text-base font-black">{value}</p>
      <p className={cn(
