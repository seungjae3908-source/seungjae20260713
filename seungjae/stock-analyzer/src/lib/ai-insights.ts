...
export function buildAiInsights(input: AiInsightInput): AiInsightResult {
  ...
  const chartSummary = [
    change > 0
      ? `오늘 등락률은 ${change.toFixed(2)}%로 상승 흐름입니다.`
      : change < 0
        ? `오늘 등락률은 ${change.toFixed(2)}%로 하락 흐름입니다.`
        : '오늘 주가는 보합권입니다.',
    close != null && ma20 != null
      ? close > ma20
        ? '현재가는 20일선 위에 있어 단기 추세는 양호합니다.'
        : '현재가는 20일선 아래에 있어 단기 회복 확인이 필요합니다.'
      : '20일선 기준 데이터는 추가 확인이 필요합니다.',
    close != null && ma60 != null
      ? close > ma60
        ? '60일선 위에서 움직여 중기 흐름도 나쁘지 않습니다.'
        : '60일선 아래라 중기 추세 회복은 아직 부족합니다.'
      : '60일선 기준 데이터는 추가 확인이 필요합니다.',
    Math.abs(change) >= 8
      ? '단기 변동성이 커서 추격매수보다 눌림과 손절 기준이 중요합니다.'
      : '단기 변동성은 과도한 구간은 아닙니다.',
  ];
