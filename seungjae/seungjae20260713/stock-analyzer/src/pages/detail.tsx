...
const TIMEFRAMES: Array<{ key: ChartTimeframe; label: string }> = CHART_TIMEFRAMES.map((item) => ({ ...item }));
...
function positiveSummary(insights: ReturnType<typeof buildAiInsights>): string {
  ...
  return (
    rows.find((row) =>
      /호재|긍정|계약|수주|승인|상향 돌파|골든크로스|지지선 반등|패턴 완성/.test(row),
    ) ??
    rows[0] ??
    "최근 확인된 뚜렷한 호재가 없습니다."
  );
...
function negativeSummary(insights: ReturnType<typeof buildAiInsights>): string {
  ...
  return (
    rows.find((row) =>
      /악재|부정|희석|상장폐지|거래정지|하향 이탈|데드크로스|지지선 이탈|패턴 무효화/.test(row),
    ) ??
    rows[0] ??
    "최근 확인된 뚜렷한 악재가 없습니다."
  );
...
function OverviewTab({
  ...
  const description = rawDescription && !isPlaceholderDescription(rawDescription)
    ? rawDescription
    : inferCompanyBusiness(name, data.ticker, data.company);
  ...
  return (
    <div className="space-y-3">
      <SectionCard title="AI 종합 판단" subtitle="현재 데이터 기준">
		...
		<div className="mt-2 grid grid-cols-3 gap-2">
		  <MiniMetric label="적정가" value={metrics.fairPrice != null ? formatAppPrice(metrics.fairPrice, currency) : "산출 불가"} valueClassName="text-primary" />
		  <MiniMetric label="목표가" value={metrics.targetPrice != null ? formatAppPrice(metrics.targetPrice, currency) : "산출 불가"} valueClassName="text-positive" />
		  <MiniMetric label="손절가" value={metrics.stopPrice != null ? formatAppPrice(metrics.stopPrice, currency) : "산출 불가"} valueClassName="text-destructive" />
		</div>
...
function ChartTab({
  ...
  useEffect(() => {
    if (!visibleTimeframes.includes(timeframe)) {
      setTimeframe(visibleTimeframes[0] ?? "1m");
    }
  }, [timeframe, visibleTimeframes]);

  const toggleVisibleTimeframe = (frame: ChartTimeframe) => {
    setVisibleTimeframes((current) => {
      const next = current.includes(frame)
        ? current.filter((item) => item !== frame)
        : [...current, frame];
    ...
    <div className="space-y-3">
      ...
      {explanation && (
        <Modal title={explanation.title} subtitle="실제 차트에서 조건 위치 확인" onClose={() => setExplanation(null)}>
          ...
          <ProfessionalChart candles={candles} loading={chartQuery.isLoading} timeframe={timeframe} indicators={{ ...indicators, rsi: explanation.focus.preferredIndicator === "rsi" || indicators.rsi, macd: explanation.focus.preferredIndicator === "macd" || indicators.macd, bollinger: explanation.focus.preferredIndicator === "bollinger" || indicators.bollinger, volume: explanation.focus.preferredIndicator === "volume" || indicators.volume, sma20: explanation.focus.preferredIndicator === "moving-average" || indicators.sma20 }} fullscreen={false} portfolioOverlay={portfolioOverlay} autoSignal={autoSignal} studyFocus={explanation.focus} tradeEntries={tradeEntries} metrics={metrics} />
...
function PriceChartCanvas({
  ...
  useEffect(() => {
    ...
    if (metrics.targetPrice != null && metrics.targetPrice > 0) {
      candleSeries.createPriceLine({
        price: metrics.targetPrice,
        color: "#16a34a",
        lineWidth: 3,
    ...
    if (metrics.stopPrice != null && metrics.stopPrice > 0) {
      candleSeries.createPriceLine({
        price: metrics.stopPrice,
        color: "#dc2626",
        lineWidth: 3,
        lineStyle: LineStyle.Solid,
        axisLabelVisible: true,
  ...
  }, [rows, timeframe, indicators, height, portfolioOverlay, autoSignal, studyFocus, technicalMarkers, numberedSignals, actualTradeMarkers, analysisMarkersVisible, metrics, onChartReady]);
