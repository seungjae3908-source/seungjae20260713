...
export default function StocksPage() {
  ...
  useEffect(() => {
    setCategory('ai');
    setQuery('');
  }, [mode.asset, mode.coinMarket, mode.stockMarket]);
  ...
  return (
    <div className="h-full overflow-y-auto overscroll-contain bg-background">
	  ...
	  <header className="sticky top-0 z-30 border-b border-card-border bg-background/95 px-3 pb-3 pt-3 backdrop-blur">
        <h1 className="text-left text-xl font-black">종목</h1>

        {/* 1) 종목 검색창 — 제목 바로 아래에 붙여 하나의 상단 영역처럼 보이게(작은 간격). 입력 텍스트 왼쪽 정렬 유지 */}
        <label className="mt-2 flex h-11 items-center gap-2 rounded-xl border border-card-border bg-card px-3">
          ...
          />
		  {query ? <button type="button" onClick={() => setQuery('')} aria-label="검색어 지우기" className="rounded-full p-1 text-muted-foreground"><X className="h-4 w-4" /></button> : null}
        </label>
      ...
      <main className="space-y-2 px-3 pb-28 pt-3">
            ...
            {mode.asset === 'stock' ? (
              <>
                ...
                <div className="space-y-2">
                  {searchStocks.slice(0, 10).map((stock) => (
              ...
              <>
                ...
                <div className="space-y-2">
                  {searchCoins.slice(0, 10).map((row) => (
...
function StockRow({ stock, onClick }: { stock: AnyObj; onClick: () => void }) {
  ...
  const name = displayStockName(ticker, String(stock.name ?? ''), String(stock.market));
  return (
    <div role="button" tabIndex={0} onClick={onClick} onKeyDown={(event) => { if (event.key === 'Enter') onClick(); }} className="flex w-full items-center gap-2 rounded-2xl border border-card-border bg-card p-3 text-left shadow-sm">
      <div className="flex min-w-0 flex-1 items-start gap-1"><FavoriteButton item={{ ticker, name, market: String(stock.market), currency: String(stock.currency) }} /><div className="min-w-0"><div className="flex items-center gap-2"><p className="truncate text-sm font-black">{name}</p><span className="rounded-full bg-secondary px-2 py-0.5 text-[9px] font-black text-muted-foreground">{stock.market}</span></div><p className="mt-0.5 text-[11px] font-bold text-muted-foreground">{ticker}</p></div></div>
      <div className="flex shrink-0 items-end gap-1"><div className="text-right"><p className="text-sm font-black">{formatAppPrice(stock.price, String(stock.currency))}</p><p className={cn('mt-0.5 text-[11px] font-black', change > 0 ? 'text-positive' : change < 0 ? 'text-destructive' : 'text-muted-foreground')}>{Number.isFinite(change) ? formatAppPercent(change) : '데이터 없음'}</p></div><InstrumentAlertButton instrument={{ ticker, name, market: String(stock.market) }} /></div>
    </div>
  );
}
...
function CoinRow({ row, coinMarket, onClick }: { row: AnyObj; coinMarket: 'spot' | 'futures'; onClick: () => void }) {
  ...
  const name = displayCoinName(ticker, row.koreanName, row.englishName);
  const market = coinMarket === 'spot' ? 'UPBIT' : 'BITGET';
  return (
    <div role="button" tabIndex={0} onClick={onClick} onKeyDown={(event) => { if (event.key === 'Enter') onClick(); }} className="flex w-full items-center gap-2 rounded-2xl border border-card-border bg-card p-3 text-left shadow-sm">
      <div className="flex min-w-0 flex-1 items-start gap-1"><FavoriteButton item={{ ticker, name, market, currency }} /><div className="min-w-0"><p className="truncate text-sm font-black">{name}</p><p className="mt-0.5 text-[11px] font-bold text-muted-foreground">{ticker} · {market}</p></div></div>
      <div className="flex shrink-0 items-end gap-1"><div className="text-right"><p className="text-sm font-black">{formatAppPrice(Number(row.price), currency)}</p><p className={cn('mt-0.5 text-[11px] font-black', change > 0 ? 'text-positive' : change < 0 ? 'text-destructive' : 'text-muted-foreground')}>{Number.isFinite(change) ? formatAppPercent(change) : '데이터 없음'}</p></div><InstrumentAlertButton instrument={{ ticker, name, market, assetType: coinMarket === 'spot' ? 'coin_spot' : 'coin_futures' }} /></div>
    </div>
  );
}
    ...
    <div className="space-y-2">
      ...
      <div className="space-y-2">
        {rows.slice(0, 10).map((row) => {
		  const name = displayStockName(row.ticker, row.name, row.market);
		  return (
          <div key={`${row.market}:${row.ticker}`} role="button" tabIndex={0} onClick={() => onOpenStock(row.ticker)} onKeyDown={(event) => { if (event.key === 'Enter') onOpenStock(row.ticker); }} className="w-full rounded-2xl border border-card-border bg-card p-3 text-left shadow-sm">
            <div className="flex items-start justify-between gap-3">
              <div className="flex min-w-0 flex-1 items-start gap-1">
				<FavoriteButton item={{ ticker: row.ticker, name, market: row.market, currency: row.currency }} />
				<div className="min-w-0"><div className="flex items-center gap-2"><p className="truncate text-sm font-black">{name}</p><span className="rounded-full bg-secondary px-2 py-0.5 text-[9px] font-black text-muted-foreground">{row.market}</span></div>
                <p className="mt-0.5 text-[11px] font-bold text-muted-foreground">{row.ticker} · 상승 가능성 {row.score}점</p>
				</div>
              </div>
			  <div className="flex shrink-0 items-end gap-1"><div className="text-right"><p className="text-sm font-black">{formatAppPrice(row.price, row.currency)}</p><p className={cn('mt-0.5 text-[11px] font-black', row.changePercent > 0 ? 'text-positive' : row.changePercent < 0 ? 'text-destructive' : 'text-muted-foreground')}>{formatAppPercent(row.changePercent)}</p></div><InstrumentAlertButton instrument={{ ticker: row.ticker, name, market: row.market }} /></div>
            </div>
...
function CoinCategoryResults({
  ...
  return (
    <div className="space-y-2">
      {sortedCoins.slice(0, 10).map((row, index) => (
