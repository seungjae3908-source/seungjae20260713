...
type AuthContextValue = {
  ...
  profile: MemberProfile | null; displayName: string | null; isAdmin: boolean; isFullMember: boolean; isApproved: boolean;
  signIn(loginName: string, password: string): Promise<void>;
  signUp(loginName: string, password: string, displayName: string, birthDate: string): Promise<void>;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<AuthContextValue>(() => ({
    ...
    isAdmin:
      ...
      isFullMember:
        profile?.status === 'approved' && (profile.role === 'member' || profile.role === 'admin'),
      isApproved:
    ...
    async signUp(loginName, password, displayName, birthDate) {
      const name = validate(loginName, password); const normalized = normalizeName(name);
      const response = await fetch('/api/auth/register', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ loginName: normalized, password, displayName, birthDate }),
      });
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(String(payload?.message ?? payload?.error ?? '가입 신청에 실패했습니다.'));
    },
