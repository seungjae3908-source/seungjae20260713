...
// ---------------------------------------------------------------------------
export const WATCHLIST_KEY = 'seungjae_watchlist_v2';
export const WATCHLIST_CHANGE_EVENT = 'seungjae-watchlist-changed';
let activeWatchlistMemberId: string | null = null;

function watchlistStorageKey(): string {
  return activeWatchlistMemberId
    ? `${WATCHLIST_KEY}:${activeWatchlistMemberId}`
    : `${WATCHLIST_KEY}:signed-out`;
}

export function setWatchlistAccount(memberId: string | null): void {
  const next = memberId?.trim() || null;
  if (activeWatchlistMemberId === next) return;
  activeWatchlistMemberId = next;
