...
import { X, Maximize2, Minimize2, Search, RefreshCw, Settings2 } from 'lucide-react';
import {
  ColorType,
  CrosshairMode,
  LineStyle,
...
async function loadCandles(target: SearchTarget, tf: Tf): Promise<{ chart: ChartData | null; rows: StudyRow[]; error: string | null }> {
  ...
  const chart = await api.chart(target.id, tf);
...
export function StudyChart({ config, onClose }: { config: StudyChartConfig; onClose: () => void }) {
  ...
  const deepLink = useMemo(() => {
    ...
    const requestedTf = p.get('studyTf');
    const tfParam = CHART_TIMEFRAMES.find((item) => item.key === requestedTf)?.key ?? '1D';
  ...
  const [visibleTimeframes, setVisibleTimeframes] = useState<Tf[]>(loadVisibleChartTimeframes);
  ...
  useEffect(() => {
    if (!visibleTimeframes.includes(tf)) setTf(visibleTimeframes[0]);
  }, [tf, visibleTimeframes]);

  useEffect(() => {
    if (!timeframeSettingsOpen) return;
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'Escape') setTimeframeSettingsOpen(false);
    };
    window.addEventListener('keydown', onKeyDown);
    return () => window.removeEventListener('keydown', onKeyDown);
  }, [timeframeSettingsOpen]);

  const toggleTimeframe = (timeframe: Tf) => {
    ...
    >
      ...
      <div className="flex-none border-b border-card-border bg-background/95 px-3 py-3 glass">
        <div className="flex items-center justify-between gap-2">
          <p className="flex-1 truncate text-left text-base font-black">{config.title}</p>
        ...
        <div className="mt-2 flex flex-wrap items-center justify-center gap-1.5">
          ...
          <Segmented
            options={CHART_TIMEFRAMES.filter((item) => visibleTimeframes.includes(item.key)).map((item) => ({ key: item.key, label: item.label }))}
            value={tf}
            onChange={(value) => {
              const selected = CHART_TIMEFRAMES.find((item) => item.key === value)?.key;
              if (selected) setTf(selected);
            }}
          />
          <button
            type="button"
            onClick={() => setTimeframeSettingsOpen(true)}
            className="rounded-lg border border-card-border bg-card p-1.5 text-muted-foreground"
            aria-label="차트 시간 설정"
          >
      ...
      {timeframeSettingsOpen && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/55 p-4" onMouseDown={() => setTimeframeSettingsOpen(false)}>
          <section className="w-full max-w-md rounded-3xl border border-card-border bg-card p-5 shadow-2xl" onMouseDown={(event) => event.stopPropagation()} role="dialog" aria-modal="true" aria-label="공부 차트 시간 설정">
            <div className="flex items-center justify-between gap-3">
              <h2 className="text-left text-lg font-black">차트 시간 설정</h2>
              <button type="button" onClick={() => setTimeframeSettingsOpen(false)} className="flex h-9 w-9 items-center justify-center rounded-full border border-card-border" aria-label="닫기"><X className="h-4 w-4" /></button>
            </div>
            <p className="mt-2 text-left text-xs font-bold text-muted-foreground">선택한 시간만 차트 위에 표시됩니다.</p>
            <div className="mt-4 grid grid-cols-3 gap-2">
              {CHART_TIMEFRAMES.map((item) => {
                const active = visibleTimeframes.includes(item.key);
                return <button key={item.key} type="button" onClick={() => toggleTimeframe(item.key)} className={cn('rounded-xl border px-2 py-2 text-xs font-black', active ? 'border-primary bg-primary text-primary-foreground' : 'border-card-border bg-background text-muted-foreground')}>{item.label}</button>;
              })}
            </div>
          </section>
        </div>
