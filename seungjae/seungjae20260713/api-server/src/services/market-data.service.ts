...
import { koreanAliases } from '../data/us-korean-names';
...
const CANDLE_CACHE_VERSION = 'v4' as const;
...
function candleCachePath(ticker: string, timeframe: string): string {
  ...
  // Windows cache paths are case-insensitive, so values such as 3m and 3M
  // must not be written under visually different but equivalent file names.
  const safeTimeframe = Buffer.from(String(timeframe), 'utf8').toString('hex');
  return path.join(
...
function aggregateDailyProviderCandles(
  ...
  if (timeframe === '3M' || timeframe === '6M') {
    const monthly = aggregateCalendarCandles(rows, '1M');
    return aggregateCachedCandles(monthly, timeframe === '3M' ? 3 : 6);
  }
...
function catalogArray(): CatalogEntry[] {
  const base = (Array.isArray(CATALOG) ? CATALOG : []).map((entry) => {
    if (entry.market !== 'US') return entry;

    const aliases = koreanAliases(entry.ticker);
    const koreanName = aliases.find((alias) => /[가-힣]/u.test(alias));

    if (!aliases.length) return entry;

    return {
      ...entry,
      name: koreanName ?? entry.name,
      aliases: Array.from(new Set([entry.name, ...(entry.aliases ?? []), ...aliases])),
    };
  });
...
async function tryCandlesProvider(
  ...
  const isIntradayTimeframe = ['1m', '3m', '5m', '15m', '30m', '60m', '1H', '4H', '8H', '12H'].includes(timeframeText);
  const minimumUsefulCandles = ['1D', '3D', '5D', '10D', '15D', '20D', 'ALL'].includes(timeframeText) ? 30 : 2;
