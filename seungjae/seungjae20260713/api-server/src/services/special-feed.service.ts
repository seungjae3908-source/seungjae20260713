...
async function scanEntry(entry: CatalogEntry) {
  ...
  // 더는 제공되지 않는 신호는 TTL을 기다리지 않고 즉시 최신 피드에서 제거합니다.
  for (const signature of before) {
    if (current.has(signature)) continue;
    const idPrefix = `${entry.market}:${entry.ticker}:signal:${normalizeKey(signature)}:`;
    for (const [id, item] of feedItems) {
      if (item.kind === "signal" && id.startsWith(idPrefix)) feedItems.delete(id);
    }
    lastSignalAddedAt.delete(signature);
  }
