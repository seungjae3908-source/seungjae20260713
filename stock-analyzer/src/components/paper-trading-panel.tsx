...
type Props = {
  execute?: (state: PaperTradingState, action: PaperTradingAction, signal?: AbortSignal) => Promise<PaperTradingActionResult>;
...
export function PaperTradingPanel({
  ...
  async function runAction(action: PaperTradingAction | (() => Promise<PaperTradingAction>)) {
    ...
    try {
      ...
      if (generation === actionGeneration.current) setError(cause instanceof Error ? cause.message : '모의거래 작업을 처리하지 못했습니다.');
    } finally {
      if (generation === actionGeneration.current) { actionController.current = null; actionLock.current = false; setBusy(false); }
    }
  }
  ...
  async function refreshMarket() {
    ...
    } catch (cause) { if (sequence === requestSequence.current) setError(cause instanceof Error ? cause.message : '시장 데이터를 갱신하지 못했습니다.'); }
  }
  ...
  async function applyCandle() {
    await runAction(async () => {
      ...
      return { type: 'process_candle', eventId: eventId('candle'), candle };
    });
  }
  ...
  async function importJson(file: File) {
    ...
    try {
      ...
      if (generation === actionGeneration.current) { actionLock.current = false; setBusy(false); }
    }
  }
  ...
  return <main className="h-full overflow-y-auto overscroll-contain pb-28" data-testid="paper-trading-page">
    ...
    {resetStep ? <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 p-3 sm:items-center" role="dialog" aria-modal="true" aria-label="전체 초기화 확인"><div className="w-full max-w-md rounded-2xl bg-card p-4"><div className="flex items-start gap-2"><AlertTriangle className="h-5 w-5 text-destructive" /><div><h2 className="font-bold">현재 모의거래 기록을 초기화할까요?</h2><p className="mt-1 text-xs text-muted-foreground">계좌·주문·포지션·현재 거래일지를 초기화합니다. 별도 보관된 과거 거래일지는 유지됩니다.</p></div></div><div className="mt-4 grid grid-cols-2 gap-2"><button className={buttonClass} onClick={() => setResetStep(false)}>돌아가기</button><button className="rounded-lg bg-destructive px-3 font-bold text-destructive-foreground" disabled={busy} onClick={resetState}>2단계 초기화</button></div></div></div> : null}
  </main>;
}
