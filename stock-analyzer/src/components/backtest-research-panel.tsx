...
function Metric({ label, value, testId }: { label: string; value: string; testId?: string }) {
  return <div className="rounded-xl border border-border/70 bg-background/70 p-3">
    <div className="text-[11px] text-muted-foreground">{label}</div>
    <div className="mt-1 break-words text-base font-semibold" data-testid={testId}>{value}</div>
  </div>;
}
      ...
      <div className="h-56 w-full min-w-0">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 8, right: 12, bottom: 8, left: 0 }}>
            ...
            <XAxis dataKey="label" minTickGap={24} fontSize={10} />
            <YAxis width={58} fontSize={10} domain={['auto', 'auto']} />
            <Tooltip />
            <Line type="monotone" dataKey={dataKey} stroke="currentColor" dot={false} isAnimationActive={false} />
          </LineChart>
        </ResponsiveContainer>
      </div>
    )}
...
export function BacktestResearchPanel({ execute = runBacktest, initialResult = null, compact = false }: Props) {
  ...
  async function submit(event: FormEvent) {
    ...
    setResult(null);
    try {
      setResult(await execute(values));
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : '백테스트를 실행하지 못했습니다.');
    } finally {
      setLoading(false);
    }
  }

  const equityData = useMemo(() => result?.equityCurve.map((point) => ({
    label: new Date(point.timestamp).toISOString().slice(5, 16).replace('T', ' '),
    equity: Number(point.equity.toFixed(2)),
  })) ?? [], [result]);
  const drawdownData = useMemo(() => result?.drawdownCurve.map((point) => ({
    label: new Date(point.timestamp).toISOString().slice(5, 16).replace('T', ' '),
    ...
    <div className={`mx-auto w-full ${compact ? 'max-w-5xl' : 'max-w-6xl'} space-y-4 px-4 py-5 sm:px-5`}>
      <header className="rounded-2xl border border-border bg-card p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 text-xs leading-5 text-muted-foreground">완료 봉 신호를 계산하고 다음 봉 시가부터 체결합니다. 실제 주문 기능과 분리된 읽기 전용 분석입니다.</p>
          </div>
        </div>
      ...
      <form onSubmit={submit} className="rounded-2xl border border-border bg-card p-4" aria-busy={loading} data-testid="backtest-form">
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
          <Field label="종목"><input id="backtest-symbol" className={inputClass} value={values.symbol} onChange={(event) => update('symbol', event.target.value.toUpperCase())} inputMode="text" /></Field>
          ...
          <Field label="레버리지"><input className={inputClass} type="number" min="1" max="10" step="1" inputMode="decimal" value={values.leverage} onChange={(event) => update('leverage', Number(event.target.value))} /></Field>
          <Field label="진입 수수료"><input className={inputClass} type="number" min="0" step="0.0001" inputMode="decimal" value={values.entryFeeRate} onChange={(event) => update('entryFeeRate', Number(event.target.value))} /></Field>
          <Field label="청산 수수료"><input className={inputClass} type="number" min="0" step="0.0001" inputMode="decimal" value={values.exitFeeRate} onChange={(event) => update('exitFeeRate', Number(event.target.value))} /></Field>
          <Field label="슬리피지"><input className={inputClass} type="number" min="0" step="0.0001" inputMode="decimal" value={values.slippageRate} onChange={(event) => update('slippageRate', Number(event.target.value))} /></Field>
          <Field label="펀딩비/구간"><input className={inputClass} type="number" step="0.0001" inputMode="decimal" value={values.fundingRatePerInterval} onChange={(event) => update('fundingRatePerInterval', Number(event.target.value))} /></Field>
          <Field label="펀딩 간격(시간)"><input className={inputClass} type="number" min="1" step="1" inputMode="decimal" value={values.fundingIntervalHours} onChange={(event) => update('fundingIntervalHours', Number(event.target.value))} /></Field>
          <Field label="손절 방식"><select className={inputClass} value={values.stopLossMode} onChange={(event) => update('stopLossMode', event.target.value as BacktestFormValues['stopLossMode'])}><option value="percent">퍼센트</option><option value="atr">ATR</option><option value="swing">스윙</option></select></Field>
          <Field label="손절 값"><input className={inputClass} type="number" min="0.01" step="0.1" inputMode="decimal" value={values.stopLossValue} onChange={(event) => update('stopLossValue', Number(event.target.value))} /></Field>
          <Field label="목표 방식"><select className={inputClass} value={values.takeProfitMode} onChange={(event) => update('takeProfitMode', event.target.value as BacktestFormValues['takeProfitMode'])}><option value="risk_multiple">위험 배수</option><option value="percent">퍼센트</option></select></Field>
      ...
      {result ? <div className="space-y-4" data-testid="backtest-results">
        <section className="rounded-2xl border border-border bg-card p-4">
          <div className="mb-3 flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-emerald-500" /><h2 className="text-sm font-semibold">비용 차감 후 성과</h2></div>
        ...
        <section className="rounded-2xl border border-border bg-card p-4" data-testid="validation-results">
          ...
          <div className="grid gap-2 sm:grid-cols-3">{result.validationPerformance.map((item) => <div key={item.name} className="rounded-xl border border-border p-3 text-xs"><div className="font-semibold uppercase">{item.name}</div><div className="mt-2 grid gap-1 text-muted-foreground"><span>거래 {item.trades}회</span><span>순손익 {money(item.netPnl)}</span><span>낙폭 {percent(item.maximumDrawdownPercent)}</span></div></div>)}</div>
        </section>
        ...
        <section className="rounded-2xl border border-border bg-card p-4" data-testid="direction-results">
          ...
          <div className="grid gap-2 sm:grid-cols-2">{([['롱', result.longPerformance], ['숏', result.shortPerformance]] as const).map(([label, item]) => <div key={label} className="rounded-xl border border-border p-3 text-xs"><div className="font-semibold">{label}</div><div className="mt-2 grid grid-cols-2 gap-1 text-muted-foreground"><span>거래 {item.trades}회</span><span>승률 {percent(item.winRate)}</span><span>순손익 {money(item.netPnl)}</span><span>기대값 {money(item.expectancy)}</span></div></div>)}</div>
        </section>

        <section className="rounded-2xl border border-border bg-card p-4" data-testid="walk-forward-results">
          <h3 className="mb-3 text-sm font-semibold">워크포워드 기본형</h3>
          <div className="overflow-x-auto rounded-xl border border-border"><table className="w-full min-w-[560px] text-left text-xs"><thead className="bg-muted"><tr><th className="p-2">구간</th><th className="p-2">거래</th><th className="p-2">순손익</th><th className="p-2">최대 낙폭</th><th className="p-2">기대값</th></tr></thead><tbody>{result.walkForward.map((item, index) => <tr key={`${item.startTime}-${item.endTime}`} className="border-t border-border"><td className="p-2 whitespace-nowrap">{index + 1} · {new Date(item.startTime).toISOString().slice(0, 10)}~{new Date(item.endTime).toISOString().slice(0, 10)}</td><td className="p-2">{item.totalTrades}</td><td className="p-2">{money(item.netPnl)}</td><td className="p-2">{money(item.maximumDrawdown)}</td><td className="p-2">{money(item.expectancy)}</td></tr>)}</tbody></table></div>
        </section>
        ...
        <section className="rounded-2xl border border-border bg-card p-4" data-testid="breakdown-results">
          ...
          <div className="grid gap-3 md:grid-cols-2"><div className="overflow-x-auto rounded-xl border border-border"><table className="w-full min-w-[360px] text-left text-xs"><thead className="bg-muted"><tr><th className="p-2">월</th><th className="p-2">거래</th><th className="p-2">순손익</th><th className="p-2">수익률</th></tr></thead><tbody>{result.monthlyPerformance.map((item) => <tr key={item.month} className="border-t border-border"><td className="p-2">{item.month}</td><td className="p-2">{item.trades}</td><td className="p-2">{money(item.netPnl)}</td><td className="p-2">{percent(item.returnPercent)}</td></tr>)}</tbody></table></div><div className="overflow-x-auto rounded-xl border border-border"><table className="w-full min-w-[360px] text-left text-xs"><thead className="bg-muted"><tr><th className="p-2">시장 상태</th><th className="p-2">거래</th><th className="p-2">순손익</th><th className="p-2">승률</th></tr></thead><tbody>{result.regimePerformance.map((item) => <tr key={item.regime} className="border-t border-border"><td className="p-2">{item.regime}</td><td className="p-2">{item.trades}</td><td className="p-2">{money(item.netPnl)}</td><td className="p-2">{percent(item.winRate)}</td></tr>)}</tbody></table></div></div>
        </section>
