...
const CHART_FRAMES_KEY = 'sa-chart-frames-v1';

const DEFAULT_VISIBLE: Timeframe[] = ['1D', '1W', '1M'];

function loadVisible(): Timeframe[] {
  if (typeof window === 'undefined') return DEFAULT_VISIBLE;

  try {
    const raw = window.localStorage.getItem(CHART_FRAMES_KEY);
    if (!raw) return DEFAULT_VISIBLE;

    const parsed = JSON.parse(raw) as unknown;
    if (!Array.isArray(parsed)) return DEFAULT_VISIBLE;

    const valid = parsed.filter((item): item is Timeframe =>
      TIMEFRAMES.some((frame) => frame.tf === item),
    );
...
function saveVisible(frames: Timeframe[]) {
  ...
  try {
    window.localStorage.setItem(CHART_FRAMES_KEY, JSON.stringify(frames));
  } catch {
    // ignore
  }
}
