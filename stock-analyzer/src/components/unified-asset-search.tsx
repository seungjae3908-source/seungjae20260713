    ...
    <div ref={rootRef} className="relative w-full">
      ...
      {open && (
        <div ref={popupRef} id="unified-asset-search-listbox" role="listbox" aria-label="통합 자산 자동완성 결과" style={popupStyle} className="z-[120] overflow-y-auto overscroll-contain rounded-2xl border border-card-border bg-card shadow-2xl">
          ...
          {!loading && !error && trimmed && response?.partial && <div className="border-b border-warning/30 bg-warning/10 px-4 py-2 text-xs font-bold text-warning">일부 공급자 목록이 지연되어 마지막 정상 인덱스를 함께 표시합니다.</div>}
          {!loading && !error && trimmed && response?.stale && <div className="border-b border-warning/30 bg-warning/10 px-4 py-2 text-xs font-bold text-warning">검색 인덱스 기준시각이 오래되었습니다: {response.dataAsOf ? new Date(response.dataAsOf).toLocaleString('ko-KR') : '확인 필요'}</div>}
          ...
          {!loading && !error && trimmed && response && response.results.length === 0 && (
            <div className="space-y-2 px-4 py-6 text-center">
              ...
              {response.hiddenMatches.length > 0 ? <p className="text-xs font-bold text-muted-foreground">다른 시장에서 찾음: {response.hiddenMatches.map((item) => `${GROUP_LABEL[item.market]} ${item.count}개`).join(', ')}</p> : response.partial ? <p className="text-xs font-bold text-warning">일부 공급자 목록 갱신이 지연되어 신규 상장 자산이 아직 반영되지 않았을 수 있습니다.</p> : <p className="text-xs font-bold text-muted-foreground">이름·티커·종목코드·심볼을 다시 확인해 주세요.</p>}
            </div>
          )}
        </div>
      )}
