  ...
  try {
    ...
    return {
      ...
      investmentPerTrade: Math.max(
        1,
        Math.round(safeNumber(parsed.investmentPerTrade, 100_000)),
      ),
      minProbability: clamp(
        Math.round(safeNumber(parsed.minProbability, 70)),
        1,
        99,
      ),
      stopLossPercent: clamp(
        safeNumber(parsed.stopLossPercent, 5),
        0.1,
        50,
      ),
      takeProfitPercent: clamp(
        safeNumber(parsed.takeProfitPercent, 10),
        0.1,
        200,
      ),
...
export function saveAutoTradeSettings(
  ...
  const normalized: AutoTradeSettings = {
    ...
    investmentPerTrade: Math.max(1, Math.round(settings.investmentPerTrade)),
    minProbability: clamp(Math.round(settings.minProbability), 1, 99),
    stopLossPercent: clamp(settings.stopLossPercent, 0.1, 50),
    takeProfitPercent: clamp(settings.takeProfitPercent, 0.1, 200),
