...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    void Promise.race([
      ...
      new Promise<never>((_, reject) =>
        window.setTimeout(() => reject(new Error('AUTH_SESSION_TIMEOUT')), 10_000),
      ),
    ]).then(async ({ data }) => {
      if (!mounted) return;
  ...
  const value = useMemo<AuthContextValue>(() => ({
    ...
    isFullMember: isApproved && (membershipRole === 'full' || membershipRole === 'admin'),
    isAdmin: isApproved && membershipRole === 'admin',
    isApproved,
