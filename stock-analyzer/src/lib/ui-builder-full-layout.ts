...
const REQUIRED_BLOCKS: Partial<Record<UiBuilderPageId, UiBuilderBlockType[]>> = {
  HOME: ['PageHeader'],
  ASSET_SEARCH: ['PageHeader'],
  STOCK_MARKET: ['PageHeader'],
  CRYPTO_MARKET: ['PageHeader'],
  ASSET_DETAIL: ['PageHeader'],
  SIGNAL_SCANNER: ['PageHeader'],
  AI_CHART: ['PageHeader'],
  POSITION: ['PageHeader'],
  PORTFOLIO: ['PageHeader'],
  AUTO_TRADING: ['PageHeader', 'EmergencyStop'],
  AI_CHAT: ['PageHeader'],
  NEWS_INFORMATION: ['PageHeader'],
  SETTINGS: ['PageHeader'],
  ACCOUNT_CONNECTION: ['PageHeader'],
};

const TEMPLATE_BLOCKS: Record<UiBuilderPageId, { mobile: UiBuilderBlockType[]; desktop: UiBuilderBlockType[] }> = {
  HOME: {
    mobile: ['PageHeader', 'UnifiedSearch', 'MarketSummary', 'TopSignals', 'Watchlist', 'PortfolioSummary'],
    desktop: ['PageHeader', 'UnifiedSearch', 'MarketSummary', 'TopSignals', 'Watchlist', 'PortfolioSummary'],
  },
  ASSET_SEARCH: {
    mobile: ['PageHeader', 'UnifiedSearch', 'AssetSearchResults', 'Watchlist'],
    desktop: ['PageHeader', 'UnifiedSearch', 'AssetSearchResults', 'Watchlist', 'MarketRankings'],
  },
  STOCK_MARKET: {
    mobile: ['PageHeader', 'MarketSummary', 'MarketRankings', 'TopSignals', 'NewsSummary', 'ThemeSummary'],
    desktop: ['PageHeader', 'MarketSummary', 'MarketRankings', 'TopSignals', 'NewsSummary', 'ThemeSummary'],
  },
  CRYPTO_MARKET: {
    mobile: ['PageHeader', 'MarketSummary', 'MarketRankings', 'TopSignals', 'AiOpinion', 'TechnicalSummary'],
    desktop: ['PageHeader', 'MarketSummary', 'MarketRankings', 'TopSignals', 'AiOpinion', 'TechnicalSummary'],
  },
  ASSET_DETAIL: {
    mobile: ['PageHeader', 'PositionSummary', 'AiOpinion', 'AiChart', 'PricePlan', 'AiReason', 'NewsSummary', 'DisclosureSummary', 'FinancialSummary', 'TechnicalSummary', 'TradeReviewButton'],
    desktop: ['PageHeader', 'PositionSummary', 'AiOpinion', 'AiChart', 'PricePlan', 'AiReason', 'NewsSummary', 'DisclosureSummary', 'FinancialSummary', 'TechnicalSummary', 'TradeReviewButton'],
  },
  SIGNAL_SCANNER: {
    mobile: ['PageHeader', 'MarketSelector', 'StrategySelector', 'TimeframeSelector', 'DirectionSelector', 'SignalSummary', 'SignalList', 'PositionSummary', 'TradeReviewButton'],
    desktop: ['PageHeader', 'MarketSelector', 'StrategySelector', 'TimeframeSelector', 'DirectionSelector', 'SignalList', 'AiChart', 'AiOpinion', 'PositionSummary', 'PricePlan', 'TradeReviewButton', 'SignalSummary'],
  },
  AI_CHART: {
    mobile: ['PageHeader', 'UnifiedSearch', 'AiChart', 'AiOpinion', 'AiReason', 'PricePlan', 'PositionSummary', 'TechnicalSummary', 'TradeReviewButton'],
    desktop: ['PageHeader', 'UnifiedSearch', 'AiChart', 'AiOpinion', 'AiReason', 'PricePlan', 'PositionSummary', 'TechnicalSummary', 'TradeReviewButton'],
  },
  POSITION: {
    mobile: ['PageHeader', 'PositionSummary', 'PositionDetail', 'PositionOpinion', 'RiskSummary', 'AiChart', 'TradeReviewButton'],
    desktop: ['PageHeader', 'PositionSummary', 'PositionDetail', 'PositionOpinion', 'RiskSummary', 'AiChart', 'TradeReviewButton'],
  },
  PORTFOLIO: {
    mobile: ['PageHeader', 'PortfolioSummary', 'PnLSummary', 'RiskSummary', 'PortfolioPositions', 'AiOpinion'],
    desktop: ['PageHeader', 'PortfolioSummary', 'PnLSummary', 'RiskSummary', 'PortfolioPositions', 'AiOpinion'],
  },
  AUTO_TRADING: {
    mobile: ['PageHeader', 'AutoTradingStatus', 'AutoTradingStrategy', 'AutoTradingRisk', 'AutomationControl', 'EmergencyStop'],
    desktop: ['PageHeader', 'AutoTradingStatus', 'AutoTradingStrategy', 'AutoTradingRisk', 'AutomationControl', 'EmergencyStop'],
  },
  AI_CHAT: {
    mobile: ['PageHeader', 'AiChat', 'AiOpinion', 'AiReason', 'PositionSummary'],
    desktop: ['PageHeader', 'AiChat', 'AiOpinion', 'AiReason', 'PositionSummary'],
  },
  NEWS_INFORMATION: {
    mobile: ['PageHeader', 'NewsSummary', 'DisclosureSummary', 'MarketSummary', 'ThemeSummary'],
    desktop: ['PageHeader', 'NewsSummary', 'DisclosureSummary', 'MarketSummary', 'ThemeSummary'],
  },
  SETTINGS: {
    mobile: ['PageHeader', 'SettingsNavigation', 'SettingCategory', 'SettingCategory', 'SettingCategory', 'SettingCategory'],
    desktop: ['PageHeader', 'SettingsNavigation', 'SettingCategory', 'SettingCategory', 'SettingCategory', 'SettingCategory'],
  },
  ACCOUNT_CONNECTION: {
    mobile: ['PageHeader', 'AccountConnectionStatus', 'ProviderStatus', 'SystemStatus', 'AdvancedInfo'],
    desktop: ['PageHeader', 'AccountConnectionStatus', 'ProviderStatus', 'SystemStatus', 'AdvancedInfo'],
  },
};

export const UI_BUILDER_PAGE_RUNTIME_OWNER: Record<UiBuilderPageId, string> = {
  HOME: 'HomePage',
  ASSET_SEARCH: 'UnifiedAssetSearchPage + UnifiedAssetSearch',
  STOCK_MARKET: 'MarketInformationPage / UsStocksPage',
  CRYPTO_MARKET: 'MarketInformationPage / CoinHomePage',
  ASSET_DETAIL: 'StockInfoPage + AiChartPage',
  SIGNAL_SCANNER: 'TechnicalWorkspacePage + SignalScannerPage + AiChartPage',
  AI_CHART: 'AiChartPage + UnifiedAnalysisChart',
  POSITION: 'PortfolioPage + AiChartPage',
  PORTFOLIO: 'PortfolioPage',
  AUTO_TRADING: 'AutoTradingPage + TradeAutomationSettings + TradingRiskPreviewPanel',
  AI_CHAT: 'AiChatPage',
  NEWS_INFORMATION: 'NewsPage + StockInfoPage tabs + ThemesPage',
  SETTINGS: 'MorePage',
  ACCOUNT_CONNECTION: 'AccountPage + BrokerageAccountConnections',
};
...
function visible(block: UiBuilderFullBlock, device: UiBuilderDeviceClass) {
  return !block.visibility.hidden && block.visibility.mode !== 'hidden' && (block.visibility.mode === 'both' || block.visibility.mode === device);
}

function rejectUnknownKeys(record: Record<string, unknown>, allowed: Set<string>, issues: UiBuilderFullValidationIssue[], code: string, blockId?: string) {
  for (const key of Object.keys(record)) if (!allowed.has(key)) issues.push({ code, message: `허용되지 않은 속성: ${key}`, blockId });
}
...
export function validateUiBuilderFullLayout(candidate: unknown, expectedPage: UiBuilderPageId, expectedDevice: UiBuilderDeviceClass): UiBuilderFullValidationResult {
  ...
  const visibleTypes = new Set<UiBuilderBlockType>();
  for (const raw of doc.blocks) {
    ...
    else if (ids.has(id)) issues.push({ code: 'DUPLICATE_BLOCK_ID', message: `중복 block id: ${id}`, blockId: id });
    else ids.add(id);
    ...
    if (!isRecord(raw.props)) issues.push({ code: 'INVALID_PROPS', message: 'props object가 필요합니다.', blockId: id });
      ...
      rejectUnknownKeys(raw.props, PROP_KEYS, issues, 'FORBIDDEN_RUNTIME_PROP', id);
      validateText(raw.props.title, 'title', issues, id);
    ...
    if (!isRecord(raw.layout)) issues.push({ code: 'INVALID_BLOCK_LAYOUT', message: 'layout object가 필요합니다.', blockId: id });
      ...
      const order = Number(raw.layout.order);
      const colSpan = Number(raw.layout.colSpan);
      if (!Number.isInteger(order) || order < 0) issues.push({ code: 'INVALID_ORDER', message: 'order는 0 이상의 정수여야 합니다.', blockId: id });
      else if (orders.has(order)) issues.push({ code: 'DUPLICATE_ORDER', message: `중복 order: ${order}`, blockId: id });
  ...
  for (const required of REQUIRED_BLOCKS[expectedPage] ?? []) {
    if (!visibleTypes.has(required)) issues.push({ code: 'REQUIRED_BLOCK_MISSING', message: `${expectedPage}에서 ${required}는 숨기거나 삭제할 수 없습니다.` });
  }
  return { valid: issues.length === 0, issues };
}

export function uiBuilderLayoutStorageKey(status: UiBuilderFullLayoutStatus, pageId: UiBuilderPageId, device: UiBuilderDeviceClass) {
  return `stock-ui-builder:${status}-layout:${pageId}:${device}`;
}

export function parseAndValidateUiBuilderLayout(raw: string, pageId: UiBuilderPageId, device: UiBuilderDeviceClass) {
  let candidate: unknown;
  try { candidate = JSON.parse(raw); }
  catch { return { valid: false, layout: null, issues: [{ code: 'INVALID_JSON', message: 'Layout JSON을 파싱할 수 없습니다.' }] } as const; }
  const validation = validateUiBuilderFullLayout(candidate, pageId, device);
  return validation.valid
    ? { valid: true, layout: candidate as UiBuilderFullLayoutDocument, issues: [] as UiBuilderFullValidationIssue[] }
    : { valid: false, layout: null, issues: validation.issues };
}

export function readUiBuilderStoredLayout(status: UiBuilderFullLayoutStatus, pageId: UiBuilderPageId, device: UiBuilderDeviceClass): string | null {
  if (typeof window === 'undefined') return null;
  return window.localStorage.getItem(uiBuilderLayoutStorageKey(status, pageId, device));
}
...
export function loadActiveUiBuilderLayout(pageId: UiBuilderPageId, device: UiBuilderDeviceClass): { source: 'active' | 'fallback'; layout: UiBuilderFullLayoutDocument; issues: UiBuilderFullValidationIssue[] } {
  ...
  const raw = readUiBuilderStoredLayout('active', pageId, device);
  if (!raw) return { source: 'fallback', layout: fallback, issues: [] };
  const parsed = parseAndValidateUiBuilderLayout(raw, pageId, device);
  return parsed.valid && parsed.layout
    ? { source: 'active', layout: structuredClone(parsed.layout), issues: [] }
    : { source: 'fallback', layout: fallback, issues: parsed.issues };
}
...
export function makeFrozenUiBuilderTemplate(pageId: UiBuilderPageId, device: UiBuilderDeviceClass): UiBuilderFullLayoutDocument {
  ...
  const specs = TEMPLATE_BLOCKS[pageId][device];
  return {
    schemaVersion: UI_BUILDER_SCHEMA_VERSION,
    layoutId: `builder-frozen-${pageId.toLowerCase()}-${device}`,
    pageId,
    deviceClass: device,
    version: 1,
    status: 'published',
    ...
    updatedAt: now,
    publishedAt: now,
    blocks: specs.map((type, order) => ({
      id: `${pageId.toLowerCase()}-${type.replace(/[A-Z]/g, (letter) => `-${letter.toLowerCase()}`).replace(/^-/, '')}-${order + 1}`,
      type,
      ...
      layout: { order, colSpan: device === 'mobile' ? 12 : type === 'PageHeader' ? 12 : 6, minHeight: type === 'AiChart' ? 420 : 88, sticky: false, bottomFixed: false },
      visibility: { mode: 'both', hidden: false },
      ...(actionFor(type) ? { actionId: actionFor(type) } : {}),
    })),
  };
}

export function uiBuilderTemplateCoverage() {
  return UI_BUILDER_PAGE_IDS.map((pageId) => ({
    pageId,
    owner: UI_BUILDER_PAGE_RUNTIME_OWNER[pageId],
    mobileBlocks: TEMPLATE_BLOCKS[pageId].mobile.length,
    desktopBlocks: TEMPLATE_BLOCKS[pageId].desktop.length,
  }));
}
