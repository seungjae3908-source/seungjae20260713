...
async function fetchChartCandles(
  ...
  const apiFrame = timeframe === "1H" ? "60m" : timeframe;
        ...
        <div className="mt-2 grid grid-cols-6 gap-1">
          {TABS.map((item) => (
            <button
              ...
              className={cn(
                "min-w-0 rounded-xl border px-0.5 py-2 text-[9px] font-extrabold",
...
function ProfessionalChart({
  ...
  const rows = useMemo(() => buildChartRows(candles).slice(-500), [candles]);
  const enabledPanels = (
    [
      ["rsi", indicators.rsi],
...
function PriceChartCanvas({
  ...
  }, [rows, timeframe, indicators, height]);
  ...
  return (
    <div className="space-y-3">
      ...
      <SectionCard
        ...
        subtitle={
          market === "KR"
            ? "최근 공시를 날짜순으로 보여주며 금융감독원 DART 원문으로 연결합니다."
            : "최근 공시를 날짜순으로 보여주며 SEC EDGAR 원문으로 연결합니다."
        }
      >
        {filings.length ? (
          <div className="space-y-2">
            {filings.slice(0, 50).map((item, index) => {
              const title = cleanContentTitle(
