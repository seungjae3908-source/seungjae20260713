...
async function fetchDetail(ticker: string, advanced: boolean): Promise<DetailData> {
...
export default function DetailPage() {
  ...
  const detail = useQuery<DetailData>({
    queryKey: ["stock-detail-v14", ticker, permissions.canUseAdvancedAnalysis],

    queryFn: () => fetchDetail(ticker, permissions.canUseAdvancedAnalysis),
...
function ChartTab({
  ...
  return (
    <div className="space-y-3">
      ...
      {!basicOnly && (
        <>
                <SectionCard title="AI 차트 실시간 생중계" defaultOpen>
                  <button
                    type="button"
                    onClick={() => setExplanation({
                      title: "AI 차트 실시간 생중계",
                      text: `${TIMEFRAMES.find((item) => item.key === timeframe)?.label ?? timeframe} 기준으로 최신 봉이 바뀔 때 추세·RSI·MACD·거래량을 다시 계산합니다.`,
                      focus: technicalSignalFocus("AI 차트 실시간 생중계", "선택 봉의 최신 데이터 기준 분석입니다."),
                    })}
                    className="w-full"
                  >
                    <div className="grid grid-cols-3 gap-2">
                    ...
                    <p className="mt-3 text-left text-xs font-bold leading-5 text-muted-foreground">{TIMEFRAMES.find((item) => item.key === timeframe)?.label ?? timeframe} · {stats.trend} · RSI {stats.rsi?.toFixed(1) ?? "데이터 없음"} · 거래량 {stats.volumeRatio?.toFixed(1) ?? "-"}배</p>
                  </button>
                </SectionCard>
