...
type ClassificationLabel = "우량주" | "중립" | "저평가주" | "잡주";
...
function classifyRow(row: AnyObj, score: number | null): ClassificationLabel {
	...
	if (
		reason.includes("저평가") ||
		reason.includes("저PER") ||
		reason.includes("저PBR")
	) {
		return "저평가주";
	}
...
export default function SearchPage() {
	...
	return (
		<div className="flex h-full min-h-0 flex-col overflow-hidden bg-background">
			<header className="sticky top-0 z-30 shrink-0 border-b border-card-border bg-background/95 px-4 pb-3 pt-5 backdrop-blur">
										...
										<div className="flex min-w-0 items-center gap-2">
											...
											<span
												className={cn(
													...
													stockClassBadgeClass(row.classification === "중립" ? "보통주" : row.classification),
												)}
											>
												{row.classification}
											</span>
