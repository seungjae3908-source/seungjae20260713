...
export default function PortfolioV2Page() {
  ...
  const tab = portfolioV2Tab(location);

  return <div className="flex h-full min-h-0 flex-col bg-background"><div className="shrink-0 border-b border-border bg-background px-3 pb-2 pt-3"><div className="mx-auto w-full max-w-6xl">{tab === 'intelligence' ? <div className="mb-3"><h1 className="text-xl font-black">내 포트폴리오</h1><p className="mt-1 text-xs font-bold text-muted-foreground">인텔리전스, 기존 보유자산, 통합 매매일지를 한 곳에서 확인합니다.</p></div> : null}<div className="grid grid-cols-3 gap-1 rounded-2xl bg-muted p-1" aria-label="포트폴리오 V2 보기"><button type="button" aria-pressed={tab === 'intelligence'} onClick={() => navigate('/portfolio')} className={cn('rounded-xl px-2 py-2.5 text-xs font-black sm:text-sm', tab === 'intelligence' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground')}>인텔리전스</button><button type="button" aria-pressed={tab === 'holdings'} onClick={() => navigate('/portfolio?tab=holdings')} className={cn('rounded-xl px-2 py-2.5 text-xs font-black sm:text-sm', tab === 'holdings' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground')}>보유자산</button><button type="button" aria-pressed={tab === 'journal'} onClick={() => navigate('/portfolio?tab=journal')} className={cn('rounded-xl px-2 py-2.5 text-xs font-black sm:text-sm', tab === 'journal' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground')}>매매일지</button></div></div></div><div className="min-h-0 flex-1">{tab === 'intelligence' ? <IntelligenceDashboard /> : <PortfolioPage />}</div></div>;
}
