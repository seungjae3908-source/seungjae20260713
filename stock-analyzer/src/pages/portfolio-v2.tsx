...
export default function PortfolioV2Page() {
  ...
  return <div className="flex h-full min-h-0 flex-col bg-background"><div className="shrink-0 border-b border-border bg-background px-3 pb-2 pt-3"><div className="mx-auto w-full max-w-6xl">{tab === 'intelligence' ? <div className="mb-3"><h1 className="text-xl font-black">내 포트폴리오</h1><p className="mt-1 text-xs font-bold text-muted-foreground">인텔리전스, AI 진단, 기존 보유자산, 통합 매매일지를 한 곳에서 확인합니다.</p></div> : null}<div className={cn('grid gap-1 rounded-2xl bg-muted p-1', tab === 'journal' ? 'grid-cols-2' : 'grid-cols-3')} aria-label="포트폴리오 V2 보기"><button type="button" aria-pressed={tab === 'intelligence'} onClick={() => selectTab('intelligence')} className={cn('rounded-xl px-2 py-2.5 text-xs font-black sm:text-sm', tab === 'intelligence' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground')}>인텔리전스</button><button type="button" aria-pressed={tab === 'holdings'} onClick={() => selectTab('holdings')} className={cn('rounded-xl px-2 py-2.5 text-xs font-black sm:text-sm', tab === 'holdings' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground')}>보유자산</button>{tab !== 'journal' ? <button type="button" aria-pressed={false} onClick={() => selectTab('journal')} className="rounded-xl px-2 py-2.5 text-xs font-black text-muted-foreground sm:text-sm">매매일지</button> : null}</div></div></div><div className="min-h-0 flex-1">{tab === 'intelligence' ? <IntelligenceDashboard /> : <PortfolioPage />}</div></div>;
}