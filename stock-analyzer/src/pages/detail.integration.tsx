...
import { Bell, ChevronDown, Maximize2, Minimize2, Settings2 } from "lucide-react";
import {
  ColorType,
  CrosshairMode,
  LineStyle,
...
async function fetchCoreDetail(ticker: string): Promise<DetailData> {
  const upper = ticker.toUpperCase();

  // 첫 화면에 꼭 필요한 데이터만 먼저 가져옵니다.
  const [quoteRaw, companyRaw, candlesRaw] = await Promise.all([
    tryJson<AnyObj>(
    ...
    tryJson<AnyObj>([`/api/stocks/${upper}/candles?tf=1D`], {}),
  ]);
...
async function fetchAdvancedDetail(ticker: string): Promise<Pick<DetailData, "financials" | "risk" | "filings" | "news">> {
  const upper = ticker.toUpperCase();

  // 재무·위험·공시·뉴스는 차트가 먼저 열린 뒤 백그라운드에서 불러옵니다.
...
function financialRows(financials: AnyObj | null, period: FinancialPeriod): AnyObj[] {
  let rows: AnyObj[] = [];
  if (Array.isArray(financials?.[period])) rows = financials[period];
  else if (period === "annual" && Array.isArray(financials?.yearly)) rows = financials.yearly;
  else if (period === "quarterly" && Array.isArray(financials?.quarters)) rows = financials.quarters;
  ...
  for (const raw of rows) {
    ...
    const hasValue = [row.revenue,row.sales,row.operatingIncome,row.netIncome,row.assets,row.totalAssets,row.liabilities,row.totalLiabilities,row.equity,row.capitalStock,row.operatingCashFlow].some((value) => financialValue(value) != null);
    if (!hasValue) continue;
    if (!unique.has(label)) unique.set(label, row);
  }
  return [...unique.values()].sort((a, b) => String(b.period).localeCompare(String(a.period), "ko", { numeric: true }));
}
...
export default function DetailPage() {
  ...
  const detail = useQuery<DetailData>({
    queryKey: ["stock-detail-core-v1", ticker],
    queryFn: () => fetchCoreDetail(ticker),
  ...
  const advancedDetail = useQuery<Pick<DetailData, "financials" | "risk" | "filings" | "news">>({
    queryKey: ["stock-detail-advanced-v1", ticker],
    queryFn: () => fetchAdvancedDetail(ticker),
    enabled: Boolean(ticker && detail.data && permissions.canUseAdvancedAnalysis),
    staleTime: 5 * 60_000,
    gcTime: 30 * 60_000,
  ...
  const data = useMemo<DetailData | undefined>(() => {
    ...
    if (!advancedDetail.data) return detail.data;

    return {
      ...detail.data,
      financials: advancedDetail.data.financials,
      risk: advancedDetail.data.risk,
      filings: advancedDetail.data.filings,
      news: advancedDetail.data.news,
    };
  }, [detail.data, advancedDetail.data]);
  ...
  return (
    ...
    >
      ...
      {alertOpen && (
        <Modal title={`${companyName} 알림`} onClose={() => setAlertOpen(false)}>
          <PriceAlertCard assetType="stock" market={market} symbol={ticker} currentPrice={currentPrice} currency={currency} />
          {permissions.canUseAdvancedAnalysis && (
            <div className="mt-3 grid grid-cols-2 gap-2">
              <button type="button" onClick={() => { setAlertOpen(false); setTab("news"); }} className="rounded-xl border border-card-border bg-secondary px-3 py-2 text-xs font-extrabold">관련 뉴스</button>
              <button type="button" onClick={() => { setAlertOpen(false); setTab("filings"); }} className="rounded-xl border border-card-border bg-secondary px-3 py-2 text-xs font-extrabold">관련 공시</button>
            </div>
          )}
        </Modal>
      )}
...
function inferCompanyBusiness(name: string, ticker: string, company: AnyObj | null) {
  const direct = firstText(company?.industry, company?.sector, company?.business, company?.businessType, company?.category);
  const text = `${name} ${ticker} ${direct ?? ""}`.toLowerCase();
  if (/삼성전자|하이닉스|nvidia|amd|intel|broadcom|micron|반도체/.test(text)) return "반도체·전자 기업으로 메모리, 시스템반도체, AI 연산칩 또는 관련 장비·부품 사업의 실적과 업황 영향을 크게 받습니다.";
  if (/바이오|제약|pharma|therapeutics|셀트리온|삼성바이오|lilly|pfizer/.test(text)) return "바이오·제약 기업으로 신약, 바이오의약품, 임상, 허가, 생산계약과 연구개발 성과가 기업가치에 큰 영향을 줍니다.";
  if (/현대차|기아|tesla|motor|rivian|자동차/.test(text)) return "자동차·모빌리티 기업으로 완성차 판매, 전기차, 배터리 원가, 환율과 글로벌 수요에 따라 실적이 움직입니다.";
  if (/은행|증권|금융|bank|jpmorgan|visa/.test(text)) return "금융 기업으로 금리, 대출·예금, 수수료, 자산건전성 또는 결제 거래량이 핵심 실적 변수입니다.";
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <SectionCard title="AI 간단요약" subtitle="현재 데이터 기준 참고용 분석">
        ...
        <div className="grid grid-cols-3 gap-2">
          <SignalBox label="호재" text={positiveSummary(insights)} positive compact onClick={() => setDetailModal({ title: "호재 근거", text: [...(insights.newsDisclosureSummary ?? []), ...(insights.financialSummary ?? []), ...(insights.chartSummary ?? [])].filter((row) => /상승|증가|개선|성장|호재|긍정|강세|흑자|계약|수주|돌파|우수/.test(row)).join(" ") || "현재 확인된 뚜렷한 호재가 없습니다." })} />

          <SignalBox label="악재" text={negativeSummary(insights)} compact onClick={() => setDetailModal({ title: "악재 근거", text: [...(insights.riskSummary ?? []), ...(insights.newsDisclosureSummary ?? []), ...(insights.financialSummary ?? [])].filter((row) => /하락|감소|악화|부진|악재|부정|약세|적자|위험|과열|부채|주의/.test(row)).join(" ") || "현재 확인된 뚜렷한 악재가 없습니다." })} />
          <SignalBox
            label="리스크"
            text={insights.riskSummary?.[0] ?? metrics.riskCaption}
            compact
            onClick={() => setDetailModal({ title: "리스크 설명", text: (insights.riskSummary ?? []).join(" ") || metrics.riskCaption || "현재 확인된 별도 리스크가 없습니다." })}
          />
        </div>
        ...
        <div className="space-y-1.5">
          {riskRows.map((row) => (
            <button
              key={row.name}
              type="button"
              onClick={() => setDetailModal({ title: `${row.name} 설명`, text: row.reason || "설명 데이터가 없습니다." })}
      ...
      {detailModal && <Modal title={detailModal.title} onClose={() => setDetailModal(null)}><p>{detailModal.text}</p></Modal>}
    </div>
  );
}
...
function AiTab({
  ...
  const [analysisModal, setAnalysisModal] = useState<{ title: string; text: string } | null>(null);
  ...
  return (
    <div className="space-y-3">
      ...
      >
        <button type="button" onClick={() => setAnalysisModal({ title: "차트 분석", text: (insights.chartSummary ?? []).join(" ") || "차트 분석 데이터가 부족합니다." })} className="w-full text-left"><SummaryItems items={insights.chartSummary} /></button>
      </CollapsibleSection>
      ...
      >
        <button type="button" onClick={() => setAnalysisModal({ title: "재무 분석", text: (insights.financialSummary ?? []).join(" ") || "재무 분석 데이터가 부족합니다." })} className="w-full text-left"><SummaryItems items={insights.financialSummary} /></button>
      </CollapsibleSection>
      ...
      >
        <div className="grid grid-cols-2 gap-2">
          <SignalBox label="호재" text={positiveSummary(insights)} positive compact onClick={() => setAnalysisModal({ title: "최근 호재", text: (insights.newsDisclosureSummary ?? []).filter((row) => /상승|증가|개선|성장|호재|긍정|강세|흑자|계약|수주|돌파|우수/.test(row)).join(" ") || "최근 확인된 뚜렷한 호재가 없습니다." })} />

          <SignalBox label="악재" text={negativeSummary(insights)} compact onClick={() => setAnalysisModal({ title: "최근 악재", text: [...(insights.riskSummary ?? []), ...(insights.newsDisclosureSummary ?? [])].filter((row) => /하락|감소|악화|부진|악재|부정|약세|적자|위험|과열|부채|주의/.test(row)).join(" ") || "최근 확인된 뚜렷한 악재가 없습니다." })} />
        </div>
      </CollapsibleSection>
      ...
      {analysisModal && <Modal title={analysisModal.title} onClose={() => setAnalysisModal(null)}><p>{analysisModal.text}</p></Modal>}
...
function technicalSignalFocus(title: string, text: string): StudyChartFocus {
  ...
  const preferredIndicator = lower.includes("rsi") ? "rsi" : lower.includes("macd") ? "macd" : lower.includes("볼린저") ? "bollinger" : lower.includes("거래량") || lower.includes("obv") ? "volume" : lower.includes("이동평균") || lower.includes("고점") ? "moving-average" : undefined;
  const markerStrategy: StudyMarkerStrategy = lower.includes("거래량") ? "highest-volume" : lower.includes("고점") || lower.includes("돌파") ? "breakout" : lower.includes("과매도") || lower.includes("저점") ? "recent-low" : "latest";
  return { id: `signal-${title}`, title, summary: text, markerText: `${title} 확인`, markerStrategy, preferredIndicator };
}
...
function ChartTab({
  ...
  const chartQuery = useQuery<CandlePoint[]>({
    queryKey: ["detail-chart-v10-fast", ticker, timeframe],

    queryFn: () => fetchChartCandles(ticker, timeframe, fallbackRows),

    enabled: Boolean(ticker),

    // 시간봉을 바꿀 때 기존 차트를 지우지 않고 즉시 유지합니다.
    placeholderData: (previousData) => previousData,

    // 짧은 주기만 자주 확인하고, 긴 주기는 캐시를 적극 재사용합니다.
    staleTime: timeframe.endsWith("m") ? 15_000 : 120_000,
  ...
  const liveStrategy = useMemo(() => {
    const price = currentPrice ?? latest?.close ?? null;
    if (price == null || !Number.isFinite(price) || price <= 0) {
      return null;
    }

    const atrBase = Math.max(
      Math.abs(price * 0.012),
      Math.abs((recentHigh ?? price) - (recentLow ?? price)) / Math.max(4, recent.length || 1),
    );
    const bullish = stats.trend === "상승 우위";
    const bearish = stats.trend === "하락 우위";
    const entryLow = bullish ? price - atrBase * 0.35 : price - atrBase * 0.15;
    const entryHigh = bullish ? price + atrBase * 0.15 : price + atrBase * 0.35;
    const stop = bullish ? price - atrBase * 1.35 : price - atrBase * 1.05;
    const target1 = bullish ? price + atrBase * 1.6 : price + atrBase * 1.1;
    const target2 = bullish ? price + atrBase * 2.7 : price + atrBase * 1.8;
    const risk = Math.max(price - stop, Number.EPSILON);
    ...
    const confidence = Math.max(
      ...
      Math.min(
        ...
        Math.round(
          50 +
            (bullish ? 14 : bearish ? -8 : 0) +
            ((stats.rsi ?? 50) >= 45 && (stats.rsi ?? 50) <= 68 ? 10 : 0) +
            ((stats.volumeRatio ?? 1) >= 1.3 ? 8 : 0),
        ),
      ),
    );

    return {
      opinion: bullish ? "매수 후보" : bearish ? "매도·관망" : "관망",
      entryLow,
      entryHigh,
      stop,
      target1,
  ...
  }, [currentPrice, latest?.close, recentHigh, recentLow, recent.length, stats.trend, stats.rsi, stats.volumeRatio]);
  ...
  return (
    <div className="space-y-3">
      {!basicOnly && portfolioOverlay && (
        ...
        >
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="rounded-xl bg-secondary/70 p-3">
              <p className="text-[10px] font-bold text-muted-foreground">매수 후 최고가</p>
              <p className="mt-1 text-sm font-extrabold">{formatAppPrice(highestAfterPurchase, currency)}</p>
            </div>
            <div className="rounded-xl bg-secondary/70 p-3">
              <p className="text-[10px] font-bold text-muted-foreground">최고점 대비 현재</p>
              <p className={cn("mt-1 text-sm font-extrabold", (highToCurrentRate ?? 0) >= 0 ? "text-positive" : "text-destructive")}>{highToCurrentRate == null ? "확인 중" : `${highToCurrentRate >= 0 ? "+" : ""}${highToCurrentRate.toFixed(2)}%`}</p>
            </div>
          </div>
          <p className="mt-2 break-keep text-[11px] font-semibold leading-5 text-muted-foreground">차트의 ‘내 평단’과 ‘매수후 최고’ 점선으로 현재 손익과 고점 대비 낙폭을 확인할 수 있습니다.</p>
        </SectionCard>
      )}
      ...
      >
          ...
          actions={
            <div className="flex flex-wrap items-center justify-end gap-1.5">
              {!basicOnly && (
                <button
                  type="button"
                  onClick={() => document.getElementById("realtime-signal-analysis")?.scrollIntoView({ behavior: "smooth", block: "start" })}
                  className="inline-flex h-9 items-center gap-1.5 rounded-xl border border-card-border bg-secondary px-3 text-[11px] font-extrabold text-foreground transition active:scale-95"
                  aria-label="실시간 신호분석 보기"
                  title="실시간 신호분석"
                >
                  <span className="h-2 w-2 rounded-full bg-positive shadow-[0_0_0_3px_rgba(34,197,94,0.12)]" />
                  실시간 신호분석
                </button>
              )}
              ...
              {!basicOnly && (
                <button
                  type="button"
                  onClick={() => document.getElementById("signal-history-section")?.scrollIntoView({ behavior: "smooth", block: "start" })}
                  className="inline-flex h-9 items-center justify-center rounded-xl border border-card-border bg-secondary px-3 text-[11px] font-extrabold text-foreground transition active:scale-95"
                  aria-label="지난 내역 보기"
                  title="지난 내역"
                >
                  지난 내역
        ...
        >
          <div className="mb-2 flex items-center justify-between gap-2 px-1">
            ...
            <span className="text-[10px] font-bold text-muted-foreground">
              {chartQuery.isFetching && !chartQuery.isLoading ? "백그라운드 갱신 중" : `빠른 표시 · 최근 ${Math.min(candles.length, timeframe === "ALL" ? 1500 : /m|H/.test(timeframe) ? 600 : 1000)}봉`}
            </span>
          </div>
        ...
        {settingsOpen && (
        <Modal title="차트 환경설정" onClose={() => setSettingsOpen(false)}>
          <p className="mb-2 text-xs font-extrabold text-foreground">봉차트 ▾</p>
      ...
      {!basicOnly && (
        <>
                <div id="realtime-signal-analysis" className="scroll-mt-24">
                  <SectionCard title="실시간 신호분석" subtitle="AI 생중계와 신호 진행 과정을 한곳에서 표시합니다" defaultOpen>
                    <button
                      type="button"
                      onClick={() => setExplanation({
                        title: "실시간 신호분석",
                        text: `${TIMEFRAMES.find((item) => item.key === timeframe)?.label ?? timeframe} 기준으로 최신 봉이 바뀔 때 추세·RSI·MACD·거래량과 패턴 진행 상태를 다시 계산합니다.`,
                        focus: technicalSignalFocus("실시간 신호분석", "선택 봉의 최신 데이터 기준 AI 생중계입니다."),
                      })}
                      className="w-full"
                    >
                      <div className="mb-3 flex items-center justify-between rounded-xl border border-card-border bg-secondary/60 px-3 py-2">
                        <span className="inline-flex items-center gap-2 text-xs font-extrabold">
                          <span className="h-2.5 w-2.5 rounded-full bg-positive" />
                          AI 실시간 생중계 연결
                        </span>
                        <span className="text-[10px] font-bold text-muted-foreground">
                          {chartQuery.isFetching ? "분석 갱신 중" : "최신 봉 기준"}
                        </span>
                      </div>

                      <div className="grid grid-cols-3 gap-2">
                        <MiniMetric label="매수 점수" value={`${Math.max(0, Math.min(100, Math.round(50 + (stats.trend === "상승 우위" ? 18 : -8) + ((stats.rsi ?? 50) >= 50 && (stats.rsi ?? 50) <= 70 ? 12 : 0) + ((stats.volumeRatio ?? 1) >= 1.5 ? 10 : 0))))}점`} valueClassName="text-positive" />
                        <MiniMetric label="매도 점수" value={`${Math.max(0, Math.min(100, Math.round(50 + (stats.trend === "하락 우위" ? 18 : -8) + ((stats.rsi ?? 50) >= 75 ? 12 : 0) + ((stats.volumeRatio ?? 1) >= 1.5 && (latest?.close ?? 0) < (previous?.close ?? 0) ? 10 : 0))))}점`} valueClassName="text-destructive" />
                        <MiniMetric label="종합신호" value={stats.trend === "상승 우위" ? "매수" : stats.trend === "하락 우위" ? "매도" : "관망"} valueClassName={stats.trend === "상승 우위" ? "text-positive" : stats.trend === "하락 우위" ? "text-destructive" : "text-primary"} />
                      </div>

                      <div className="mt-3 space-y-1.5 rounded-xl bg-secondary/60 p-3 text-left text-xs font-semibold leading-5 text-muted-foreground">
                        <p>• {TIMEFRAMES.find((item) => item.key === timeframe)?.label ?? timeframe} 추세: {stats.trend}</p>
                        <p>• RSI {stats.rsi?.toFixed(1) ?? "데이터 없음"} · 거래량 {stats.volumeRatio?.toFixed(1) ?? "-"}배</p>
                        <p>• 최신 캔들과 선택한 지표·패턴 신호를 실시간으로 추적합니다.</p>
                      </div>
                    </button>
                  </SectionCard>
                </div>

                <SectionCard title="실시간 차트분석" subtitle="매수가·손절가·목표가와 현재 전략을 표시합니다" defaultOpen>
                  {liveStrategy ? (
                    <>
                      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                        <MiniMetric label="현재 판단" value={liveStrategy.opinion} valueClassName={liveStrategy.opinion === "매수 후보" ? "text-positive" : liveStrategy.opinion === "매도·관망" ? "text-destructive" : "text-primary"} />
                        <MiniMetric label="신뢰도" value={`${liveStrategy.confidence}%`} valueClassName="text-primary" />
                        <MiniMetric label="손익비" value={`1 : ${liveStrategy.rewardRisk.toFixed(2)}`} valueClassName="text-primary" />
                        <MiniMetric label="추천 매수가" value={`${formatAppPrice(liveStrategy.entryLow, currency)} ~ ${formatAppPrice(liveStrategy.entryHigh, currency)}`} valueClassName="text-foreground" />
                        <MiniMetric label="손절가" value={formatAppPrice(liveStrategy.stop, currency)} valueClassName="text-destructive" />
                        <MiniMetric label="목표가 1" value={formatAppPrice(liveStrategy.target1, currency)} valueClassName="text-positive" />
                        <MiniMetric label="목표가 2" value={formatAppPrice(liveStrategy.target2, currency)} valueClassName="text-positive" />
                        <MiniMetric label="분석 기준" value={TIMEFRAMES.find((item) => item.key === timeframe)?.label ?? timeframe} />
                      </div>
                      <p className="mt-3 break-keep text-[11px] font-semibold leading-5 text-muted-foreground">
                        이 값은 최신 캔들·변동성·추세·거래량을 이용한 실시간 참고 분석이며 실제 주문을 실행하지 않습니다.
                      </p>
                    </>
                  ) : (
                    <p className="text-sm font-semibold text-muted-foreground">전략 가격을 계산할 데이터가 부족합니다.</p>
                  )}
                </SectionCard>
      ...
      {!basicOnly && explanation && (
        <Modal title={explanation.title} subtitle="실제 차트에서 조건 위치 확인" onClose={() => setExplanation(null)}>
          ...
          <ProfessionalChart candles={candles} loading={chartQuery.isLoading} timeframe={timeframe} indicators={{ ...indicators, rsi: explanation.focus.preferredIndicator === "rsi" || indicators.rsi, macd: explanation.focus.preferredIndicator === "macd" || indicators.macd, bollinger: explanation.focus.preferredIndicator === "bollinger" || indicators.bollinger, volume: explanation.focus.preferredIndicator === "volume" || indicators.volume, sma20: explanation.focus.preferredIndicator === "moving-average" || indicators.sma20 }} fullscreen={false} height={360} portfolioOverlay={portfolioOverlay} autoSignal={autoSignal} studyFocus={explanation.focus} tradeEntries={tradeEntries} />
...
function MarketFlowPanel({ ticker }: { ticker: string }) {
  ...
  const [selectedFlowMetric, setSelectedFlowMetric] = useState<{ title: string; text: string } | null>(null);
    ...
    <SectionCard title="수급현황" subtitle={summaryLine} defaultOpen>
      ...
      <div className="mt-3">
        ...
        <div className="mt-3 grid grid-cols-2 gap-2">
          <FlowMetric
            label="거래량"
            value={totals.volume != null ? flowCompact(Number(totals.volume)) : "제공 불가"}
            onClick={() => setSelectedFlowMetric({ title: "거래량 설명", text: totals.volume != null ? `${periodLabel}에 체결된 실제 거래 수량의 합계입니다. 거래량 증가가 가격 방향과 함께 이어지는지 확인합니다.` : "현재 공급자 응답에 거래량 데이터가 없습니다." })}
          />
          <FlowMetric
            label="거래대금"
            value={totals.value != null || totals.tradeValue != null ? flowCompact(Number(totals.value ?? totals.tradeValue)) : "제공 불가"}
            onClick={() => setSelectedFlowMetric({ title: "거래대금 설명", text: totals.value != null || totals.tradeValue != null ? `${periodLabel} 거래량과 가격을 바탕으로 집계한 실제 거래대금입니다. 규모가 커질수록 시장 참여가 활발한지 확인할 수 있습니다.` : "현재 공급자 응답에 거래대금 데이터가 없습니다." })}
          />
        </div>
      ...
      {selectedFlowMetric && (
        <Modal title={selectedFlowMetric.title} onClose={() => setSelectedFlowMetric(null)}>
...
function ShortSellingPanel({ ticker }: { ticker: string }) {
  const [period, setPeriod] = useState<FlowPeriod>("daily");
  const [selectedShortMetric, setSelectedShortMetric] = useState<{ title: string; text: string } | null>(null);
  ...
  return (
    <SectionCard title="공매도현황" subtitle={summaryLine} defaultOpen>
      ...
      <div className="mt-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          <FlowMetric label="공매도 거래량" value={available ? metric(latest.shortVolume) : "제공 불가"} onClick={() => setSelectedShortMetric({ title: "공매도 거래량 설명", text: available ? "선택 기간에 공매도로 체결된 주식 수량입니다. 증가세가 이어지는지 가격 흐름과 함께 확인합니다." : "현재 공급자에서 공매도 거래량을 제공하지 않았습니다." })} />
          <FlowMetric label="공매도 거래대금" value={available ? metric(latest.shortValue) : "제공 불가"} onClick={() => setSelectedShortMetric({ title: "공매도 거래대금 설명", text: available ? "공매도 체결금액의 합계입니다. 거래량뿐 아니라 실제 금액 규모를 확인하는 항목입니다." : "현재 공급자에서 공매도 거래대금을 제공하지 않았습니다." })} />
          <FlowMetric label="공매도 비중" value={available ? metric(latest.ratio, "%") : "제공 불가"} onClick={() => setSelectedShortMetric({ title: "공매도 비중 설명", text: available ? `전체 거래 중 공매도가 차지한 비율은 ${metric(latest.ratio, "%")}입니다. 단독 수치보다 추세와 잔고를 함께 확인합니다.` : "현재 공급자에서 공매도 비중을 제공하지 않았습니다." })} />
          <FlowMetric label="공매도 잔고" value={available ? metric(latest.balance ?? latest.shortBalance) : "제공 불가"} onClick={() => setSelectedShortMetric({ title: "공매도 잔고 설명", text: available ? "아직 상환되지 않은 공매도 잔량입니다. 잔고가 크면 향후 환매수 압력이 생길 수 있지만 즉시 상승을 뜻하지는 않습니다." : "현재 공급자에서 공매도 잔고를 제공하지 않았습니다." })} />
        </div>

        <div className="mt-2 grid grid-cols-3 gap-2">
          <FlowMetric label="대차잔고" value={available ? metric(latest.loanBalance) : "제공 불가"} onClick={() => setSelectedShortMetric({ title: "대차잔고 설명", text: available ? "주식을 빌린 뒤 아직 상환하지 않은 수량입니다. 공매도 가능 물량과 관련되지만 모두 공매도로 사용되는 것은 아닙니다." : "현재 공급자에서 대차잔고를 제공하지 않았습니다." })} />
          <FlowMetric label="대차 변화량" value={available ? metric(latest.loanChange) : "제공 불가"} onClick={() => setSelectedShortMetric({ title: "대차 변화량 설명", text: available ? "직전 집계 대비 대차잔고 증감입니다. 증가하면 빌린 주식이 늘었고 감소하면 상환이 진행됐을 가능성이 있습니다." : "현재 공급자에서 대차 변화량을 제공하지 않았습니다." })} />
          <FlowMetric label="대차 이자율" value={available ? metric(latest.borrowRate, "%") : "제공 불가"} onClick={() => setSelectedShortMetric({ title: "대차 이자율 설명", text: available ? "주식을 빌리는 비용입니다. 이자율이 높으면 차입 수요 또는 물량 부족이 반영됐을 수 있습니다." : "현재 공급자에서 대차 이자율을 제공하지 않았습니다." })} />
        </div>

        <button type="button" onClick={() => setSelectedShortMetric({ title: "숏스퀴즈 가능성 설명", text: squeezeText })} className="mt-3 w-full rounded-xl bg-secondary/70 p-3 text-center">
          <div className="flex items-center justify-center gap-2">
      ...
      {selectedShortMetric && (
        <Modal title={selectedShortMetric.title} onClose={() => setSelectedShortMetric(null)}>
...
function FlowMetric({ label, value, onClick }: { label: string; value: string; onClick?: () => void }) {
  const className = "rounded-xl border border-card-border bg-secondary/50 p-3 text-center";
  if (onClick) {
    return (
...
function ProfessionalChart({
  ...
  // 화면에는 필요한 최근 봉만 먼저 그려 초기 렌더링과 확대·이동을 빠르게 합니다.
  // 원본 candles는 React Query 캐시에 그대로 남아 있으므로 분석 데이터는 유지됩니다.
  const rows = useMemo(() => {
    const renderLimit = timeframe === "ALL" ? 1_500 : /m|H/.test(timeframe) ? 600 : 1_000;
    return buildChartRows(candles.slice(-renderLimit));
  }, [candles, timeframe]);
  ...
  useEffect(() => {
    localStorage.setItem("sa-chart-main-height-v1", String(chartHeight));
  }, [chartHeight]);
  ...
  const beginChartResize = useCallback((event: React.PointerEvent<HTMLButtonElement>) => {
    event.preventDefault();
    const startY = event.clientY;
    const startHeight = chartHeight;
    const maximum = fullscreen ? Math.max(760, Math.floor(window.innerHeight * 0.82)) : 760;
    const move = (moveEvent: PointerEvent) => {
      setChartHeight(Math.min(maximum, Math.max(260, startHeight + moveEvent.clientY - startY)));
    };
    const finish = () => {
  ...
  return (
    <div className="space-y-2">
      <div className="mb-1 flex flex-wrap items-center justify-end gap-1.5">
        <span className="mr-auto text-[10px] font-bold text-muted-foreground">차트 높이</span>
        {[
          [300, "작게"],
          [360, "기본"],
          [520, "크게"],
          ...
          <button
            key={String(value)}
            type="button"
            onClick={() => setChartHeight(Number(value))}
            className={cn(
              "rounded-lg border border-card-border px-2.5 py-1.5 text-[10px] font-extrabold",
              Math.abs(chartHeight - Number(value)) < 12 ? "bg-primary text-primary-foreground" : "bg-secondary text-muted-foreground",
            )}
          >
            {label}
          </button>
      ...
      <button
        type="button"
        onPointerDown={beginChartResize}
        ...
        aria-label="메인 차트 높이 조절"
        title="위아래로 끌어서 메인 차트 높이 조절"
      >
...
function PriceChartCanvas({
  ...
  const lineSeriesRef = useRef<Record<string, AnyObj>>({});
  const previousRowsRef = useRef<ChartCandleRow[]>([]);
  const clickableMarkersRef = useRef<AnyObj[]>([]);
  ...
  // 차트와 시리즈는 데이터 갱신 때마다 다시 만들지 않고, 설정이 바뀔 때만 재구성합니다.
  useEffect(() => {
    const container = containerRef.current;
    if (!container) return;

    const chart = createChart(container, {
      ...chartBaseOptions(resolvedHeight, indicators.priceGrid),
      width: Math.max(container.clientWidth, 1),
      timeScale: {
        ...chartBaseOptions(resolvedHeight, indicators.priceGrid).timeScale,
    ...
    const addLineSeries = (
      ...
      lineSeriesRef.current[key] = chart.addLineSeries({
        color,
        lineWidth,
        lineStyle,
    ...
    if (indicators.sma5) addLineSeries("sma5", "#f59e0b", 2);
    if (indicators.sma20) addLineSeries("sma20", "#22c55e", 2);
    if (indicators.sma60) addLineSeries("sma60", "#a855f7", 2);
    if (indicators.sma120) addLineSeries("sma120", "#ec4899", 2);
    if (indicators.bollinger) {
      addLineSeries("bollingerUpper", "#14b8a6", 1, LineStyle.Dashed);
      addLineSeries("bollingerMiddle", "#14b8a6", 1, LineStyle.Dotted);
      addLineSeries("bollingerLower", "#14b8a6", 1, LineStyle.Dashed);
    }
    if (indicators.vwap) addLineSeries("vwap", "#06b6d4", 2);
    if (indicators.ichimoku) {
      addLineSeries("ichimokuConversion", "#ef4444", 1);
      addLineSeries("ichimokuBase", "#3b82f6", 1);
      addLineSeries("ichimokuSpanA", "#22c55e", 1, LineStyle.Dashed);
      addLineSeries("ichimokuSpanB", "#f97316", 1, LineStyle.Dashed);
    }
    ...
    const clickHandler = (param: AnyObj) => {
      ...
      const matches = clickableMarkersRef.current.filter(
        (marker) => Number(marker.time) === Number(param.time),
      );
  ...
  // 실시간 데이터는 차트를 다시 만들지 않고 마지막 봉만 update 합니다.
  useEffect(() => {
    const candleSeries = candleSeriesRef.current;
    const chart = chartRef.current;
    if (!candleSeries || !chart || rows.length < 2) return;
    ...
    const onlyLatestChanged =
      ...
      previous.length > 0 &&
      Number(previous[previous.length - 1].time) === Number(rows[rows.length - 1].time);

    if (onlyLatestChanged) {
      candleSeries.update(candleData[candleData.length - 1]);
    } else {
      candleSeries.setData(candleData);
      if (previous.length === 0 || Number(previous[previous.length - 1]?.time) !== Number(rows[rows.length - 1]?.time)) {
        chart.timeScale().fitContent();
      }
    }
    previousRowsRef.current = rows;

    const lines = lineSeriesRef.current;
    lines.sma5?.setData(movingAverageData(rows, 5));
    lines.sma20?.setData(movingAverageData(rows, 20));
    lines.sma60?.setData(movingAverageData(rows, 60));
    lines.sma120?.setData(movingAverageData(rows, 120));
    if (lines.bollingerUpper || lines.bollingerMiddle || lines.bollingerLower) {
      const band = bollingerData(rows);
      lines.bollingerUpper?.setData(band.upper);
      lines.bollingerMiddle?.setData(band.middle);
      lines.bollingerLower?.setData(band.lower);
    }
    lines.vwap?.setData(vwapData(rows));
    if (lines.ichimokuConversion || lines.ichimokuBase || lines.ichimokuSpanA || lines.ichimokuSpanB) {
      const cloud = ichimokuData(rows);
      lines.ichimokuConversion?.setData(cloud.conversion);
      lines.ichimokuBase?.setData(cloud.base);
      lines.ichimokuSpanA?.setData(cloud.spanA);
      lines.ichimokuSpanB?.setData(cloud.spanB);
    }
  }, [rows]);
  ...
  useEffect(() => {
    ...
    candleSeries.setMarkers(
      [...analysisForDisplay, ...actualTradeMarkers].sort(
...
function FinancialTab({
  ...
  const [financialDetail, setFinancialDetail] = useState<{ title: string; text: string } | null>(null);
  ...
  const performanceCards = [
    ...
    },
    {
      label: "총자산",
      color: "bg-amber-500",
      values: annualRows.map((row) => financialValue(row.assets, row.totalAssets)),
    },
    {
      label: "총부채",
      color: "bg-rose-500",
      values: annualRows.map((row) => financialValue(row.debt, row.totalLiabilities, row.liabilities)),
    },
    {
      label: "자본총계",
      color: "bg-indigo-500",
      values: annualRows.map((row) => financialValue(row.equity, row.totalEquity, row.stockholdersEquity)),
    },
    {
      label: "영업현금흐름",
      color: "bg-teal-500",
      values: annualRows.map((row) => financialValue(row.operatingCashFlow, row.cashFromOperations, row.netCashProvidedByOperatingActivities)),
    },
    {
      label: "현금성자산",
      color: "bg-sky-500",
      values: annualRows.map((row) => financialValue(row.cash, row.cashAndCashEquivalents, row.cashEquivalents)),
    },
  ];
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      >
        ...
        {metricModalOpen && (
          <Modal title={`${selectedMetric.label} · ${selectedMetric.status}`} onClose={() => setMetricModalOpen(false)}>
            <div className="mb-3 rounded-xl bg-primary/10 p-3 text-center text-lg font-extrabold text-primary">{selectedMetric.valueText}</div>
            <div className="space-y-2">
              <ExplanationBlock label="지표 뜻" text={selectedMetric.meaning} />
              <ExplanationBlock label="현재 수치 해석" text={selectedMetric.interpretation} />
        ...
        >
          {annualRows.length ? (
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              {performanceCards.filter((card) => card.values.some((value) => value != null)).map((card) => (
                <FinancialTrendCard
                  key={card.label}
                  label={card.label}
                  values={card.values}
                  ...
                  color={card.color}
                  onOpen={() => {
                    const availableValues = card.values.filter((value): value is number => value != null && Number.isFinite(value));
                    const first = availableValues[0];
                    const latest = availableValues[availableValues.length - 1];
                    const direction = first == null || latest == null ? "흐름을 계산할 데이터가 부족합니다." : latest >= first ? "최근 값이 과거 값보다 증가하는 흐름입니다." : "최근 값이 과거 값보다 감소하는 흐름입니다.";
                    setFinancialDetail({ title: `${card.label} 설명`, text: `${card.label}의 연도별 실제 제공값을 비교한 차트입니다. ${direction} 단일 항목만으로 판단하지 말고 매출·이익·부채·현금흐름을 함께 확인하세요.` });
                  }}
                />
              ))}
            </div>
      ...
      <div className="order-2">
        <SectionCard title="실적" subtitle="기간별 매출과 이익">
          ...
          {rows.length ? (
            <button type="button" onClick={() => setFinancialDetail({ title: `${period === "quarterly" ? "분기별" : "연별"} 실적 설명`, text: "매출액·영업이익·순이익의 실제 기간별 값을 비교합니다. 매출이 늘어도 이익이 줄 수 있으므로 세 항목의 방향과 일회성 요인을 함께 확인하세요." })} className="w-full text-left">
      ...
      {financialDetail && (
        <Modal title={financialDetail.title} onClose={() => setFinancialDetail(null)}>
...
function contentTimestamp(item: AnyObj): number {
  const raw = String(
    item.date ?? item.filingDate ?? item.rcept_dt ?? item.publishedAt ?? item.published_at ?? item.time ?? item.acceptedAt ?? "",
  ).trim();
  ...
  if (digits.length >= 8) {
    ...
    const parsedDigits = new Date(year, Math.max(0, month - 1), day, hour, minute).getTime();
...
function FilingTab({ ticker, market, filings, summary }: { ticker: string; market: Market; filings: AnyObj[]; summary: string }) {
  ...
  const renderItems = (items: AnyObj[]) => <div className="space-y-2">{items.map((item, index) => {
    ...
    const date = formatContentDate(item.date ?? item.filingDate ?? item.rcept_dt ?? item.acceptedAt);
...
function FinancialTrendCard({
  ...
  return (
    <button type="button" onClick={onOpen} className="w-full rounded-2xl border border-card-border bg-secondary/35 p-3 text-left transition active:scale-[0.99]">
