...
import { ArrowLeft, Clock3, LogIn, LogOut, UserPlus } from 'lucide-react';
      ...
      {!auth.loading && auth.user ? <Card>
        <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            <p className="truncate text-sm font-bold">{auth.displayName ?? '사용자'}</p>
            <span data-testid="membership-label" className="mt-1 inline-flex rounded-full bg-secondary px-2.5 py-1 text-[11px] font-semibold">{MEMBER_TIER_LABELS[auth.membershipLevel]}</span>
          </div>
          <div className="flex shrink-0 gap-2">
            {auth.isAdmin ? <button type="button" onClick={() => navigate('/admin')} className="min-h-10 rounded-xl bg-primary px-3 text-xs font-semibold text-primary-foreground">회원 관리</button> : null}
            <button type="button" onClick={() => void auth.signOut()} className="flex min-h-10 items-center justify-center gap-1.5 rounded-xl border border-card-border px-3 text-xs font-semibold"><LogOut className="h-4 w-4" />로그아웃</button>
          </div>
        </div>
        {stateMessage ? <div className="mt-3 flex items-center gap-2 rounded-xl bg-warning/10 px-3 py-2 text-xs font-semibold text-warning"><Clock3 className="h-4 w-4 shrink-0" /><span className="min-w-0 break-words">{stateMessage}</span></div> : null}
