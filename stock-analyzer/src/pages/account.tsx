...
export default function AccountPage() {
  ...
  return <div className="flex h-full min-h-0 flex-col overflow-y-auto bg-background">
    <header className="border-b border-card-border px-4 py-4"><div className="flex items-center gap-3">
      <button type="button" aria-label="뒤로 가기" onClick={() => navigate(auth.isApproved ? '/settings' : '/login')} className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-card-border"><ArrowLeft className="h-5 w-5" /></button>
    ...
    <main className="min-w-0 flex-1 px-4 pb-28 pt-5">
      {!auth.configured && <Card><p className="font-extrabold text-destructive">계정 저장소 설정이 필요합니다.</p><p className="mt-2 text-sm text-muted-foreground">계정 저장소 연결 정보를 관리자 설정에 등록해 주세요.</p></Card>}
      ...
      {!auth.loading && auth.user ? <Card>
        ...
        {auth.isAdmin && <button type="button" onClick={() => navigate('/admin')} className="mt-4 min-h-11 w-full rounded-2xl bg-primary px-4 py-3 text-sm font-extrabold text-primary-foreground">관리자 회원 관리</button>}
        <button type="button" onClick={() => void auth.signOut()} className="mt-3 flex min-h-11 w-full items-center justify-center gap-2 rounded-2xl border border-card-border px-4 py-3 text-sm font-extrabold"><LogOut className="h-4 w-4" />로그아웃</button>
      </Card> : !auth.loading && auth.configured && <Card>
        <div className="flex rounded-2xl bg-secondary p-1"><button type="button" aria-label="로그인 탭" aria-pressed={!register} onClick={() => setRegister(false)} className={`min-h-11 flex-1 rounded-xl px-3 text-sm font-bold ${!register ? 'bg-card shadow' : ''}`}>로그인</button><button type="button" aria-label="회원가입 탭" aria-pressed={register} onClick={() => setRegister(true)} className={`min-h-11 flex-1 rounded-xl px-3 text-sm font-bold ${register ? 'bg-card shadow' : ''}`}>회원가입</button></div>
        <form onSubmit={submit} className="mt-5 space-y-4">
          ...
          <button type="submit" disabled={busy} className="flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl bg-primary px-4 py-3.5 text-sm font-extrabold text-primary-foreground disabled:opacity-50">{register ? <UserPlus className="h-4 w-4" /> : <LogIn className="h-4 w-4" />}{busy ? '처리 중...' : register ? '가입 신청' : '로그인'}</button>
        </form>
      </Card>}
      ...
      {(notice || error) && <p role={error ? 'alert' : 'status'} className={`mt-3 break-words rounded-2xl p-4 text-sm font-bold ${error ? 'bg-destructive/10 text-destructive' : 'bg-positive/10 text-positive'}`}>{error || notice}</p>}
