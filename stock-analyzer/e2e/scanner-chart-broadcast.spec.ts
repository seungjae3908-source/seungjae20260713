...
async function waitForIsolatedVite(url: string, child: ChildProcess) {
  ...
  while (Date.now() < deadline) {
    if (child.exitCode != null) throw new Error(`격리 Vite 서버가 조기 종료됐습니다.\n${isolatedViteOutput}`);
    try {
...
function timeframeSeconds(timeframe: string) {
  const values: Record<string, number> = { '1m': 60, '3m': 180, '5m': 300, '15m': 900, '30m': 1_800, '1H': 3_600, '4H': 14_400, '1D': 86_400 };
...
function mixedCandles(base: number, timeframe: string) {
  ...
  return [
    ...
    { time: 1_700_000_000 + rows.length * step, open: 7_000, high: 6_990, low: 6_980, close: 7_010, volume: 10 },
  ];
}
...
function monitorBrowser(page: Page): BrowserEvidence {
  const evidence: BrowserEvidence = {
    consoleErrors: [], expectedConsoleDiagnostics: [], pageErrors: [], unhandledRejections: [],
    unexpectedRequestFailures: [], apiHttpErrors: [], orderRequests: [],
  };
  page.on('console', (message) => {
    if (message.type() !== 'error') return;
    const text = message.text();
    if (text.startsWith('[e2e-unhandledrejection]')) evidence.unhandledRejections.push(text);
    else if (/Failed to load resource.*(?:429|500|502|503)/i.test(text)) evidence.expectedConsoleDiagnostics.push(text);
    else evidence.consoleErrors.push(text);
  });
  page.on('pageerror', (error) => evidence.pageErrors.push(error.message));
  page.on('requestfailed', (request: Request) => {
    const errorText = request.failure()?.errorText ?? '';
    if (!/ERR_ABORTED|NS_BINDING_ABORTED/i.test(errorText)) evidence.unexpectedRequestFailures.push(`${request.method()} ${request.url()} ${errorText}`);
  });
  page.on('request', (request) => {
    if (request.method() !== 'GET' && ORDER_ENDPOINT.test(request.url())) evidence.orderRequests.push(`${request.method()} ${request.url()}`);
  });
  page.on('response', (response) => {
    if (response.url().includes('/api/') && response.status() >= 400) evidence.apiHttpErrors.push({ status: response.status(), url: response.url() });
  });
  return evidence;
}
...
async function installApplicationMocks(page: Page, state: MockState) {
  ...
  await page.route('**/api/market/scan**', (route) => route.fulfill({
    status: 200,
    contentType: 'application/json',
    body: JSON.stringify({ ok: true, searchRunId: 'legacy-chart-broadcast:e2e', timeframe: '1D', supportedIndicators: [], cards: [] }),
  }));
  ...
  await page.route(/\/api\/stocks\/[^/]+\/(?:chart|candles)(?:\?|$)/, async (route) => {
    ...
    if (state.scenario === 'rate-limited') {
      await route.fulfill({ status: 429, contentType: 'application/json', body: JSON.stringify({ message: 'RATE_LIMITED' }) }); return;
    }
    if (state.scenario === 'server-error') {
      await route.fulfill({ status: 503, contentType: 'application/json', body: JSON.stringify({ message: 'SERVER_FAILURE' }) }); return;
    }
    ...
    const candles = state.scenario === 'invalid-only' ? invalidOnlyCandles()
      : state.scenario === 'empty' ? []
        : state.scenario === 'mixed' ? mixedCandles(base, timeframe)
          : validCandles(base, timeframe);

    await route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({ ticker, timeframe, provider: 'legacy-chart-broadcast-fixture', fetchedAt: '2020-01-01T00:00:00.000Z', updatedAt: '2020-01-01T00:00:00.000Z', candles }),
    }).catch(() => undefined);
  });
}
...
test.describe('mobile scanner legacy ChartBroadcastPanel contract', () => {
  ...
  test.beforeAll(async () => {
    ...
    isolatedVite = spawn(pnpm, ['exec', 'vite', '--config', 'vite.config.ts', '--host', '127.0.0.1', '--port', String(port), '--strictPort'], {
      ...
      env: { ...process.env, VITE_PHASE4_E2E: 'true', VITE_PHASE11_E2E: 'true', VITE_SUPABASE_URL: `https://${SUPABASE_HOST}`, VITE_SUPABASE_ANON_KEY: ACCESS_TOKEN },
