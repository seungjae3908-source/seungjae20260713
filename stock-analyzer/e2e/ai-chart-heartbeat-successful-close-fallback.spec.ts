...
class HeartbeatSendFailureCloseSuccessSocketFake {
  static instances: HeartbeatSendFailureCloseSuccessSocketFake[] = [];
  ...
  constructor(_url: string) {
    HeartbeatSendFailureCloseSuccessSocketFake.instances.push(this);
  }
...
test('heartbeat send failure fails closed even when socket close succeeds', () => {
  ...
  try {
    HeartbeatSendFailureCloseSuccessSocketFake.instances = [];
    Object.defineProperty(globalThis, 'WebSocket', {
      configurable: true,
      writable: true,
      value: HeartbeatSendFailureCloseSuccessSocketFake,
    });

    const currentNow = 23_000;
    const firstStatuses: Array<{ status: string; reason: string }> = [];
    const first = createAiChartPublicStreamClient({
      market: 'BITGET',
      symbol: 'HEARTBEATCLOSE',
      now: () => currentNow,
      setTimeoutFn,
      clearTimeoutFn,
      onTrade: () => {},
    ...
    first.start();
    expect(HeartbeatSendFailureCloseSuccessSocketFake.instances).toHaveLength(1);
    const firstSocket = HeartbeatSendFailureCloseSuccessSocketFake.instances[0];
