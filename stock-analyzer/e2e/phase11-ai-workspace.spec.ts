...
const scannerCard = {
  ticker: '000660', symbol: '000660', name: 'SK하이닉스', assetClass: 'stock', market: 'KR', exchange: 'KRX', currency: 'KRW',
  assetType: 'STOCK', listingStatus: 'LISTED', price: 205000, changePercent: 1.2, score: 88, confidence: 82, dataCompleteness: 94,
  matched: ['거래량 증가', '5일선 돌파', 'AI 점수 상위'], notMatched: [], unverified: [], missing: [], breakoutProbability: 75,
  expectedPeriod: '단기', entry: ['200000'], stop: ['190000'], matchCount: 3, selectedCount: 3, riskLevel: 'LOW', riskScore: 10,
  liquidity: 1_000_000_000, volume: 1_000_000, tradingValue: 205_000_000_000, spreadPercent: 0.05, volatilityPercent: 1.2,
  dataState: 'complete', analyzedAt: '2026-08-03T00:00:00Z', observedAt: '2026-08-03T00:00:00Z', expiresAt: '2026-08-06T00:00:00Z',
  signalId: 'signal:phase11-sk-hynix', direction: 'LONG', signalState: 'WATCHING', strongSignalEligible: true, warnings: [],
  dataSources: ['market-quote', 'market-candles'],
  evidence: [{ key: '거래량 증가', label: '거래량 증가', status: 'matched', source: 'market-candles-volume', observedAt: '2026-08-03T00:00:00Z', reasons: ['실제 거래량 증가를 확인했습니다.'] }],
  pricePlan: { entryZone: { from: 200000, to: 205000 }, invalidation: 190000, stopLoss: 190000, targets: [220000, 230000], riskReward: 1.5 },
  scoreBreakdown: { trend: { score: 88, status: 'ok', reasons: ['상승 구조'] } },
};

const scannerPayload = {
  ok: true, provider: 'fixture', searchRunId: 'scan:test:1D:1', requestId: 'scan:test:1D:1', assetClass: 'stock', timeframe: '1D', market: 'KR',
  supportedIndicators: ['거래량 증가', '5일선 돌파', 'AI 점수 상위'], rows: [scannerCard], cards: [scannerCard], results: [scannerCard], alerts: [], failures: [],
  execution: { requestedCount: 1, startedCount: 1, completedCount: 1, excludedCount: 0, providerErrorCount: 0, timeoutCount: 0, partial: false, timedOut: false, cancelled: false, duplicate: false, elapsedMs: 10, deadlineMs: 12000, itemTimeoutMs: 3500, maxConcurrency: 1 },
  universe: { totalCount: 1, cursor: 0, nextCursor: null, source: 'krx-symbol-master', partial: false, stale: false, listingStatusCoverage: 'listed-or-unknown' },
  dataState: 'complete', message: '1종목 공개 데이터 분석을 완료했습니다.', generatedAt: '2026-08-03T00:00:00Z', orderSubmitted: false, exchangeRequestSent: false,
};

async function mockWorkspace(page: Page) {
  await page.route('**/api/**', (route) => route.fulfill({ status: 200, contentType: 'application/json', body: '{}' }));
  await page.route('**/api/market/scan**', (route) => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(scannerPayload) }));
  await page.route('**/api/stocks/*/chart**', (route) => route.fulfill({
    status: 200, contentType: 'application/json',
    body: JSON.stringify({ ticker: route.request().url().includes('000660') ? '000660' : '005930', timeframe: '1D', provider: 'fixture', fetchedAt: '2026-08-03T00:00:00Z', candles }),
  }));
...
test('AI chat handles send, refusal response, and cancellation-safe UI', async ({ page }) => {
  ...
  await page.route('**/api/ai/chat', async (route) => {
    ...
    await route.fulfill({
      status: 200, contentType: 'application/json',
      body: JSON.stringify({ ok: true, kind: request.message.includes('주문') ? 'refusal' : 'answer', answer: request.message.includes('주문') ? '주문 작업은 실행할 수 없습니다.' : 'RSI는 가격 변화의 상대적 강도를 보는 기술지표입니다.' }),
    }).catch(() => undefined);
  });
  await page.goto('/__phase11-ai-chat-e2e');
  const input = page.getByPlaceholder(/질문 입력/);
  await input.fill('RSI를 설명해줘'); await input.press('Enter');
  await expect(page.getByText(/상대적 강도/)).toBeVisible();
  await input.fill('실제 주문 실행해줘'); await input.press('Enter');
  await expect(page.getByText('주문 작업은 실행할 수 없습니다.')).toBeVisible();
  await input.fill('조금 느리게 설명해줘'); await input.press('Enter');
