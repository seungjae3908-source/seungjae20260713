...
test('shared search component applies capability policy and preserves last-good results', () => {
...
test('current-main unified search page hides unavailable market controls and rejects disallowed navigation', () => {
  ...
  expect(pageSource).toContain('if (nextMarket && !isUnifiedSearchMarketAllowed(nextMarket, allowedMarkets)) return;');
