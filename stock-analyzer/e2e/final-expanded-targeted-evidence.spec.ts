...
test('slow Financial stays secondary while stock-info primary quote and navigation remain usable', async ({ page }) => {
  ...
  await expectPrimaryStockInfoUsable(page);
  const financialToggle = page.getByRole('button', { name: /재무요약/ });
  await expect(financialToggle).toContainText('데이터를 불러오는 중입니다.');
  await financialToggle.click();
  const financialPanel = financialToggle.locator('xpath=..');
