...
for (const viewport of [
  ...
  test(`desktop scanner uses split workspace, bounded desktop navigation, and keeps coin spot functional at ${viewport.width}px`, async ({ page }) => {
    const requests: string[] = [];
    await installMocks(page, requests);
    await page.setViewportSize(viewport);
    await page.goto('/__phase11-technical-workspace-e2e');
    ...
    expect(navBox.x).toBeGreaterThanOrEqual(90);
