...
for (const route of ROUTES) {
  test(`${route.title} owns content scrolling without moving BottomNav`, async ({ page }) => {
    ...
    expect(await scroll.evaluate((node) => node.scrollHeight > node.clientHeight + 100)).toBe(true);
    ...
    await scroll.hover({ position: { x: 24, y: Math.min(120, Math.max(20, (await scroll.boundingBox())?.height ?? 100)) } });
