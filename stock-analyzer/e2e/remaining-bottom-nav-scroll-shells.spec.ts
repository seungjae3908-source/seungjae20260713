...
for (const route of ROUTES) {
  test(`${'label' in route ? route.label : route.title} owns content scrolling without moving BottomNav`, async ({ page }) => {
    ...
    const compositorPrimed = await scroll.evaluate((node) => {
      ...
      // This test makes an initially short overflow:auto node scrollable after paint.
      // Prime Chromium's compositor scroll node, then restore the real wheel start.
      const scrollNode = node as HTMLElement;
      scrollNode.scrollTop = 1;
      const primed = scrollNode.scrollTop === 1;
      scrollNode.scrollTop = 0;
      return primed;
    });
    expect(compositorPrimed).toBe(true);
