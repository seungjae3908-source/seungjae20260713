...
export const SCHEMA_VERSION = 'paper-forward-risk-window-rollover-v1';
...
export function expectedRiskWindow(state, now) {
  if (!Number.isFinite(now?.getTime?.()) || !state?.riskState) fail('RISK_WINDOW_INVALID');
  ...
  if (!dailyWindowRolled && !weeklyWindowRolled) fail('RISK_WINDOW_ALREADY_CURRENT');
  return {
    riskState: {
      dayKey: currentDay,
      weekKey: currentWeek,
...
export function buildRolloverAction({ stateDigest, symbol, price }) {
  if (!digest(stateDigest)) fail('ROLLOVER_STATE_DIGEST_INVALID');
  if (!/^[A-Z0-9]{2,20}$/u.test(symbol) || !finite(price) || price <= 0) fail('ROLLOVER_ANCHOR_INVALID');
  return {
    type: 'mark_price',
...
export function createAccountingOnlyRolloverState(beforeState, action, nowIso) {
  const now = new Date(nowIso);
  if (!Number.isFinite(now.getTime()) || now.toISOString() !== nowIso) fail('ROLLOVER_TIMESTAMP_INVALID');
  const eventValid = /^[A-Za-z0-9._:-]{1,120}$/u.test(String(action?.eventId ?? ''));
  const markPrice = action?.type === 'mark_price'
    && /^[A-Z0-9]{2,20}$/u.test(String(action?.symbol ?? ''))
    && finite(action?.price) && action.price > 0;
  if (!eventValid || !markPrice) fail('ROLLOVER_ACTION_INVALID');
  const expected = expectedRiskWindow(beforeState, now);
  const beforeEvents = Array.isArray(beforeState?.processedEventIds) ? beforeState.processedEventIds : [];
  if (beforeEvents.includes(action.eventId)) fail('ROLLOVER_EVENT_ALREADY_PROCESSED');
  const afterState = clone(beforeState);
  afterState.riskState = clone(expected.riskState);
  ...
  if (canonicalJson(invariantStateView(afterState)) !== canonicalJson(invariantStateView(beforeState))) {
    fail('ROLLOVER_NON_ACCOUNTING_STATE_CHANGED');
  }
  if (openPositions(afterState).length !== 0 || pendingOrders(afterState).length !== 0
    || !zero(afterState.account?.usedMargin) || !zero(afterState.account?.unrealizedPnl)) fail('ROLLOVER_RESULT_NOT_FLAT');
  return { afterState, ...expected };
}

export function validateRolloverResponse(body, { beforeState, targetSha, publisherDigest, action, nowIso }) {
  if (!body || body.ok !== true || body.mode !== 'paper-only' || body.orderSubmitted !== false || body.exchangeRequestSent !== false) {
    fail('ROLLOVER_SAFETY_ENVELOPE_INVALID');
  }
  const result = body.result;
  if (!result || result.ok !== true || result.mode !== 'paper-only' || result.orderSubmitted !== false || result.exchangeRequestSent !== false) {
    fail('ROLLOVER_RESULT_INVALID');
  }
  if (result.duplicateEvent !== false || result.order !== null || result.position !== null || !Array.isArray(result.fills) || result.fills.length !== 0) {
    fail('ROLLOVER_MUST_NOT_TRADE');
  }
  const expected = createAccountingOnlyRolloverState(beforeState, action, nowIso);
  const afterState = result.state;
  if (canonicalJson(afterState) !== canonicalJson(expected.afterState)) fail('ROLLOVER_ACCOUNTING_DELTA_INVALID');
...
export async function runRiskWindowRollover() {
  ...
  const action = buildRolloverAction({ stateDigest: beforeSnapshot.stateDigestSha256, ...anchor });
  const now = new Date();
  const expectedWindow = expectedRiskWindow(beforeState, now);
  ...
  const validated = validateRolloverResponse(response.body, { beforeState, targetSha, publisherDigest, action, nowIso });
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    status: 'RISK_WINDOW_ROLLED',
    targetSha,
    sourceShaBefore: beforeSnapshot.sourceSha,
    sourceShaAfter: finalSnapshot.sourceSha,
    dailyWindowRolled: expectedWindow.dailyWindowRolled,
    weeklyWindowRolled: expectedWindow.weeklyWindowRolled,
    cumulativeRealizedPnlMutation: 0,
    consecutiveLossesPreserved: true,
    stateDigestChanged: beforeSnapshot.stateDigestSha256 !== finalSnapshot.stateDigestSha256,
    economicStatePreservedExceptRiskWindow: true,
...
const isDirect = process.env.PAPER_RISK_WINDOW_ROLLOVER_EXECUTE === '1'
  || (process.argv[1] && new URL(import.meta.url).pathname === new URL(`file://${process.argv[1]}`).pathname);
if (isDirect) {
  runRiskWindowRollover().catch((error) => {
    process.stderr.write(`[paper-risk-window-rollover] ${String(error?.code ?? 'PAPER_RISK_WINDOW_ROLLOVER_FAILED')}\n`);
    process.exit(1);
  });
}
