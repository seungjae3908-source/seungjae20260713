...
function spotEvidence(overrides = {}) {
  return {
    market: "CRYPTO_SPOT",
    provider: "upbit",
    source: "UPBIT_PUBLIC_WEBSOCKET",
    symbol: "KRW-BTC",
    quoteObservedAt: NOW - 100,
    tradeObservedAt: NOW - 120,
    lastTradePrice: 100,
    bids: [
      { price: 99.9, size: 2 },
      { price: 99.8, size: 5 },
...
function guardPolicy(overrides = {}) {
  return {
    marketData: dataPolicy(),
    maxSpreadBps: 30,
    maxPriceDeviationBps: 40,
    maxSlippageBps: 20,
...
function paperPolicy() {
  return {
    maxQuantityByMarket: { KR_STOCK: 1_000 },
...
test("provider mismatch, stale quote and crossed book fail closed", () => {
  assert.throws(
    () => normalizePublicMarketDataEvidence(spotEvidence({ provider: "bitget" }), dataPolicy()),
    (error) => error.code === "PUBLIC_MARKET_DATA_PROVIDER_MISMATCH",
  );
  assert.throws(
    () => normalizePublicMarketDataEvidence(spotEvidence({ quoteObservedAt: NOW - 5_000 }), dataPolicy()),
    (error) => error.code === "STALE_PUBLIC_QUOTE",
  );
  assert.throws(
    () => normalizePublicMarketDataEvidence(spotEvidence({ bids: [{ price: 100.2, size: 1 }] }), dataPolicy()),
    (error) => error.code === "CROSSED_PUBLIC_BOOK",
  );
});

test("future and stale public trade evidence fail closed", () => {
  assert.throws(
    () => normalizePublicMarketDataEvidence(spotEvidence({ tradeObservedAt: NOW + 500 }), dataPolicy()),
    (error) => error.code === "FUTURE_PUBLIC_TRADE",
  );
  assert.throws(
    () => normalizePublicMarketDataEvidence(spotEvidence({ tradeObservedAt: NOW - 5_000 }), dataPolicy()),
    (error) => error.code === "STALE_PUBLIC_TRADE",
  );
});
...
test("wide spread and fat-finger price deviation are explicit blockers", () => {
  const wide = assessExecutionGuards({
    intent: spotIntent(),
    marketData: spotEvidence({
      bids: [{ price: 99, size: 3 }],
      asks: [{ price: 101, size: 3 }],
    }),
    policy: guardPolicy({ maxSpreadBps: 50 }),
  });
  assert.ok(wide.blockers.includes("SPREAD_TOO_WIDE"));

  const deviated = assessExecutionGuards({
    intent: spotIntent({ referencePrice: 105 }),
    marketData: spotEvidence(),
    policy: guardPolicy({ maxPriceDeviationBps: 100 }),
  });
  assert.ok(deviated.blockers.includes("PRICE_DEVIATION_EXCEEDED"));
});

test("insufficient public depth and slippage fail closed", () => {
  const shallow = assessExecutionGuards({
    intent: spotIntent({ quantity: 10 }),
    marketData: spotEvidence(),
    policy: guardPolicy(),
  });
  assert.ok(shallow.blockers.includes("INSUFFICIENT_PUBLIC_DEPTH"));

  const slipped = assessExecutionGuards({
    intent: spotIntent({ quantity: 4 }),
    marketData: spotEvidence({
      asks: [
        { price: 100.1, size: 1 },
        { price: 101, size: 5 },
    ...
    policy: guardPolicy({ maxSlippageBps: 10, maxSpreadBps: 100 }),
  });
...
test("paper OMS supports ACCEPTED -> PARTIALLY_FILLED -> FILLED only", async () => {
  ...
  const filled = await gateway.applyPaperFill(order.orderId, {
    quantity: 6,
    price: 70_000,
...
test("paper overfill and fill-after-terminal are rejected", async () => {
  ...
  await assert.rejects(
    gateway.applyPaperFill(order.orderId, { quantity: 1, price: 70_000, observedAt: "2026-08-24T10:00:01Z" }),
    (error) => error.code === "INVALID_PAPER_FILL_TRANSITION",
  );
...
test("cancel/replace is planning-only and preserves exact order identity", async () => {
  ...
  const plan = buildCancelReplacePlan({
    ...
    replacementIntent: {
      ...order.intent,
      limitPrice: 69_900,
      idempotencyKey: "v04-replace-new-0001",
    },
  });
...
test("bracket OCO validates long and short price geometry without submitting children", () => {
  const longPlan = buildBracketPlan({
  ...
  assert.throws(
    ...
    (error) => error.code === "INVALID_BRACKET_PRICES",
  );
});

test("trailing stop remains an inactive plan requiring future public prices", () => {
  const plan = buildTrailingPlan({
    market: "CRYPTO_SPOT",
    symbol: "KRW-BTC",
    side: "BUY",
    activationPrice: 105_000_000,
    trailPercent: 1.5,
  });
